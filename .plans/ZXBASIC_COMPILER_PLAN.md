...
**Where to start:** Phase 2 in §14 (binder, types, constants, W-codes and error classes, CODEBANK
rules), under `src/main/kbasic/semantics/` (§3.3), working on the `Program` that `parseProgram`
(`src/main/kbasic/front-end.ts`) returns; `KBasicCompiler.runFrontEnd` is where the binder's
diagnostics join the front end's. Phase 0's pieces it will meet later: the compiler hands its program to the assembler as parsed units
