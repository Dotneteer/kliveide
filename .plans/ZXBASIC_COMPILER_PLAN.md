...
IDE. **Phase 3 (core code generation) is next, and it is gated:** R11's four design notes are
drafted in `.docs/` and must be approved by the project author before its code starts. Decisions D1–D12 settled
(§0.2–§0.3). See **Handoff**, immediately below, before doing anything else.
...
| Phase 1: IDE integration — `KBasicCompiler` (front end only; a build reports K001 "cannot generate code yet"), `ZxBasicDispatcher` on `zxbasic.compiler`, the worker's `checkFile` hook, exact error columns for `zxbas`, lexer-based `lineCanHaveBreakpoint` | `src/main/kbasic/{KBasicCompiler,breakpoints}.ts`, `src/main/zxb-integration/ZxBasicDispatcher.ts`, `MonacoEditor.tsx`, `zxBasLanguageProvider.ts`; `.ai/kbasic/klive-integration-map.md` is current; test `test/kbasic/compiler.test.ts` | that test |
...
**Where to start:** Phase 3 in §14, but **first R11** (§17.2): the four design notes in `.docs/`
(`kbasic-mir.md`, `kbasic-lir-regalloc.md`, `kbasic-string-ownership.md`,
`kbasic-debug-builder.md`) are drafted and wait for the project author's approval — each ends with
a table of decisions to confirm. No Phase 3 code before that approval; then fold the author's
answers into the notes and start with L5 (the binder's FASTCALL-locals check) and L1's
measurement.
Phase 3's input is the typed tree: `bind()` (`src/main/kbasic/semantics/binder.ts`) returns a
`BoundProgram` (`bound.ts`) whose expressions carry their `type` and, when constant, their folded
`constant`, and whose symbols (`symbols.ts`) say where each variable lives (`storage`, `at`,
`initial`, `bank`). Lowering starts from `KBasicCompiler.runFrontEnd`'s `bound`. Phase 0's pieces it
meets: the compiler hands its program to the assembler as parsed units (`parseSourceUnit`/
`compileProgram`, §4.1) followed by `runtimeUnits(...)` from `runtime-linker.ts`. Read
`src/main/kbasic/runtime/README.md` before adding a runtime module, and retire the walking-skeleton
spike (R3) in Phase 3.
...
| R11 | **Stage design notes.** | Phase 3 (MIR, LIR, allocation, strings, debug builder); Phase 7 (optimiser) | **Drafted 2026-09-26, awaiting the project author's approval:** `.docs/kbasic-mir.md`, `.docs/kbasic-lir-regalloc.md`, `.docs/kbasic-string-ownership.md`, `.docs/kbasic-debug-builder.md`, each ending with its open decisions (Q1–Q6, L1–L5, O1–O4, D1–D4). Before coding each stage, write a short design note in `.docs/` (the implementation-pattern docs folder): `kbasic-mir.md` (instruction set, types, SSA form, statement-id rules), `kbasic-lir-regalloc.md` (instruction objects, register classes, allocation, spill slots, the level-0 stack scheme), `kbasic-string-ownership.md` (temporaries, free points, by-value parameters, returned strings — the part most likely to leak), `kbasic-debug-builder.md` (joining list items and tags, the §8.2 guarantees and their validator). Each note is reviewed by the project author before its code starts. | The four notes exist and are approved before Phase 3 coding. |
