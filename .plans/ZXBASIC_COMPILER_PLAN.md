...
breakpoints only on statement lines. **Phase 2 done** (2026-09-26): the binder produces a typed tree
(names resolved, every expression typed, conversions explicit, constants folded ROM-exact); every
spec'd error class and warning has a code and a triggering test; the R8 semantics annex exists,
with the front-end questions decided or provisional (the zxbc oracle could not be installed in the
cloud container, so "provisional" entries await it). Semantic diagnostics checked in the running
IDE. **Phase 3 done** (2026-09-26): R11's design notes approved; MIR, lowering, the level-0 stack
machine, the emitter, runtime linking and the debug-info builder with the classic tables and its
validator; integers (8/16/32-bit), Strings with the ownership rules, PRINT and colours, control
flow, SUB/FUNCTION in both conventions, arrays (global, parameter, local), inline asm, BORDER,
...
| Phase 1: the front end — diagnostics and spans, lexer, preprocessor, syntax tree and parser, header options | `src/main/kbasic/{diagnostics,front-end}.ts`, `src/main/kbasic/syntax/`, `src/main/kbasic/options/`; tests `test/kbasic/syntax/` (`spec-forms.test.ts` is keyed by the spec's exact EBNF strings), `test/kbasic/options/` | those tests |
| Phase 2: semantics — type model (`types.ts`), constant folding (`constants.ts`), symbols and scopes (`symbols.ts`), the typed tree (`bound.ts`), the binder (`binder.ts` statements and declarations, `expressions.ts` expressions, calls and built-ins), flow warnings and bank checks (`flow.ts`), the diagnostic code table (`codes.ts`) | `src/main/kbasic/semantics/`; run from `KBasicCompiler.runFrontEnd` when the source parses cleanly; tests `test/kbasic/semantics/` (`diagnostics.test.ts` has a case for every code in `codes.ts` and fails on a code without one) | those tests |
| The R8 semantics annex | `semantics` in `.ai/zxbasic-syntax/zxbasic-syntax.json`; the contract test `test/ai-notes/zxbasic-syntax.test.ts` checks every entry | the contract test |
| Phase 1: IDE integration — `KBasicCompiler` (since Phase 3 a build generates code for the 48K; K001 is gone), `ZxBasicDispatcher` on `zxbasic.compiler`, the worker's `checkFile` hook, exact error columns for `zxbas`, lexer-based `lineCanHaveBreakpoint` | `src/main/kbasic/{KBasicCompiler,breakpoints}.ts`, `src/main/zxb-integration/ZxBasicDispatcher.ts`, `MonacoEditor.tsx`, `zxBasLanguageProvider.ts`; `.ai/kbasic/klive-integration-map.md` is current; test `test/kbasic/compiler.test.ts` | that test |
...
npx vitest run --config build/vitest.config.ts --project node \
  test/kbasic test/harness/sp48 test/ai-notes                                # all pass
...
- The pipeline: `runFrontEnd` → `lowerProgram` (MIR) → `selectFunction` per function (LIR lines) →
  `emitProgram` (text + a line table of statement ids and call sites) → `parseSourceUnit` +
  `runtimeUnits` + `compileProgram` → `buildDebugInfo` (classic tables, statement ranges, validator
  problems). `generateProgram` in `codegen.ts` is the whole of it; `KBasicCompiler.build` wraps it.
- Level 0 is a stack machine: every value is in its accumulator (A, HL, DE:HL), an earlier operand
  on the Z80 stack, so the stack is empty at every statement marker (G4 by construction). Levels
  1–3 are Phase 7; `optimize` is accepted and generates level 0 (the corpus runner's `LEVELS`).
- Codes: E501 not generated yet, E502 a target other than the 48K, E503 an assembler error in an
  `ASM` block (reported at the block's line), E599 an internal error (a bug).
- Names (plan §8.4): globals and routines `_name`; an array's descriptor `_a`, its data `_a.data`
  (an `equ` for `DIM ... AT`), tables `_a.dims`, `_a.lbound`, `_a.ubound`; a local array's tables
  `_routine.a.dims`; labels `_label.x`; epilogues `_name.leave`; strings `__str<n>`; blocks `__b<n>`;
  selector-local labels `__k<fn><n>`. The binder's address constant for `@a(i)` is
  `{symbol: "array:a"}` (the data); `@a` is the descriptor.
- Arrays: global and local arrays with known bounds are addressed inline; array parameters through
  `core.ArrayAddress`. An upper-bound table is emitted for every array when the program asks
  `UBOUND` at run time (runtime-abi §2.3). Local arrays' data are on the heap (`ArrayAlloc` raises
  "4 Out of memory"), freed at the epilogue.
- A jump in a block that holds nothing else is glue (sid −1), or a branch lands inside the
  statement before it (G2; `.docs/kbasic-mir.md` §3).
- The classic `sourceMap` columns follow the assembler's convention (0-based start, exclusive end);
  the `zxbas` language provider marks whole lines, so the IDE does not draw them yet.
- The 48K harness holds keys (`keyDown`/`keyUp`). With keys held and the CPU in the ROM's editor,
  the editor reacts to them between calls: park the CPU in an idle loop while frames run (the
  keyboard runtime test does). From the IDE prompt, `debug` builds and starts debugging;
  `klive.debug` refuses interactive use.

**Phase 2 facts a later phase must know:**

- Diagnostic codes: E401–E430 for the spec's error classes, E450–E454 for CODEBANK, the spec's W-codes, and K401–K407
  for the spec's uncoded warnings and Klive's own (`src/main/kbasic/semantics/codes.ts` is the
  single table). A new code goes in that table and gets a case in `diagnostics.test.ts`.
- The binder runs only when the front end reported no error (syntax errors would only cascade).
  Warnings listed in `'@disable-warning` are dropped at the end of `runFrontEnd`.
- Literal-only constant arithmetic is exact (`200 + 100` is a UInteger 300); anything typed wraps in
  its type, exactly as the program would compute it (R8 `constant-arithmetic-*`). Float constants
  fold through `float40`; `^` folds provisionally (R8 `constant-power`).
- Labels and routine headers are hoisted (a FUNCTION called before its definition gets its real
  signature, R8 `forward-function-call`); every other name follows source order. A label's address
  constant is `{kind: "address", symbol: "label:<name>"}`; a global's is its own name.
- W150 and W170 are reported whenever `optimize > 0`, which is the default (2), as upstream does.
- CODEBANK errors that need code sizes or assembly-level analysis — a bank larger than the window,
  ORG inside a bank, the window overlapping the program, `jp`/`call` between banks inside ASM — are
  Phase 6's; Phase 2 covers the source-level rules (E450–E454, W900–W920).
- Driving the IDE from a script (`scripts/doc-shots/harness.cjs`): after `newp … -o` the IDE page
  is replaced, so create with `newp` and then `open` the folder to keep using the prompt.

**Phase 1 facts a later phase must know:**

- Diagnostic codes: E0xx options, E1xx lexer, E2xx preprocessor, E3xx parser (Phase 2 takes E4xx
  and up); warnings keep the spec's W-codes; K-codes are Klive's own warnings and notes (K010
  unknown option, K011 unsupported NextBuild option, K012 header in an included file, K001/K002/
  K000 from `KBasicCompiler`). Errors carry spans; `toErrorInfo` maps them through `#line`.
- The preprocessor keeps a `#define` body as raw text and reports its lexical problems only where
  the macro is expanded; a macro may be named like a keyword.
- The parser recovers at the statement level and never throws on a missing block closer
  (`afterMissingCloser`); a SUB/FUNCTION/CODEBANK header must end its line (`SUB f: END SUB` is an
...
- `test/harness/sp48/` — the 48K execution harness; `test/kbasic/corpus/` — the test corpus (R10);
  `scripts/kbasic-ide-check.cjs` — the IDE-level breakpoint check. (The walking-skeleton spike of
  §17.1, R3, was retired in Phase 3.)
