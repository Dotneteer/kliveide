...
## 4e. Execution log — Phase 6 (2026-09-03)

Commit `ec2a926`. **Target stack reached: Nextra 4.6.1 + React 19.1 + Next 16.3.4.**
Live: <https://dotneteer.github.io/kliveide/preview/update-nextra/>

### R1 was real, but LOUDER than Decision 3 predicted

Built deliberately without `--webpack` first, to see what a careless upgrade
would actually do. Next 16 defaults to Turbopack and the build **fails
outright**:

```
Error: loader nextra/loader.cjs for match "./{src/,}app/**/page.{md,mdx}"
does not have serializable options.
```

Decision 3's *mechanism* was right — Nextra sets a top-level `turbopack` key on
Next 16, so Next's own "webpack config found" guard never fires. But the
conclusion drawn from it was wrong: Turbopack rejects Nextra's non-serializable
loader options at load time rather than silently ignoring them. **There is no
silent-drop path here.** `--webpack` fixes it, and R1 should be downgraded from
"silent, catastrophic" to "loud, obvious".

The highlighting check keeps its value regardless — it caught a genuinely silent
regression in Phase 4 (Shiki v3), and it still guards against someone
"resolving" this Turbopack error by deleting `getHighlighter`.

### Changes

- `next` 16.3.4; `engines.node >= 20.9` (Next 16's floor; Nextra's own is `>=18`)
- `dev` / `build` carry `--webpack`, which **only exists as a flag from Next 16** —
  see the Phase 4 correction
- `outputFileTracingRoot` pinned to the docs dir: the isolated docs package nested
  inside the Electron repo means two lockfiles, and Next inferred the wrong
  workspace root
- `next-env.d.ts` regenerated by Next 16

### Route golden re-baselined 90 → 91

Next 16 additionally exports the App Router not-found route as
`/_not-found/index.html`. 404 handling is unchanged — an unknown path still
returns 404 with the 404 page; `/_not-found/` is just an extra reachable page.
It starts with an underscore, so like `_next/` and `_pagefind/` it depends on the
root `.nojekyll`.

### The async-request-API codemod is a false positive here

`@next/codemod next-async-request-api` flags
`<MDXContent {...props} params={params} />` with a `@next-codemod-error`
comment. It cannot statically see that `params={params}` — the already-awaited
value — comes *after* the spread and overrides the promise. The code (verbatim
from Nextra's official example) is correct; the annotation was reverted.

### Verified

- 91 routes, 102 assets, all six Z80 colours unchanged
  (2154 / 1372 / 1579 / 1968 / 1157 / 385).
- Pagefind still built and served; `nextreg` returns 31 hits live with URL
  `/book/02-io-and-nextregs/`.
- No workspace-root warning; root `build:check` green; root `node_modules`
...
|---|---|---|---|
| R1 | ~~Turbopack silently drops the custom grammar~~ — **measured in Phase 6: it fails the build loudly instead** (`loader … does not have serializable options`). Downgraded. | ~~High~~ Low | `--webpack` on `dev` and `build` (Next 16 only — the flag does not exist in 15). `doc:check:highlighting` retained: it caught a *genuinely* silent Shiki v3 regression in Phase 4 and guards against removing `getHighlighter` to appease Turbopack |
