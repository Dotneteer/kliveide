...
|---|---|---|---|---|---|
| NR-001 | Select/read round trip | S | 1 | | Write `$7F` via `$243B`/`$253B` with several values; `$243B` then `$253B` read returns each. Proves the register-select latch. |
| NR-002 | `$243B` select persists | S | 1 | | Select `$7F`, write, perform unrelated `out`s to other ports, read `$253B` again: still `$7F`'s value. |
| NR-003 | `$243B` read back | S | 2 | | After `out $243B,$15`, `in $243B` returns the selected register number (verify read behaviour in `zxnext.vhd` before asserting). |
| NR-004 | Z80N `NEXTREG n,v` equals port path | S | 1 | | Same writes via `ED 91` and via ports produce identical `nextRegValue`; the `$243B` selection after `NEXTREG` matches the VHDL. |
| NR-005 | `NEXTREG n,A` | S | 1 | | `ED 92` writes register `n` from A for several n/A pairs. |
| NR-006 | Machine ID `$00` | S | 1 | | Reads the emulator's configured machine ID (`g_machine_id`); constant across resets. |
| NR-007 | Core version `$01`/`$0E` | S | 2 | | Major/minor in `$01`, sub-version in `$0E`, consistent with the version string the app reports. |
| NR-008 | Board issue `$0F` | S | 3 | | Upper nibble reads 0. |
| NR-009 | Unused register reads | S | 2 | | Reads of unassigned registers (e.g. `$0C`, `$0D`, `$1D`, `$21`, `$24`, `$25`, `$3A`–`$3F`, `$45`–`$49`, `$58`–`$5F`, `$65`–`$67`, `$72`–`$7E`, `$FE`) return the VHDL default (`others =>` branch). |
| NR-010 | Write-only bits read as 0 | S | 2 | | For each register whose read mux pads bits with `'0'` (e.g. `$0A` bit 5/1, `$22` bits 6–3, `$6E` bit 6), write `$FF` and assert padded bits are 0. |
| NR-011 | User register `$7F` | S | 1 | | Full 8-bit read/write; soft reset behaviour per VHDL. |
| NR-012 | Hard-reset values table | S | 1 | | One parametrised test: after `hardReset()`, every register in the read mux equals its VHDL reset value (`$14=$E3`, `$4A=$E3`, `$4B=$E3`, `$4C=$0F`, `$50–$57`, `$15`, `$43`, `$68`, `$6B`, `$70`, …). Guards bug B3. |
| NR-013 | Soft-reset values table | S | 1 | | Same as NR-012 after `reset()`: registers reset on soft reset change, registers reset only on hard reset keep a value written before. |
| NR-014 | `$1C` clip index readback | S | 2 | | Write two values to `$18`, `$19`, `$1A`, `$1B`; `$1C` shows each 2-bit clip index at its bit position; `$1C` write bits 0–3 reset the matching index. |
| NR-015 | `$34` sprite mirror index readback | S | 2 | | `$34` read returns the selected sprite number with bit 7 = 0. |
| NR-016 | `$28` stored palette value | S | 2 | | After a `$44` first-byte write, `$28` returns that byte (`nr_stored_palette_value`). |
| NR-017 | `$69` display control 1 readback | S | 2 | | Bit 7 mirrors `$123B` Layer 2 enable, bit 6 the `$7FFD` shadow flag, bits 5–0 port `$FF`; writing `$69` changes all three sources. |
| NR-018 | `$8E` composite readback | S | 1 | | After various `$7FFD`/`$DFFD`/`$1FFD` writes, `$8E` returns the bit formula in the read mux. |

### 4.2 `RST` – Reset types, machine type, config mode

| ID | Name | FE | Pri | Needs | Description |
|---|---|---|---|---|---|
| RST-001 | `$02` soft reset | S | 1 | | Program writes `$02` bit 0; machine resets, PC = 0, RAM kept, soft-reset registers reset. |
| RST-002 | `$02` hard reset | S | 1 | | Bit 1 performs hard reset: all hard-reset registers restored; `$02` reset type bits reflect the last reset. |
| RST-003 | Reset type readback | S | 2 | | `$02` bits 1–0 after power-on, after soft reset, after hard reset match the VHDL encoding. |
| RST-004 | `$02` generate DivMMC NMI | S | 2 | | Bit 2 raises an NMI routed to DivMMC (PC lands at `$0066` with DivMMC paged if automap/NMI enabled). |
| RST-005 | `$02` generate Multiface NMI | S | 2 | | Bit 3 raises the Multiface NMI; bit clears when the NMI is taken. |
| RST-006 | `$02` I/O trap flag | S | 3 | | Bit 4 reflects `nr_02_iotrap` after an FDC trap (see FDC-003). |
| RST-007 | `$03` machine type in config mode | S | 1 | | With `$03` low bits = `111` (config mode) writes to machine type are accepted; outside config mode they are ignored as the VHDL describes. |
| RST-008 | `$03` timing selection | S | 1 | | Bits 6–4 select 48K / 128K / +3 / Pentagon timing; `$03` readback and the frame length (tacts per frame, lines per frame) follow (see VT-002). |
| RST-009 | `$03` user DT lock | S | 3 | | Bit 3 locks the display timing; later timing writes are ignored. |
| RST-010 | `$03` bit 7 palette sub-index readback | S | 2 | | Bit 7 reads `nr_palette_sub_idx` (1 between the two `$44` bytes). |
| RST-011 | `$0A` MF type only in config mode | S | 3 | | Bits 7–6 change only when config mode is set (read mux of `$0A`). |
| RST-012 | Config mode leaves via `$03` | S | 2 | | Writing a non-`111`, non-zero machine type clears config mode; zero leaves it unchanged. Guards bug B2 dependency. |
| RST-013 | Soft reset keeps RAM, clears paging | S | 1 | | After `reset()`, bank contents written before reset survive; MMU, `$7FFD`, `$1FFD`, `$DFFD` are at reset values. |
| RST-014 | Soft reset keeps copper RAM | S | 1 | | Upload a list, soft reset, set `$62` run: list executes without re-upload. Guards bug B5. |
...
| ID | Name | FE | Pri | Needs | Description |
|---|---|---|---|---|---|
| MEM-001 | MMU reset layout | S | 1 | | `$50`–`$57` after hard reset = `$FF,$FF,$0A,$0B,$04,$05,$00,$01`. |
...
| MEM-007 | `$7FFD` 128K paging | S | 1 | | Bits 2–0 select the bank at `$C000` (MMU6/7 updated); bit 4 selects ROM; `$50/$51` readback follow. |
| MEM-008 | `$DFFD` extended bank bits | S | 1 | | `$DFFD` bits extend the `$7FFD` bank number beyond 7. |
| MEM-009 | `$7FFD` lock (bit 5 / `$08` bit 7) | S | 2 | | After `$7FFD` bit 5, further `$7FFD` writes are ignored; `$08` bit 7 read = not locked; writing `$08` bit 7 unlocks. |
...
| MEM-025 | Bank 5/7 shadow screen | P | 1 | | Write distinct patterns into bank 5 and bank 7 display files; `$7FFD` bit 3 (and `$69` bit 6) switches which one is displayed. |
...
| ID | Name | FE | Pri | Needs | Description |
|---|---|---|---|---|---|
| VT-001 | Tacts per frame 50 Hz | S | 1 | | For 48K, 128K, +3, Pentagon timing: `tacts` difference over one `runFrames(1)` equals lines × tacts/line from `zxula_timing.vhd` (at 3.5 MHz). |
...
| VT-004 | `$1E/$1F` counter origin | S | 1 | | Line counter 0 aligns with the first paper line minus the VHDL offset (C00 established paper row = cvc − `$64` for copper). `$1F` read at the frame interrupt returns the documented line. |
...
| ID | Name | FE | Pri | Needs | Description |
|---|---|---|---|---|---|
| ULA-001 | Border colours 0–7 | P | 1 | | `out $FE,n` for n=0..7 with palette 16+n written; border pixels equal each colour. |
...
| ULA-008 | Attribute change mid-frame | V | 1 | | From a line interrupt rewrite attribute memory; top/bottom halves of a cell row differ. Guards bug B8 (WASM screen-memory racing). |
| ULA-009 | Bitmap change mid-frame | V | 2 | | Same as ULA-008 for pixel memory. |
| ULA-010 | ULA scroll X `$26` | V | 1 | | Static scroll 0/1/8/255: picture shifts left with wrap at 256; border unaffected. |
...
| ULA-015 | ULA disable `$68` bit 7 | V | 1 | | ULA and border transparent; fallback `$4A` visible. (C11 covers the per-line copper variant.) |
...
| ULA-018 | ULA transparency via `$14` | V | 1 | | ULA ink colour equal to `$14` is transparent (RGB compare), Layer 2 underneath shows. |
| ULA-019 | Stencil mode `$68` bit 0 | V | 2 | | With tilemap enabled, output = ULA AND tilemap (P02 covers basics; add all four transparency combinations). |
...
| ID | Name | FE | Pri | Needs | Description |
|---|---|---|---|---|---|
| ULN-001 | Enable `$43` bit 0 | S | 1 | | Readback; `$42` default `$07`. |
| ULN-002 | Ink mask `$07` | V | 1 | | Attribute bits 2–0 ink index 0–7, bits 7–3 paper index 128+n; border 128+border. Guards bug B7. |
| ULN-003 | Ink masks `$01,$03,$0F,$1F,$3F,$7F` | V | 1 | | Parametrised: ink = attr & mask, paper = 128 + (attr >> bits). One case with a row per mask. |
| ULN-004 | Full-ink mode `$FF` | V | 1 | | Mask `$FF`: ink = attribute, paper and border use the fallback `$4A`. |
...
| ID | Name | FE | Pri | Needs | Description |
|---|---|---|---|---|---|
| ULP-001 | Register/data ports | S | 1 | | `$BF3B` selects group/index, `$FF3B` writes/reads palette data; read back matches. |
| ULP-002 | Mode group enable | S | 1 | | Writing group 1 (`$40`) with data bit 0 = 1 enables ULA+; `$68` bit 3 reads the enable. |
| ULP-003 | Palette write updates Next palette | S | 2 | | ULA+ palette write lands in ULA palette index 192+n (verify via `$40`/`$41` read). |
| ULP-004 | ULA+ attribute decode | V | 1 | | Attribute bits 7–6 select 4 CLUTs; ink = 192 + clut×16 + ink, paper = 192 + clut×16 + 8 + paper. Border = paper of CLUT 0 (200+n). Guards bug B7. |
...
| L2-015 | Transparency by RGB `$14` | V | 1 | | L01 covers; add the 9-bit compare detail: two indices mapped to colours differing only in blue LSB are both transparent (8-bit compare). |
| L2-016 | Priority bit from palette | V | 1 | | Layer 2 palette entry with `$44` second byte bit 7 set draws above sprites/ULA in any `$15` order (P01 covers one band; add every order). |
...
| TM-009 | ULA-over-tilemap per tile | V | 1 | | Attribute bit 0 with `$6B` bit 0 = 0: tile below ULA (P02 covers basics). |
...
| TM-018 | Base address `$6E` | V | 1 | | Map relocated to other address `$4000–$FFFF` (bits 6–0 × 256 + `$C000`? read VHDL); bit 7 selects bank 7 vs bank 5 mapping. Guards bug B10 area. |
| TM-019 | Tile definitions `$6F` | V | 1 | | Definitions relocated; bit 7 same as `$6E`. |
...
| ID | Name | FE | Pri | Needs | Description |
|---|---|---|---|---|---|
| SPR-001 | Pattern upload via `$5B` | P | 1 | | `$303B` selects pattern n, 256 bytes written through `$5B`; a sprite showing pattern n displays it pixel-exact. |
...
| SPR-006 | Attribute writes via `$35`–`$39` | P | 1 | | `$34` selects sprite; `$35`–`$39` write attributes 0–4 without advancing; `$75`–`$79` write and advance. |
...
| SPR-009 | Visibility bit | P | 1 | | Attr 3 bit 7 = 0 hides the sprite. |
| SPR-010 | Sprites enable `$15` bit 0 | P | 1 | | Disabled: no sprite visible, collision flags not set. |
...
| SPR-025 | Collision flag | S | 1 | | Exists (`sprite-collision.test.ts`). Extend: transparent pixels do not collide, clipped pixels do not. |
| SPR-026 | Max sprites per line flag | S | 1 | | >100 sprites on one line set `$303B` bit 1; further sprites not drawn (per VHDL limit). |
| SPR-027 | Status read clears | S | 1 | | Both bits clear after reading `$303B`. |
...
| PAL-002 | `$41` 8-bit colour | S | 1 | | `$41` write of RRRGGGBB sets blue bit 0 = `B1 or B0` (C02 covered visually); `$41` read returns bits 8–1. |
...
| PAL-011 | Fallback `$4A` | V | 1 | | Visible where all layers transparent; reset value `$E3` (bug B3). |
...
| PAL-014 | Palette change mid-frame | V | 2 | | Line-interrupt palette change: colour bands (copper C02 covers the copper path). |
...
| ID | Name | FE | Pri | Needs | Description |
|---|---|---|---|---|---|
| CMP-001 | Layer orders `$15` 000–101 | V | 1 | | Six bands with overlapping opaque S, L (Layer 2), U pixels; each band shows the top layer per SLU/LSU/SUL/LUS/USL/ULS. P01 covers part – complete it. |
...
| CMP-004 | Blend source `$68` bits 6–5 | V | 1 | | 00 ULA, 01 none, 10 ULA/tilemap mix, 11 tilemap as blend operand; tilemap placement per VHDL `ula_blend_mode_2`. Guards bug B6.1 (TS). |
| CMP-005 | Blend with ULA disabled | V | 1 | | `$68` bit 7 during a blend band: VHDL blends `ula_mix_rgb` ignoring `ula_en`. Guards bug B6.2 (TS). |
| CMP-006 | Sprite over ULA border in LUS/USL/ULS | V | 1 | | Sprite over opaque border with transparent tilemap shows. Guards bug B6.3 (TS). |
| CMP-007 | Layer 2 priority bit in all orders | V | 1 | | See L2-016. |
| CMP-008 | Tilemap/ULA merge | V | 1 | | Tilemap over/under ULA per attribute and `$6B` bit 0 in every `$15` order. |
| CMP-009 | Stencil with sprites and Layer 2 | V | 2 | | Stencil output participates as the "U" layer. |
| CMP-010 | Transparent everywhere → fallback | V | 1 | | All layers enabled but transparent: `$4A`. |
...
| CMP-014 | Order change mid-frame | V | 2 | | `$15` change from a line interrupt (C10 does `$14` via copper). |

### 4.18 `COP` – Copper (gaps beyond C00–D05)

| ID | Name | FE | Pri | Needs | Description |
|---|---|---|---|---|---|
| COP-001 | `$60` then `$63` mixed upload | S | 1 | | `$61=0; $60=hi; $63=lo`: copper RAM holds hi/lo. Guards bug B4. |
...
| COP-004 | List survives soft reset | S | 1 | | Same as RST-014. Guards bug B5. |
...
| INT-007 | Line interrupt `$22/$23` | S | 1 | | Enable bit 1, 9-bit line; interrupt fires at that line (handler reads `$1E/$1F`). |
...
| ID | Name | FE | Pri | Needs | Description |
|---|---|---|---|---|---|
| NMI-001 | Multiface NMI via `$02` | S | 1 | | See RST-005; PC = `$0066` with MF memory paged. |
| NMI-002 | DivMMC NMI via `$02` | S | 1 | | See RST-004; DivMMC automap at `$0066`. |
...
| AY-012 | Stereo mode ABC vs ACB `$08` bit 5 | A | 1 | | Channel A left, C/B right as per mode; B centered (ABC) etc. |
...
| ID | Name | FE | Pri | Needs | Description |
|---|---|---|---|---|---|
| DAC-001 | DAC enable `$08` bit 3 | A | 1 | | With disable, port writes produce no output. |
...
| MF-002 | Page in via NMI | S | 1 | | NMI (via `$02`) pages MF ROM at `$0000`, RAM at `$2000`. |
...
| ID | Name | FE | Pri | Needs | Description |
|---|---|---|---|---|---|
| FDC-001 | `$D8` enable | S | 3 | | Readback. |
| FDC-002 | Ports `$2FFD/$3FFD` decoded only when enabled | S | 3 | | |
| FDC-003 | Trap generates NMI | S | 3 | | Access to `$2FFD/$3FFD` with trap enabled sets `$DA` cause, `$D9` write value, `$02` bit 4, and NMI. |
...
| PAR-004 | Long-run timing parity | S | 2 | | 3000 frames: `tacts`, `frames`, registers equal. |
