...
/**
 * This script disassembles the second 16KB block (ROM 1) of the enNextZX.rom file
 * and saves the output to nextRom1.txt in the next-disassembly folder.
 * The disassembly starts at address 0x0000 (not 0x4000).
 */
async function disassembleNextRom1() {
  try {
    ...
    let output = "";
    output += "ZX Spectrum Next ROM Disassembly (ROM 1 - Second 16KB)\n";
    output += "=" .repeat(80) + "\n";
    ...
    // --- Save to file
    const outputPath = path.join(__dirname, "nextRom1.txt");
