...
/**
 * This script disassembles the third 16KB block (ROM 2) of the enNextZX.rom file
 * and saves the output to nextRom2.txt in the next-disassembly folder.
 * The disassembly starts at address 0x0000 (not 0x8000).
 */
async function disassembleNextRom2() {
  try {
    ...
    // --- Extract third 16KB (bytes 0x8000-0xBFFF from file, disassembled as 0x0000-0x3FFF)
    const rom16k = new Uint8Array(romData.buffer, romData.byteOffset + 0x8000, Math.min(16384, romData.length - 0x8000));
    
    console.log(`ROM size: ${romData.length} bytes`);
    console.log(`Extracting ROM 2 (bytes 0x8000-0xBFFF from file)...`);
    ...
    let output = "";
    output += "ZX Spectrum Next ROM Disassembly (ROM 2 - Third 16KB)\n";
    output += "=" .repeat(80) + "\n";
    output += `File: enNextZX.rom\n`;
    output += `Source ROM Block: ROM 2 (file bytes 0x8000-0xBFFF)\n`;
    ...
    // --- Save to file
    const outputPath = path.join(__dirname, "nextRom2.txt");
