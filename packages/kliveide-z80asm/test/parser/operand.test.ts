...
describe("Parser - operands", () => {
  ...
  const hregOps = ["bc", "BC", "de", "DE", "hl", "HL", "ix", "IX", "iy", "IY"];
  hregOps.forEach((opr) => {
    it(`regop ${opr} #1`, () => {
      const parser = createParser(`inc hreg(${opr})`);
      const parsed = parser.parseProgram();
      expect(parser.hasErrors).toBe(false);
      expect(parsed).not.toBeNull();
      expect(parsed.assemblyLines.length).toBe(1);
      ...
      expect(operand.register).toBe(opr.toLowerCase());
      expect(operand.regOperation).toBe("hreg");

      const line = parsed.assemblyLines[0] as Z80AssemblyLine;
      expect(line.label).toBe(null);
      expect(line.startPosition).toBe(0);
      expect(line.endPosition).toBe(opr.length + 10);
      expect(line.line).toBe(1);
      expect(line.startColumn).toBe(0);
      expect(line.endColumn).toBe(opr.length + 10);
    });
    ...
    it(`regop ${opr} #3`, () => {
      const parser = createParser(`inc lreg(${opr})`);
      const parsed = parser.parseProgram();
      expect(parser.hasErrors).toBe(false);
      expect(parsed).not.toBeNull();
      expect(parsed.assemblyLines.length).toBe(1);
      ...
      expect(operand.operandType).toBe(OperandType.RegOperation);
      expect(operand.register).toBe(opr.toLowerCase());
      expect(operand.regOperation).toBe("lreg");
  ...
  const hregMacroOps = ["lreg", "LREG", "hreg", "HREG"];
  hregMacroOps.forEach((opr) => {
    it(`regop-macroparam ${opr} #1`, () => {
      const parser = createParser(`inc ${opr}({{abc}})`);
      const parsed = parser.parseProgram();
      expect(parser.hasErrors).toBe(false);
      expect(parsed).not.toBeNull();
      expect(parsed.assemblyLines.length).toBe(1);
      ...
      expect(operand.macroParam).toBe("abc");
      expect(operand.regOperation).toBe(opr.toLowerCase());

      const line = parsed.assemblyLines[0] as Z80AssemblyLine;
      expect(line.label).toBe(null);
      expect(line.startPosition).toBe(0);
      expect(line.endPosition).toBe(opr.length + 13);
      expect(line.line).toBe(1);
      expect(line.startColumn).toBe(0);
      expect(line.endColumn).toBe(opr.length + 13);
    });
  });
