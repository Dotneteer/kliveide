...
export class SplitContainer extends React.Component<Props> {
  ...
  setupSplitter(isInitial: boolean) {
    removeGutters(this._hostElement.current);
    ...
    Split((children as unknown) as HTMLElement[], {
      sizes,
      gutterSize: comp._gutterSize,
      minSize: comp._minimumSize,
...
function Split(
  ...
  const gutterStyle = getOption(
    options,
    "gutterStyle",
    (dim: Dimension, gutterSize: number) => ({
      [dim]: `${gutterSize}px`,
