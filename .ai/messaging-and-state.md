...
- Generated SAVE output uses `MainApi.saveGeneratedTapeFile(defaultName, contents)`. The main process opens an Electron save dialog, defaults to the last tape folder stored under `appSettings.folders.tapeFileFolder`, parses the generated TZX bytes, writes them, dispatches `SET_TAPE_MEDIA` with the saved file path/metadata, then saves settings so the generated tape is restorable on the next startup. Renderer code must call this API rather than importing Node or Electron file APIs.
- `EmulatorPanelReact` handles `Sp48FrameCompletedEvent.savedTapeFileInfo` through `createGeneratedTapeSaveQueue(...)` in `src/renderer/lib/EmulatorPanel/generatedTapeSave.ts`. The queue serializes save dialogs so multiple generated SAVE events cannot overlap. On a successful write, it calls `attachGeneratedTapeFile(...)`, which parses the generated bytes and uploads them through the active SP48 session bridge so the saved file is immediately selected for later LOAD. Success/cancel are logged; failure is surfaced through the panel's existing error display.
