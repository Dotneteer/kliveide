...
- Keep Explorer keyboard rules in a small helper and let `ExplorerPanel` only bridge DOM events to selection, scrolling, expansion refresh, and dialogs.
- Keep Explorer row focus separate from selected/opened state. Arrow keys move focus; selection should change only from click or leaf activation.
- Store Explorer selection by node identity, not visible index. Visible indexes are transient and can point to the wrong row after collapse, expand, filtering, or refresh.
