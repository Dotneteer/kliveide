 ...
 */
type M6510Operation = (cpu: M6510VaCpu) => void;

/**
 * This class implements the emulation of the MOS 6510 VIC-aware CPU
 */
export class M6510VaCpu implements IM6510VaCpu {
  ...
  get interruptedInstructionStartTact(): number {
    return this._interruptedInstructionStartTact;
  }
  ...
  /**
   * Waits while the CPU gets released and has access to the address bus.
   */
  waitForCpuReleased(): void {
    while (this._stalled) {
      // --- The CPU is stalled, wait for it to be released
      this.vicPhi1();
      this.vicPhi2();
      this.incrementTact();
    }
  ...
  /**
   * Waits while the CPU can execute its phi1 phase job.
   */
  waitForPhi1(): void {
    this.vicPhi1();
    if (this._stalled) {
      this.vicPhi2();
      this.incrementTact();
      this.waitForCpuReleased();
    }
  }

  /**
   * Waits while the CPU can execute its phi2 phase job.
   */
  waitForPhi2(): void {
    this.vicPhi2();
    this.waitForCpuReleased();
  ...
  /**
   * Handles the internal clock cycle for the CPU.
   */
  internalTact(): void {
    // --- Internal CPU clock cycle handling logic
    this.vicPhi1();
    this.vicPhi2();
    this.incrementTact();
  }
  ...
  executeCpuCycle(): void {
    ...
    if (this._nmiRequested) {
      this._interruptedInstructionStartTact = this.tacts;
      this.handleNmi();
    ...
    // --- Process IRQ
    if (this._irqRequested && !this.isIFlagSet()) {
      this._interruptedInstructionStartTact = this.tacts;
      this.handleIrq();
   ...
   * NMI has higher priority than IRQ and cannot be disabled by the I flag.
   * The processor pushes the current PC and status register to the stack,
   * sets the I flag, and jumps to the NMI vector at $FFFA/$FFFB.
   */
  private handleNmi() {
    // NMI interrupt sequence takes 7 cycles total
    this.incrementTact(); // Cycle 1: Internal operation

    // Push current PC to stack (high byte first, then low byte)
    this.pushStack((this._pc >> 8) & 0xff); // Cycle 2: Push PCH
    this.pushStack(this._pc & 0xff); // Cycle 3: Push PCL

    // Push status register to stack (without B flag, but with UNUSED flag)
    const statusWithoutBreak = (this._p & ~FlagSetMask6510.B) | FlagSetMask6510.UNUSED;
    this.pushStack(statusWithoutBreak); // Cycle 4: Push status

    // Set interrupt disable flag to prevent further IRQs during NMI handling
    this._p |= FlagSetMask6510.I;

    // Jump to NMI vector at $FFFA/$FFFB
    const vectorLow = this.readMemory(0xfffa); // Cycle 5: Read vector low
    const vectorHigh = this.readMemory(0xfffb); // Cycle 6: Read vector high
    this._pc = (vectorHigh << 8) | vectorLow;

    this.incrementTact(); // Cycle 7: Internal operation
  }

  /**
   * Handle maskable Interrupt Request (IRQ)
   * IRQ can be disabled by the I flag. The processor pushes the current PC
   * and status register to the stack, sets the I flag, and jumps to the
   * IRQ vector at $FFFE/$FFFF.
   */
  private handleIrq() {
    // IRQ interrupt sequence takes 7 cycles total
    this.internalTact(); // Cycle 1: Internal operation
    ...
    // Set interrupt disable flag to prevent further IRQs during interrupt handling
    this._p |= FlagSetMask6510.I;

    // Jump to IRQ vector at $FFFE/$FFFF
    const vectorLow = this.readMemory(0xfffe); // Cycle 5: Read vector low
    const vectorHigh = this.readMemory(0xffff); // Cycle 6: Read vector high
    this._pc = (vectorHigh << 8) | vectorLow;

    this.internalTact(); // Cycle 7: Internal operation
  }
  ...
  /**
   * This function implements the memory read delay of the CPU. In 6510, there is no 
   * direct memory read delay.
   */
  ...
  /**
   * This function implements the memory write delay of the CPU. In 6510, there is no
   * direct memory write delay.
   */
  delayMemoryWrite(_address: number): void {
    // --- No memory write delay
  }
   ...
   * NMI will be handled at the end of the current instruction, regardless of the I flag.
   */
  triggerNmi(): void {
    this._nmiSignal = true;
  }

  /**
   * Clears the NMI request flag.
   */
  clearNmi(): void {
    this._nmiSignal = false;
  }

  /**
   * Triggers an Interrupt Request (IRQ).
   * IRQ will be handled at the end of the current instruction if the I flag is clear.
   */
  triggerIrq(): void {
    this._irqSignal = true;
  }
   ...
   */
  clearIrq(): void {
    this._irqSignal = false;
  }

  /**
   * Clears any pending interrupt requests.
   * This can be useful for acknowledging interrupts or resetting interrupt state.
   */
  ...
  /**
   * Increment the current CPU tact counter
   */
  incrementTact(): void {
    this.tacts++;
 ...
 */
function brk(cpu: M6510VaCpu): void {
 ...
 */
function oraIndX(cpu: M6510VaCpu): void {
 ...
 */
function jam(cpu: M6510VaCpu): void {
  cpu.setJammed();
  cpu.incrementTact(); // Increment tacts to simulate a cycle
}
 ...
 */
function sloIndX(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact(); // Indexed addressing cycle
  ...
  cpu.incrementTact(); // Read-modify-write: extra cycle for internal operations
 ...
 */
function dopZp(cpu: M6510VaCpu): void {
  cpu.readMemory(cpu.pc++); // Read and discard the zero page address
  cpu.incrementTact(); // Extra cycle for zero page access (but no actual memory access)
}
 ...
 * Addressing mode: Zero Page - 2 bytes
 */
function oraZp(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
 ...
 * Addressing mode: Zero Page - 2 bytes
 */
function aslZp(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  const originalValue = cpu.readMemory(zpAddress);
  cpu.incrementTact(); // Internal operation cycle
  const shiftedValue = (originalValue << 1) & 0xff;
 ...
 * Addressing mode: Zero Page - 2 bytes
 */
function sloZp(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  const originalValue = cpu.readMemory(zpAddress);
  cpu.incrementTact(); // Read-modify-write: extra cycle for internal operations
  const shiftedValue = (originalValue << 1) & 0xff;
 ...
 * Addressing mode: Implied - 1 byte
 */
function php(cpu: M6510VaCpu): void {
  cpu.incrementTact(); // Internal operation cycle
 ...
 */
function oraImm(cpu: M6510VaCpu): void {
 ...
 */
function aslA(cpu: M6510VaCpu): void {
  cpu.incrementTact(); // Internal operation cycle
 ...
 */
function aacImm(cpu: M6510VaCpu): void {
  const value = cpu.readMemory(cpu.pc++);
  cpu.a = cpu.a & value;
  cpu.setZeroAndNegativeFlags(cpu.a);
 ...
 */
function topAbs(cpu: M6510VaCpu): void {
 ...
 * Addressing mode: Absolute - 3 bytes
 */
function oraAbs(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  const value = cpu.readMemory(address);
 ...
 * Addressing mode: Absolute - 3 bytes
 */
function aslAbs(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
 ...
 * Alternative names: ASO
 */
function sloAbs(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  const originalValue = cpu.readMemory(address);
 ...
 */
function bpl(cpu: M6510VaCpu): void {
 ...
 */
function oraIndY(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
 ...
 * Alternative names: ASO
 */
function sloIndY(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  const low = cpu.readMemory(zpAddress);
  const high = cpu.readMemory((zpAddress + 1) & 0xff);
  const baseAddress = (high << 8) | low;
  const targetAddress = (baseAddress + cpu.y) & 0xffff;
  cpu.incrementTact(); // Page boundary check for read
  const originalValue = cpu.readMemory(targetAddress);
  cpu.incrementTact(); // Read-modify-write: extra cycle for internal operations
  const shiftedValue = (originalValue << 1) & 0xff;
  cpu.writeMemory(targetAddress, shiftedValue);
 ...
 * Alternative names: NOP, SKB
 */
function dopZpX(cpu: M6510VaCpu): void {
  cpu.readMemory(cpu.pc++); // Read and discard the zero page address
  cpu.incrementTact(); // Extra cycle for indexed addressing
  cpu.incrementTact(); // Extra cycle for zero page access (but no actual memory access)
}
 ...
 * Addressing mode: Zero Page,X - 2 bytes
 */
function oraZpX(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact();
 ...
 * Addressing mode: Zero Page,X - 2 bytes
 */
function aslZpX(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact();
  const address = (zpAddress + cpu.x) & 0xff;
  const originalValue = cpu.readMemory(address);
 ...
 * Alternative names: ASO
 */
function sloZpX(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact(); // Indexed addressing cycle
 ...
 * Addressing mode: Implied - 1 byte
 */
function clc(cpu: M6510VaCpu): void {
  cpu.incrementTact(); // Internal operation cycle
  cpu.p &= ~FlagSetMask6510.C;
}
 ...
 */
function oraAbsY(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  const value = cpu.readMemoryWithPageBoundary(address, cpu.y);
 ...
 */
function illegal(_cpu: M6510VaCpu): void {
 ...
 * Addressing mode: Absolute,Y - 3 bytes
 */
function sloAbsY(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const baseAddress = (high << 8) | low;
  const targetAddress = (baseAddress + cpu.y) & 0xffff;
 ...
 * Alternative names: NOP
 */
function topAbsX(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
 ...
 * Addressing mode: Absolute,X - 3 bytes
 */
function oraAbsX(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
 ...
 * Addressing mode: Absolute,X - 3 bytes
 */
function aslAbsX(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  cpu.incrementTact(); // Extra cycle for indexed addressing
 ...
 * Alternative names: ASO
 */
function sloAbsX(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const baseAddress = (high << 8) | low;
 ...
 * Addressing mode: Absolute - 3 bytes
 */
function jsr(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
 ...
 * Addressing mode: (Indirect,X) - 2 bytes
 */
function andIndX(cpu: M6510VaCpu): void {
 ...
 * Addressing mode: (Indirect,X) - 2 bytes
 */
function rlaIndX(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact(); // Indexed addressing cycle
  const indirectAddress = (zpAddress + cpu.x) & 0xff; // Wrap around in zero page
  const targetLow = cpu.readMemory(indirectAddress);
 ...
 * Addressing mode: Zero Page - 2 bytes
 */
function bitZp(cpu: M6510VaCpu): void {
 ...
 * Addressing mode: Zero Page - 2 bytes
 */
function andZp(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  const value = cpu.readMemory(zpAddress);
  cpu.a = cpu.a & value;
  cpu.setZeroAndNegativeFlags(cpu.a);
 ...
 * Addressing mode: Zero Page - 2 bytes
 */
function rolZp(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  const originalValue = cpu.readMemory(zpAddress);
  cpu.incrementTact(); // Internal operation cycle
  const carryIn = cpu.isCFlagSet() ? 1 : 0;
 ...
 * Addressing mode: Zero Page - 2 bytes
 */
function rlaZp(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  const originalValue = cpu.readMemory(zpAddress);
  cpu.incrementTact(); // Read-modify-write: extra cycle for internal operations
  const carryIn = cpu.isCFlagSet() ? 1 : 0;
 ...
 * Addressing mode: Implied - 1 byte
 */
function plp(cpu: M6510VaCpu): void {
  cpu.incrementTact(); // Internal operation cycle 1
  cpu.incrementTact(); // Internal operation cycle 2
 ...
 * Addressing mode: Immediate - 2 bytes
 */
function andImm(cpu: M6510VaCpu): void {
  const value = cpu.readMemory(cpu.pc++);
  cpu.a = cpu.a & value;
  cpu.setZeroAndNegativeFlags(cpu.a);
}
 ...
 * Addressing mode: Accumulator - 1 byte
 */
function rolA(cpu: M6510VaCpu): void {
  cpu.incrementTact(); // Internal operation cycle
  const originalValue = cpu.a;
  const carryIn = cpu.isCFlagSet() ? 1 : 0;
 ...
 * Addressing mode: Absolute - 3 bytes
 */
function bitAbs(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  const value = cpu.readMemory(address);
 ...
 * Addressing mode: Absolute - 3 bytes
 */
function andAbs(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  const value = cpu.readMemory(address);
 ...
 * Addressing mode: Absolute - 3 bytes
 */
function rolAbs(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  const originalValue = cpu.readMemory(address);
 ...
 * Addressing mode: Absolute - 3 bytes
 */
function rlaAbs(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  const originalValue = cpu.readMemory(address);
 ...
 * Addressing mode: Relative - 2 bytes
 */
function bmi(cpu: M6510VaCpu): void {
  cpu.performBranch(cpu.isNFlagSet());
}
 ...
 * Addressing mode: (Indirect),Y - 2 bytes
 */
function andIndY(cpu: M6510VaCpu): void {
  const zeroPageAddress = cpu.readMemory(cpu.pc++);
 ...
 * Addressing mode: (Indirect),Y - 2 bytes
 */
function rlaIndY(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  const low = cpu.readMemory(zpAddress);
  const high = cpu.readMemory((zpAddress + 1) & 0xff);
  const baseAddress = (high << 8) | low;
 ...
 * Addressing mode: Zero Page,X - 2 bytes
 */
function andZpX(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact(); // Increment tacts for indexed addressing
 ...
 * Addressing mode: Zero Page,X - 2 bytes
 */
function rolZpX(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact();
  const address = (zpAddress + cpu.x) & 0xff;
  const originalValue = cpu.readMemory(address);
 ...
 * Addressing mode: Zero Page,X - 2 bytes
 */
function rlaZpX(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact(); // Indexed addressing cycle
  const address = (zpAddress + cpu.x) & 0xff; // Wrap around in zero page
  const originalValue = cpu.readMemory(address);
 ...
 * Addressing mode: Implied - 1 byte
 */
function sec(cpu: M6510VaCpu): void {
  cpu.incrementTact(); // Internal operation cycle
  cpu.p |= FlagSetMask6510.C;
}
 ...
 * Addressing mode: Absolute,Y - 3 bytes
 */
function andAbsY(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  const value = cpu.readMemoryWithPageBoundary(address, cpu.y);
 ...
 * Addressing mode: Absolute,Y - 3 bytes
 */
function rlaAbsY(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const baseAddress = (high << 8) | low;
  const targetAddress = (baseAddress + cpu.y) & 0xffff;
 ...
 * Addressing mode: Absolute,X - 3 bytes
 */
function andAbsX(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  const value = cpu.readMemoryWithPageBoundary(address, cpu.x);
 ...
 * Addressing mode: Absolute,X - 3 bytes
 */
function rolAbsX(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  cpu.incrementTact(); // Extra cycle for indexed addressing
 ...
 * Addressing mode: Absolute,X - 3 bytes
 */
function rlaAbsX(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const baseAddress = (high << 8) | low;
  const targetAddress = (baseAddress + cpu.x) & 0xffff;
 ...
 * Addressing mode: Implied - 1 byte
 */
function rti(cpu: M6510VaCpu): void {
  cpu.incrementTact(); // Internal operation cycle 1
 ...
 */
function eorIndX(cpu: M6510VaCpu): void {
  const zeroPageAddress = cpu.readMemory(cpu.pc++);
  const effectiveAddress = (zeroPageAddress + cpu.x) & 0xff; // Wrap around in zero page
  cpu.incrementTact();
  const low = cpu.readMemory(effectiveAddress);
 ...
 */
function sreIndX(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact(); // Indexed addressing cycle
  const indirectAddress = (zpAddress + cpu.x) & 0xff; // Wrap around in zero page
  const targetLow = cpu.readMemory(indirectAddress);
 ...
 * Addressing mode: Zero Page - 2 bytes
 */
function eorZp(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  const value = cpu.readMemory(zpAddress);
  cpu.a = cpu.a ^ value;
  cpu.setZeroAndNegativeFlags(cpu.a);
 ...
 * Addressing mode: Zero Page - 2 bytes
 */
function lsrZp(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  const originalValue = cpu.readMemory(zpAddress);
  cpu.incrementTact(); // Internal operation cycle
  const shiftedValue = originalValue >> 1;
 ...
 * Alternative names: LSE
 */
function sreZp(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  const originalValue = cpu.readMemory(zpAddress);
  cpu.incrementTact(); // Read-modify-write: extra cycle for internal operations
  const shiftedValue = originalValue >> 1;
 ...
 * Addressing mode: Implied - 1 byte
 */
function pha(cpu: M6510VaCpu): void {
  cpu.incrementTact(); // Internal operation cycle
  cpu.pushStack(cpu.a);
}
 ...
 * Addressing mode: Immediate - 2 bytes
 */
function eorImm(cpu: M6510VaCpu): void {
  const value = cpu.readMemory(cpu.pc++);
  cpu.a = cpu.a ^ value;
  cpu.setZeroAndNegativeFlags(cpu.a);
}
 ...
 * Addressing mode: Accumulator - 1 byte
 */
function lsrA(cpu: M6510VaCpu): void {
  cpu.incrementTact(); // Internal operation cycle
  const originalValue = cpu.a;
 ...
 */
function asrImm(cpu: M6510VaCpu): void {
  const value = cpu.readMemory(cpu.pc++);
 ...
 * Addressing mode: Absolute - 3 bytes
 */
function jmp(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const targetAddress = (high << 8) | low;
  cpu.pc = targetAddress;
 ...
 * Addressing mode: Absolute - 3 bytes
 */
function eorAbs(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  const value = cpu.readMemory(address);
 ...
 * Addressing mode: Absolute - 3 bytes
 */
function lsrAbs(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  const originalValue = cpu.readMemory(address);
 ...
 * Alternative names: LSE
 */
function sreAbs(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  const originalValue = cpu.readMemory(address);
 ...
 * Addressing mode: Relative - 2 bytes
 */
function bvc(cpu: M6510VaCpu): void {
 ...
 */
function eorIndY(cpu: M6510VaCpu): void {
  const zeroPageAddress = cpu.readMemory(cpu.pc++);
  const low = cpu.readMemory(zeroPageAddress);
  const high = cpu.readMemory((zeroPageAddress + 1) & 0xff); // Handle zero page wrap-around
  const baseAddress = (high << 8) | low;
 ...
 * Alternative names: LSE
 */
function sreIndY(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  const low = cpu.readMemory(zpAddress);
  const high = cpu.readMemory((zpAddress + 1) & 0xff);
  const baseAddress = (high << 8) | low;
 ...
 * Addressing mode: Zero Page,X - 2 bytes
 */
function eorZpX(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact(); // Increment tacts for indexed addressing
  const value = cpu.readMemory((zpAddress + cpu.x) & 0xff); // Wrap around in zero page
  cpu.a = cpu.a ^ value;
 ...
 * Addressing mode: Zero Page,X - 2 bytes
 */
function lsrZpX(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact();
  const address = (zpAddress + cpu.x) & 0xff;
  const originalValue = cpu.readMemory(address);
 ...
 * Alternative names: LSE
 */
function sreZpX(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact(); // Indexed addressing cycle
  const address = (zpAddress + cpu.x) & 0xff; // Wrap around in zero page
  const originalValue = cpu.readMemory(address);
 ...
 * Addressing mode: Implied - 1 byte
 */
function cli(cpu: M6510VaCpu): void {
  cpu.incrementTact(); // Internal operation cycle
  cpu.p &= ~FlagSetMask6510.I;
}
 ...
 * Addressing mode: Absolute,Y - 3 bytes
 */
function eorAbsY(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  const value = cpu.readMemoryWithPageBoundary(address, cpu.y);
 ...
 * Alternative names: LSE
 */
function sreAbsY(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const baseAddress = (high << 8) | low;
  const targetAddress = (baseAddress + cpu.y) & 0xffff;
 ...
 * Addressing mode: Absolute,X - 3 bytes
 */
function eorAbsX(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  const value = cpu.readMemoryWithPageBoundary(address, cpu.x);
 ...
 * Addressing mode: Absolute,X - 3 bytes
 */
function lsrAbsX(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  cpu.incrementTact(); // Extra cycle for indexed addressing
 ...
 * Alternative names: LSE
 */
function sreAbsX(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const baseAddress = (high << 8) | low;
  const targetAddress = (baseAddress + cpu.x) & 0xffff;
 ...
 * Addressing mode: Implied - 1 byte
 */
function rts(cpu: M6510VaCpu): void {
  cpu.incrementTact(); // Internal operation cycle 1
  cpu.incrementTact(); // Internal operation cycle 2
  ...
  cpu.incrementTact(); // Internal operation cycle 3
 ...
 * Addressing mode: (Indirect,X) - 2 bytes
 */
function adcIndX(cpu: M6510VaCpu): void {
  const zeroPageAddress = cpu.readMemory(cpu.pc++);
  const effectiveAddress = (zeroPageAddress + cpu.x) & 0xff; // Wrap around in zero page
  cpu.incrementTact(); // Indexed addressing cycle
  const low = cpu.readMemory(effectiveAddress);
 ...
 * Addressing mode: (Indirect,X) - 2 bytes
 */
function rraIndX(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact(); // Indexed addressing cycle
  const indirectAddress = (zpAddress + cpu.x) & 0xff; // Wrap around in zero page
  const targetLow = cpu.readMemory(indirectAddress);
 ...
 * Addressing mode: Zero Page - 2 bytes
 */
function adcZp(cpu: M6510VaCpu): void {
  const address = cpu.readMemory(cpu.pc++);
 ...
 * Addressing mode: Zero Page - 2 bytes
 */
function rorZp(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  const originalValue = cpu.readMemory(zpAddress);
  cpu.incrementTact(); // Internal operation cycle
  const carryIn = cpu.isCFlagSet() ? 0x80 : 0;
 ...
 * Addressing mode: Zero Page - 2 bytes
 */
function rraZp(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  const originalValue = cpu.readMemory(zpAddress);
  cpu.incrementTact(); // Read-modify-write: extra cycle for internal operations
 ...
 * Addressing mode: Implied - 1 byte
 */
function pla(cpu: M6510VaCpu): void {
  cpu.incrementTact(); // Internal operation cycle 1
  cpu.incrementTact(); // Internal operation cycle 2
  cpu.a = cpu.pullStack();
  cpu.setZeroAndNegativeFlags(cpu.a);
 ...
 * Addressing mode: Immediate - 2 bytes
 */
function adcImm(cpu: M6510VaCpu): void {
 ...
 * Addressing mode: Accumulator - 1 byte
 */
function rorA(cpu: M6510VaCpu): void {
  cpu.incrementTact(); // Internal operation cycle
  const originalValue = cpu.a;
  const carryIn = cpu.isCFlagSet() ? 0x80 : 0;
 ...
 * Addressing mode: Immediate - 2 bytes
 */
function arrImm(cpu: M6510VaCpu): void {
  const value = cpu.readMemory(cpu.pc++);
 ...
 */
function jmpInd(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
 ...
 * Addressing mode: Absolute - 3 bytes
 */
function adcAbs(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  const operand = cpu.readMemory(address);
 ...
 * Addressing mode: Absolute - 3 bytes
 */
function rorAbs(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  const originalValue = cpu.readMemory(address);
 ...
 * Addressing mode: Absolute - 3 bytes
 */
function rraAbs(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  const originalValue = cpu.readMemory(address);
 ...
 * Addressing mode: Relative - 2 bytes
 */
function bvs(cpu: M6510VaCpu): void {
  cpu.performBranch(cpu.isVFlagSet());
}
 ...
 * Addressing mode: (Indirect),Y - 2 bytes
 */
function adcIndY(cpu: M6510VaCpu): void {
  const zeroPageAddress = cpu.readMemory(cpu.pc++);
  const low = cpu.readMemory(zeroPageAddress);
  const high = cpu.readMemory((zeroPageAddress + 1) & 0xff); // Handle zero page wrap-around
  const baseAddress = (high << 8) | low;
 ...
 * Addressing mode: (Indirect),Y - 2 bytes
 */
function rraIndY(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  const low = cpu.readMemory(zpAddress);
  const high = cpu.readMemory((zpAddress + 1) & 0xff);
  const baseAddress = (high << 8) | low;
 ...
 * Addressing mode: Zero Page,X - 2 bytes
 */
function adcZpX(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact(); // Indexed addressing cycle
  const address = (zpAddress + cpu.x) & 0xff; // Wrap around in zero page
  const operand = cpu.readMemory(address);
 ...
 * Addressing mode: Zero Page,X - 2 bytes
 */
function rorZpX(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact();
  const address = (zpAddress + cpu.x) & 0xff;
  const originalValue = cpu.readMemory(address);
 ...
 * Addressing mode: Zero Page,X - 2 bytes
 */
function rraZpX(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact(); // Indexed addressing cycle
  const address = (zpAddress + cpu.x) & 0xff; // Wrap around in zero page
  const originalValue = cpu.readMemory(address);
 ...
 * Addressing mode: Implied - 1 byte
 */
function sei(cpu: M6510VaCpu): void {
  cpu.incrementTact(); // Internal operation cycle
  cpu.p |= FlagSetMask6510.I;
}
 ...
 * Addressing mode: Absolute,Y - 3 bytes
 */
function adcAbsY(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const baseAddress = (high << 8) | low;
  const operand = cpu.readMemoryWithPageBoundary(baseAddress, cpu.y);
 ...
 * Addressing mode: Absolute,Y - 3 bytes
 */
function rraAbsY(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const baseAddress = (high << 8) | low;
  const targetAddress = (baseAddress + cpu.y) & 0xffff;
 ...
 * Addressing mode: Absolute,X - 3 bytes
 */
function adcAbsX(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const baseAddress = (high << 8) | low;
 ...
 * Addressing mode: Absolute,X - 3 bytes
 */
function rorAbsX(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  cpu.incrementTact(); // Extra cycle for indexed addressing
 ...
 * Addressing mode: Absolute,X - 3 bytes
 */
function rraAbsX(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const baseAddress = (high << 8) | low;
  const targetAddress = (baseAddress + cpu.x) & 0xffff;
 ...
 */
function dopImm(cpu: M6510VaCpu): void {
 ...
 * Addressing mode: Indexed Indirect X - 2 bytes
 */
function staIndX(cpu: M6510VaCpu): void {
  const zeroPageAddress = cpu.readMemory(cpu.pc++);
  const effectiveAddress = (zeroPageAddress + cpu.x) & 0xff; // Wrap around in zero page
  cpu.incrementTact();
  const low = cpu.readMemory(effectiveAddress);
 ...
 */
function saxIndX(cpu: M6510VaCpu): void {
  const zeroPageAddress = cpu.readMemory(cpu.pc++);
  const effectiveAddress = (zeroPageAddress + cpu.x) & 0xff; // Wrap around in zero page
  cpu.incrementTact();
  const low = cpu.readMemory(effectiveAddress);
 ...
 * Addressing mode: Zero Page - 2 bytes
 */
function styZp(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
 ...
 * Addressing mode: Zero Page - 2 bytes
 */
function staZp(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
 ...
 * Addressing mode: Zero Page - 2 bytes
 */
function stxZp(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
 ...
 * Alternative names: AXS
 */
function saxZp(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  const result = cpu.a & cpu.x;
 ...
 * Addressing mode: Implied - 1 byte
 */
function dey(cpu: M6510VaCpu): void {
  cpu.incrementTact(); // One additional cycle for the operation
  cpu.y = (cpu.y - 1) & 0xff;
 ...
 * Addressing mode: Implied - 1 byte
 */
function txa(cpu: M6510VaCpu): void {
  cpu.incrementTact(); // Internal operation cycle
  cpu.a = cpu.x;
  cpu.setZeroAndNegativeFlags(cpu.a);
}
 ...
 */
function xaaImm(cpu: M6510VaCpu): void {
  const operand = cpu.readMemory(cpu.pc++);
 ...
 * Addressing mode: Absolute - 3 bytes
 */
function styAbs(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
 ...
 * Addressing mode: Absolute - 3 bytes
 */
function staAbs(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
 ...
 * Addressing mode: Absolute - 3 bytes
 */
function stxAbs(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
 ...
 * Alternative names: AXS
 */
function saxAbs(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  const result = cpu.a & cpu.x;
 ...
 * Addressing mode: Relative - 2 bytes
 */
function bcc(cpu: M6510VaCpu): void {
 ...
 * Addressing mode: Indirect Indexed Y - 2 bytes
 */
function staIndY(cpu: M6510VaCpu): void {
  const zeroPageAddress = cpu.readMemory(cpu.pc++);
  const low = cpu.readMemory(zeroPageAddress);
  const high = cpu.readMemory((zeroPageAddress + 1) & 0xff); // Handle zero page wrap-around
  const baseAddress = (high << 8) | low;
  const targetAddress = (baseAddress + cpu.y) & 0xffff;
  cpu.incrementTact(); // Extra cycle for page crossing
  cpu.writeMemory(targetAddress, cpu.a);
}
 ...
 */
function axaIndY(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  const low = cpu.readMemory(zpAddress);
  const high = cpu.readMemory((zpAddress + 1) & 0xff);
  const baseAddress = (high << 8) | low;
  cpu.incrementTact(); // Page boundary check for indexed addressing
  const targetAddress = (baseAddress + cpu.y) & 0xffff;
 ...
 * Addressing mode: Zero Page X - 2 bytes
 */
function styZpX(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact(); // Increment tacts for indexed addressing
 ...
 * Addressing mode: Zero Page X - 2 bytes
 */
function staZpX(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact(); // Increment tacts for indexed addressing
 ...
 */
function stxZpY(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact(); // Increment tacts for indexed addressing
 ...
 * Alternative names: AXS
 */
function saxZpY(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact(); // Indexed addressing cycle
 ...
 * Addressing mode: Implied - 1 byte
 */
function tya(cpu: M6510VaCpu): void {
  cpu.incrementTact(); // Internal operation cycle
  cpu.a = cpu.y;
  cpu.setZeroAndNegativeFlags(cpu.a);
}
 ...
 * Addressing mode: Absolute Y - 3 bytes
 */
function staAbsY(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const baseAddress = (high << 8) | low;
  const targetAddress = (baseAddress + cpu.y) & 0xffff;
 ...
 * Addressing mode: Implied - 1 byte
 */
function txs(cpu: M6510VaCpu): void {
  cpu.incrementTact(); // Internal operation cycle
  cpu.sp = cpu.x;
}
 ...
 * on different 6502/6510 chips. The high byte calculation can vary.
 */
function syaAbsX(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const baseAddress = (high << 8) | low;
  const targetAddress = (baseAddress + cpu.x) & 0xffff;
 ...
 * Addressing mode: Absolute X - 3 bytes
 */
function staAbsX(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const baseAddress = (high << 8) | low;
  const targetAddress = (baseAddress + cpu.x) & 0xffff;
 ...
 * on different 6502/6510 chips. The high byte calculation can vary.
 */
function sxaAbsY(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const baseAddress = (high << 8) | low;
  const targetAddress = (baseAddress + cpu.y) & 0xffff;
 ...
 * on different 6502/6510 chips. The high byte calculation can vary.
 */
function axaAbsY(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const baseAddress = (high << 8) | low;
  cpu.incrementTact(); // Extra cycle for indexed addressing
 ...
 * Addressing mode: Immediate - 2 bytes
 */
function ldyImm(cpu: M6510VaCpu): void {
  const value = cpu.readMemory(cpu.pc++);
  cpu.y = value;
 ...
 * Addressing mode: Indexed Indirect X - 2 bytes
 */
function ldaIndX(cpu: M6510VaCpu): void {
  const zeroPageAddress = cpu.readMemory(cpu.pc++);
 ...
 * Addressing mode: Immediate - 2 bytes
 */
function ldxImm(cpu: M6510VaCpu): void {
  const value = cpu.readMemory(cpu.pc++);
  cpu.x = value;
  cpu.setZeroAndNegativeFlags(value);
}
 ...
 * Addressing mode: Indexed Indirect X - 2 bytes
 */
function laxIndX(cpu: M6510VaCpu): void {
  const zeroPageAddress = cpu.readMemory(cpu.pc++);
  const effectiveAddress = (zeroPageAddress + cpu.x) & 0xff; // Wrap around in zero page
  cpu.incrementTact();
  const low = cpu.readMemory(effectiveAddress);
 ...
 * Addressing mode: Zero Page - 2 bytes
 */
function ldyZp(cpu: M6510VaCpu): void {
  const value = cpu.readMemory(cpu.pc++);
 ...
 * Addressing mode: Zero Page - 2 bytes
 */
function ldaZp(cpu: M6510VaCpu): void {
  const value = cpu.readMemory(cpu.pc++);
 ...
 * Addressing mode: Zero Page - 2 bytes
 */
function ldxZp(cpu: M6510VaCpu): void {
  const value = cpu.readMemory(cpu.pc++);
 ...
 * Addressing mode: Zero Page - 2 bytes
 */
function laxZp(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  const value = cpu.readMemory(zpAddress);
  cpu.a = value;
  cpu.x = value;
 ...
 * Addressing mode: Implied - 1 byte
 */
function tay(cpu: M6510VaCpu): void {
  cpu.incrementTact(); // Internal operation cycle
  cpu.y = cpu.a;
  cpu.setZeroAndNegativeFlags(cpu.y);
}
 ...
 * Addressing mode: Immediate - 2 bytes
 */
function ldaImm(cpu: M6510VaCpu): void {
  const value = cpu.readMemory(cpu.pc++);
  cpu.a = value;
  cpu.setZeroAndNegativeFlags(value);
}
 ...
 * Addressing mode: Implied - 1 byte
 */
function tax(cpu: M6510VaCpu): void {
  cpu.incrementTact(); // Internal operation cycle
  cpu.x = cpu.a;
  cpu.setZeroAndNegativeFlags(cpu.x);
}
 ...
 */
function atxImm(cpu: M6510VaCpu): void {
  const value = cpu.readMemory(cpu.pc++);
 ...
 * Addressing mode: Absolute - 3 bytes
 */
function ldyAbs(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
 ...
 * Addressing mode: Absolute - 3 bytes
 */
function ldaAbs(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const addr = (high << 8) | low;
 ...
 * Addressing mode: Absolute - 3 bytes
 */
function ldxAbs(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const addr = (high << 8) | low;
 ...
 * Addressing mode: Absolute - 3 bytes
 */
function laxAbs(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  const value = cpu.readMemory(address);
 ...
 * Addressing mode: Relative - 2 bytes
 */
function bcs(cpu: M6510VaCpu): void {
  cpu.performBranch(cpu.isCFlagSet());
}
 ...
 * Addressing mode: Indirect Indexed Y - 2 bytes
 */
function ldaIndY(cpu: M6510VaCpu): void {
  const zeroPageAddress = cpu.readMemory(cpu.pc++);
  const low = cpu.readMemory(zeroPageAddress);
 ...
 * Addressing mode: Indirect Indexed Y - 2 bytes
 */
function laxIndY(cpu: M6510VaCpu): void {
  const zeroPageAddress = cpu.readMemory(cpu.pc++);
  const low = cpu.readMemory(zeroPageAddress);
 ...
 * Addressing mode: Zero Page X - 2 bytes
 */
function ldyZpX(cpu: M6510VaCpu): void {
  const zeroPageAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact();
 ...
 * Addressing mode: Zero Page X - 2 bytes
 */
function ldaZpX(cpu: M6510VaCpu): void {
  const zeroPageAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact();
 ...
 * Addressing mode: Zero Page Y - 2 bytes
 */
function ldxZpY(cpu: M6510VaCpu): void {
  const zeroPageAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact();
 ...
 * Addressing mode: Zero Page Y - 2 bytes
 */
function laxZpY(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact(); // Indexed addressing cycle
  const address = (zpAddress + cpu.y) & 0xff; // Wrap around in zero page
  const value = cpu.readMemory(address);
 ...
 * Addressing mode: Implied - 1 byte
 */
function clv(cpu: M6510VaCpu): void {
  cpu.incrementTact(); // Internal operation cycle
  cpu.p &= ~FlagSetMask6510.V;
}
 ...
 * Addressing mode: Absolute Y - 3 bytes
 */
function ldaAbsY(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const addr = (high << 8) | low;
 ...
 * Addressing mode: Implied - 1 byte
 */
function tsx(cpu: M6510VaCpu): void {
  cpu.incrementTact(); // Internal operation cycle
  cpu.x = cpu.sp;
  cpu.setZeroAndNegativeFlags(cpu.x);
}
 ...
 */
function larAbsY(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const baseAddress = (high << 8) | low;
  const targetAddress = (baseAddress + cpu.y) & 0xffff;
  ...
  if ((baseAddress & 0xff00) !== (targetAddress & 0xff00)) {
    cpu.incrementTact(); // Extra cycle for page boundary crossing
  }
 ...
 * Addressing mode: Absolute X - 3 bytes
 */
function ldyAbsX(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const addr = (high << 8) | low;
 ...
 * Addressing mode: Absolute X - 3 bytes
 */
function ldaAbsX(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const addr = (high << 8) | low;
 ...
 * Addressing mode: Absolute Y - 3 bytes
 */
function ldxAbsY(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const addr = (high << 8) | low;
 ...
 * Addressing mode: Absolute Y - 3 bytes
 */
function laxAbsY(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const baseAddress = (high << 8) | low;
  const value = cpu.readMemoryWithPageBoundary(baseAddress, cpu.y);
 ...
 * Addressing mode: Immediate - 2 bytes
 */
function cpyImm(cpu: M6510VaCpu): void {
  const value = cpu.readMemory(cpu.pc++);
 ...
 * Addressing mode: Indexed Indirect X - 2 bytes
 */
function cmpIndX(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact(); // Indexed addressing cycle
  const indirectAddress = (zpAddress + cpu.x) & 0xff; // Wrap around in zero page
  const targetLow = cpu.readMemory(indirectAddress);
 ...
 */
function dcpIndX(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact(); // Indexed addressing cycle
  const indirectAddress = (zpAddress + cpu.x) & 0xff; // Wrap around in zero page
  const targetLow = cpu.readMemory(indirectAddress);
 ...
 * Addressing mode: Zero Page - 2 bytes
 */
function cpyZp(cpu: M6510VaCpu): void {
  const address = cpu.readMemory(cpu.pc++);
  const value = cpu.readMemory(address);
  cpu.setCompareFlags(cpu.y, value);
}
 ...
 * Addressing mode: Zero Page - 2 bytes
 */
function cmpZp(cpu: M6510VaCpu): void {
  const address = cpu.readMemory(cpu.pc++);
  const value = cpu.readMemory(address);
  cpu.setCompareFlags(cpu.a, value);
}
 ...
 * Addressing mode: Zero Page - 2 bytes
 */
function decZp(cpu: M6510VaCpu): void {
  const address = cpu.readMemory(cpu.pc++);
  cpu.incrementTact(); // Read-modify-write: extra cycle for internal operations
  const value = cpu.readMemory(address);
 ...
 * Alternative names: DCM
 */
function dcpZp(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  const originalValue = cpu.readMemory(zpAddress);
  cpu.incrementTact(); // Read-modify-write: extra cycle for internal operations
  const decrementedValue = (originalValue - 1) & 0xff;
 ...
 * Addressing mode: Implied - 1 byte
 */
function iny(cpu: M6510VaCpu): void {
  cpu.incrementTact(); // One additional cycle for the operation
  cpu.y = (cpu.y + 1) & 0xff;
  cpu.setZeroAndNegativeFlags(cpu.y);
}
 ...
 * Addressing mode: Immediate - 2 bytes
 */
function cmpImm(cpu: M6510VaCpu): void {
  const value = cpu.readMemory(cpu.pc++);
  cpu.setCompareFlags(cpu.a, value);
}
 ...
 * Addressing mode: Implied - 1 byte
 */
function dex(cpu: M6510VaCpu): void {
  cpu.incrementTact(); // One additional cycle for the operation
  cpu.x = (cpu.x - 1) & 0xff;
  cpu.setZeroAndNegativeFlags(cpu.x);
}
 ...
 */
function axsImm(cpu: M6510VaCpu): void {
  const value = cpu.readMemory(cpu.pc++);
 ...
 * Addressing mode: Absolute - 3 bytes
 */
function cpyAbs(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  const value = cpu.readMemory(address);
 ...
 * Addressing mode: Absolute - 3 bytes
 */
function cmpAbs(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  const value = cpu.readMemory(address);
 ...
 * Addressing mode: Absolute - 3 bytes
 */
function decAbs(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  cpu.incrementTact(); // Read-modify-write: extra cycle for internal operations
 ...
 * Alternative names: DCM
 */
function dcpAbs(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  const originalValue = cpu.readMemory(address);
 ...
 * Addressing mode: Relative - 2 bytes
 */
function bne(cpu: M6510VaCpu): void {
 ...
 * Addressing mode: Indirect Indexed Y - 2 bytes
 */
function cmpIndY(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
 ...
 * Alternative names: DCM
 */
function dcpIndY(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  const low = cpu.readMemory(zpAddress);
  const high = cpu.readMemory((zpAddress + 1) & 0xff);
  const baseAddress = (high << 8) | low;
 ...
 * Addressing mode: Zero Page X - 2 bytes
 */
function cmpZpX(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact(); // Indexed addressing cycle
  const address = (zpAddress + cpu.x) & 0xff; // Wrap around in zero page
  const value = cpu.readMemory(address);
 ...
 * Addressing mode: Zero Page X - 2 bytes
 */
function decZpX(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact(); // Indexed addressing cycle
  const address = (zpAddress + cpu.x) & 0xff; // Wrap around in zero page
  cpu.incrementTact(); // Read-modify-write: extra cycle for internal operations
 ...
 * Alternative names: DCM
 */
function dcpZpX(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact(); // Indexed addressing cycle
  const address = (zpAddress + cpu.x) & 0xff; // Wrap around in zero page
  const originalValue = cpu.readMemory(address);
 ...
 * Addressing mode: Implied - 1 byte
 */
function cld(cpu: M6510VaCpu): void {
  cpu.incrementTact(); // Internal operation cycle
 ...
 * Addressing mode: Absolute Y - 3 bytes
 */
function cmpAbsY(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const baseAddress = (high << 8) | low;
  const value = cpu.readMemoryWithPageBoundary(baseAddress, cpu.y);
 ...
 * Alternative names: DCM
 */
function dcpAbsY(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const baseAddress = (high << 8) | low;
  cpu.incrementTact(); // Extra cycle for indexed addressing
 ...
 * Addressing mode: Absolute X - 3 bytes
 */
function cmpAbsX(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const baseAddress = (high << 8) | low;
 ...
 * Addressing mode: Absolute X - 3 bytes
 */
function decAbsX(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const baseAddress = (high << 8) | low;
 ...
 * Alternative names: DCM
 */
function dcpAbsX(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const baseAddress = (high << 8) | low;
  cpu.incrementTact(); // Extra cycle for indexed addressing
 ...
 * Addressing mode: Immediate - 2 bytes
 */
function cpxImm(cpu: M6510VaCpu): void {
  const value = cpu.readMemory(cpu.pc++);
 ...
 * Addressing mode: Indexed Indirect X - 2 bytes
 */
function sbcIndX(cpu: M6510VaCpu): void {
  const zeroPageAddress = cpu.readMemory(cpu.pc++);
  const effectiveAddress = (zeroPageAddress + cpu.x) & 0xff; // Wrap around in zero page
  cpu.incrementTact(); // Indexed addressing cycle
  const low = cpu.readMemory(effectiveAddress);
 ...
 */
function iscIndX(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact(); // Indexed addressing cycle
  const indirectAddress = (zpAddress + cpu.x) & 0xff; // Wrap around in zero page
  const targetLow = cpu.readMemory(indirectAddress);
 ...
 * Addressing mode: Zero Page - 2 bytes
 */
function cpxZp(cpu: M6510VaCpu): void {
  const address = cpu.readMemory(cpu.pc++);
  const value = cpu.readMemory(address);
  cpu.setCompareFlags(cpu.x, value);
}
 ...
 * Addressing mode: Zero Page - 2 bytes
 */
function sbcZp(cpu: M6510VaCpu): void {
  const address = cpu.readMemory(cpu.pc++);
  const operand = cpu.readMemory(address);
  cpu.a = cpu.performSbc(operand);
}
 ...
 * Addressing mode: Zero Page - 2 bytes
 */
function incZp(cpu: M6510VaCpu): void {
  const address = cpu.readMemory(cpu.pc++);
  cpu.incrementTact(); // Read-modify-write: extra cycle for internal operations
  const value = cpu.readMemory(address);
  const result = (value + 1) & 0xff;
 ...
 * Alternative names: INS, ISB
 */
function iscZp(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  const originalValue = cpu.readMemory(zpAddress);
  cpu.incrementTact(); // Read-modify-write: extra cycle for internal operations
  const incrementedValue = (originalValue + 1) & 0xff;
 ...
 * Addressing mode: Implied - 1 byte
 */
function inx(cpu: M6510VaCpu): void {
  cpu.incrementTact(); // One additional cycle for the operation
  cpu.x = (cpu.x + 1) & 0xff;
  cpu.setZeroAndNegativeFlags(cpu.x);
}
 ...
 * Addressing mode: Immediate - 2 bytes
 */
function sbcImm(cpu: M6510VaCpu): void {
  const operand = cpu.readMemory(cpu.pc++);
  cpu.a = cpu.performSbc(operand);
}
 ...
 * Addressing mode: Implied - 1 byte
 */
function nop(cpu: M6510VaCpu): void {
  cpu.internalTact(); // Internal operation cycle
}
 ...
 * Addressing mode: Absolute - 3 bytes
 */
function cpxAbs(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  const value = cpu.readMemory(address);
 ...
 * Addressing mode: Absolute - 3 bytes
 */
function sbcAbs(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  const operand = cpu.readMemory(address);
 ...
 * Addressing mode: Absolute - 3 bytes
 */
function incAbs(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  cpu.incrementTact(); // Read-modify-write: extra cycle for internal operations
 ...
 * Alternative names: INS, ISB
 */
function iscAbs(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const address = (high << 8) | low;
  const originalValue = cpu.readMemory(address);
 ...
 * Addressing mode: Relative - 2 bytes
 */
function beq(cpu: M6510VaCpu): void {
  cpu.performBranch(cpu.isZFlagSet());
}
 ...
 * Addressing mode: Indirect Indexed Y - 2 bytes
 */
function sbcIndY(cpu: M6510VaCpu): void {
  const zeroPageAddress = cpu.readMemory(cpu.pc++);
  const low = cpu.readMemory(zeroPageAddress);
  const high = cpu.readMemory((zeroPageAddress + 1) & 0xff); // Handle zero page wrap-around
  const baseAddress = (high << 8) | low;
 ...
 * Alternative names: INS, ISB
 */
function iscIndY(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  const low = cpu.readMemory(zpAddress);
  const high = cpu.readMemory((zpAddress + 1) & 0xff);
  const baseAddress = (high << 8) | low;
 ...
 * Addressing mode: Zero Page X - 2 bytes
 */
function sbcZpX(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact(); // Indexed addressing cycle
  const address = (zpAddress + cpu.x) & 0xff; // Wrap around in zero page
  const operand = cpu.readMemory(address);
 ...
 * Addressing mode: Zero Page X - 2 bytes
 */
function incZpX(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact(); // Indexed addressing cycle
  const address = (zpAddress + cpu.x) & 0xff; // Wrap around in zero page
  cpu.incrementTact(); // Read-modify-write: extra cycle for internal operations
 ...
 * Alternative names: INS, ISB
 */
function iscZpX(cpu: M6510VaCpu): void {
  const zpAddress = cpu.readMemory(cpu.pc++);
  cpu.incrementTact(); // Indexed addressing cycle
  const address = (zpAddress + cpu.x) & 0xff; // Wrap around in zero page
  const originalValue = cpu.readMemory(address);
 ...
 * Addressing mode: Implied - 1 byte
 */
function sed(cpu: M6510VaCpu): void {
  cpu.incrementTact(); // Internal operation cycle
  cpu.p |= FlagSetMask6510.D;
}
 ...
 * Addressing mode: Absolute Y - 3 bytes
 */
function sbcAbsY(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const baseAddress = (high << 8) | low;
  const operand = cpu.readMemoryWithPageBoundary(baseAddress, cpu.y);
 ...
 * Alternative names: INS, ISB
 */
function iscAbsY(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const baseAddress = (high << 8) | low;
  cpu.incrementTact(); // Extra cycle for indexed addressing
 ...
 * Addressing mode: Absolute X - 3 bytes
 */
function sbcAbsX(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const baseAddress = (high << 8) | low;
  const operand = cpu.readMemoryWithPageBoundary(baseAddress, cpu.x);
 ...
 * Addressing mode: Absolute X - 3 bytes
 */
function incAbsX(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const baseAddress = (high << 8) | low;
  const address = (baseAddress + cpu.x) & 0xffff;
 ...
 * Alternative names: INS, ISB
 */
function iscAbsX(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const baseAddress = (high << 8) | low;
  cpu.incrementTact(); // Extra cycle for indexed addressing
...
function xasAbsY(cpu: M6510VaCpu): void {
  const low = cpu.readMemory(cpu.pc++);
  const high = cpu.readMemory(cpu.pc++);
  const baseAddress = (high << 8) | low;
  const targetAddress = (baseAddress + cpu.y) & 0xffff;
