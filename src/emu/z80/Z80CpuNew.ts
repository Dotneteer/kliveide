 ...
 */
export class Z80CpuNew implements IZ80Cpu {
  // --- Register variables (optimized direct access)
  private _a = 0;
  private _f = 0;
  private _b = 0;
  private _c = 0;
   ...
   */
  get indexReg(): number {
    return this.prefix == OpCodePrefix.DD || this.prefix == OpCodePrefix.DDCB 
      ? (this._xh << 8) | this._xl 
      : (this._yh << 8) | this._yl;
  }
   ...
   */
  get indexL(): number {
    return this.prefix == OpCodePrefix.DD || this.prefix == OpCodePrefix.DDCB ? this._xl : this._yl;
  }
   ...
   */
  get indexH(): number {
    return this.prefix == OpCodePrefix.DD || this.prefix == OpCodePrefix.DDCB ? this._xh : this._yh;
  }
  ...
  get flagsSZPVValue(): number {
    return this._f & FlagsSetMask.SZPV;
  }
  ...
  executeCpuCycle(): void {
    ...
    switch (this.prefix) {
      ...
      case OpCodePrefix.FDCB:
        ...
        const indexRegValue = this.indexReg;
        const wzValue = indexRegValue + sbyte(this.opCode);
        this._wh = (wzValue >>> 8) & 0xff;
        this._wl = wzValue & 0xff;
        this.opCode = this.readMemory(this.pc);
        this.tactPlus2WithPC();
        this.pc++;
  ...
  private processInt(): void {
    ...
    if (this.interruptMode === 2) {
      ...
      var addr = (this._i << 8) + 0xff;
      this._wl = this.readMemory(addr++);
      this._wh = this.readMemory(addr);
    } else {
  ...
  relativeJump(e: number): void {
    this.tactPlus5WithPC();
    const newPc = this.pc + sbyte(e);
    this.pc = newPc;
    this._wh = (newPc >>> 8) & 0xff;
    this._wl = newPc & 0xff;
  }
  ...
  add16(regHl: number, regOther: number): number {
    ...
    this._wh = ((regHl + 1) >>> 8) & 0xff;
    this._wl = (regHl + 1) & 0xff;
    this._f =
      (this._f & FlagsSetMask.SZPV) |
  ...
  adc16(value: number): void {
    const hlValue = (this._h << 8) | this._l;
    const tmpVal = hlValue + value + (this._f & FlagsSetMask.C);
    const lookup =
      ((hlValue & 0x8800) >>> 11) | ((value & 0x8800) >>> 10) | ((tmpVal & 0x8800) >>> 9);
    this._wh = ((hlValue + 1) >>> 8) & 0xff;
    this._wl = (hlValue + 1) & 0xff;
    this._h = (tmpVal >>> 8) & 0xff;
    this._l = tmpVal & 0xff;
    this._f =
      ((tmpVal & 0x10000) !== 0 ? FlagsSetMask.C : 0) |
      overflowAddFlags[lookup >>> 4] |
      (this._h & (FlagsSetMask.R3R5 | FlagsSetMask.S)) |
      halfCarryAddFlags[lookup & 0x07] |
      (((this._h << 8) | this._l) !== 0 ? 0 : FlagsSetMask.Z);
  }
  ...
  sub8(value: number): void {
    const tmp = this._a - value;
    const lookup = ((this._a & 0x88) >>> 3) | ((value & 0x88) >>> 2) | ((tmp & 0x88) >>> 1);
    this._a = tmp & 0xff;
    this._f =
  ...
  sbc8(value: number): void {
    const tmp = this._a - value - (this._f & FlagsSetMask.C);
    const lookup = ((this._a & 0x88) >>> 3) | ((value & 0x88) >>> 2) | ((tmp & 0x88) >>> 1);
    this._a = tmp & 0xff;
    this._f =
      ((tmp & 0x100) !== 0 ? FlagsSetMask.C : 0) |
  ...
  add8(value: number): void {
    const tmp = this._a + value;
    var lookup = ((this._a & 0x88) >>> 3) | ((value & 0x88) >>> 2) | ((tmp & 0x88) >>> 1);
    this._a = tmp & 0xff;
    this._f =
      ((tmp & 0x100) != 0 ? FlagsSetMask.C : 0) |
      halfCarryAddFlags[lookup & 0x07] |
  ...
  and8(value: number): void {
    this._a &= value;
    this._f = FlagsSetMask.H | sz53pvTable[this._a];
  }
  ...
  rlc8(oper: number): number {
    ...
    this._f = (result & FlagsSetMask.C) | sz53pvTable[result];
    return result;
  }
  ...
  rrc8(oper: number): number {
    this._f = oper & FlagsSetMask.C;
    const result = ((oper >>> 1) | (oper << 7)) & 0xff;
    this._f |= sz53pvTable[result];
    return result;
  }
  ...
  rl8(oper: number): number {
    const result = ((oper << 1) | (this._f & FlagsSetMask.C)) & 0xff;
    this._f = (oper >>> 7) | sz53pvTable[result];
    return result;
  }
  ...
  rr8(oper: number): number {
    const result = ((oper >>> 1) | (this._f << 7)) & 0xff;
    this._f = (oper & FlagsSetMask.C) | sz53pvTable[result];
    return result;
  }
  ...
  bit8(bit: number, oper: number): void {
    this._f = (this._f & FlagsSetMask.C) | FlagsSetMask.H | (oper & FlagsSetMask.R3R5);
    const bitVal = oper & (0x01 << bit);
    if (bitVal === 0) {
      this._f |= FlagsSetMask.PV | FlagsSetMask.Z;
    }
    this._f |= bitVal & FlagsSetMask.S;
  }
  ...
  bit8W(bit: number, oper: number): void {
    this._f = (this._f & FlagsSetMask.C) | FlagsSetMask.H | (this._wh & FlagsSetMask.R3R5);
    const bitVal = oper & (0x01 << bit);
    if (bitVal === 0) {
      this._f |= FlagsSetMask.PV | FlagsSetMask.Z;
    }
  ...
  tactPlus4WithAddress(address: number): void {
    if (this.delayedAddressBus) {
      // Slow path with address bus delay
      for (let i = 0; i < 4; i++) {
        this.delayAddressBusAccess(address);
        this.tacts += 1;
        this.frameTacts += 1;
  ...
  // ----------------------------------------------------------------------------------------------------------------
  // Specialized timing functions for common patterns

  /**
   * Specialized timing function for tactPlus1WithAddress(this.ir) - very common in I/O operations
   */
  tactPlus1WithIR(): void {
    const irAddress = (this._i << 8) | this._r;
    if (this.delayedAddressBus) {
      // Slow path with address bus delay
      this.delayAddressBusAccess(irAddress);
      this.tacts += 1;
      this.frameTacts += 1;
      if (this.frameTacts >= this.tactsInCurrentFrame) {
        this.frames++;
  ...
  /**
   * Specialized timing function for tactPlus2WithAddress(this.ir) - common in ED prefix operations
   */
  tactPlus2WithIR(): void {
    const irAddress = (this._i << 8) | this._r;
    if (this.delayedAddressBus) {
      // Slow path with address bus delay
      for (let i = 0; i < 2; i++) {
  ...
  /**
   * Specialized timing function for tactPlus5WithAddress(this.pc) - common in PC-relative operations
   */
  tactPlus5WithPC(): void {
    if (this.delayedAddressBus) {
      // Slow path with address bus delay
      for (let i = 0; i < 5; i++) {
        this.delayAddressBusAccess(this.pc);
        this.tacts += 1;
        this.frameTacts += 1;
        if (this.frameTacts >= this.tactsInCurrentFrame) {
          this.frames++;
  ...
  /**
   * Specialized timing function for tactPlus7WithAddress(this.ir) - common in 16-bit operations
   */
  tactPlus7WithIR(): void {
    const irAddress = (this._i << 8) | this._r;
    if (this.delayedAddressBus) {
      // Slow path with address bus delay
      for (let i = 0; i < 7; i++) {
  ...
  /**
   * Specialized timing function for tactPlus1WithAddress(HL) operations - very common in bit operations
   */
  tactPlus1WithHL(): void {
    const hlAddress = this.hl;
    if (this.delayedAddressBus) {
      // Slow path with address bus delay
      this.delayAddressBusAccess(hlAddress);
      this.tacts += 1;
      this.frameTacts += 1;
      if (this.frameTacts >= this.tactsInCurrentFrame) {
        this.frames++;
  ...
  /**
   * Specialized timing function for tactPlus1WithAddress(this.pc) operations
   */
  tactPlus1WithPC(): void {
    if (this.delayedAddressBus) {
      // Slow path with address bus delay
      this.delayAddressBusAccess(this.pc);
      this.tacts += 1;
  ...
  /**
   * Specialized timing function for tactPlus2WithAddress(this.pc) operations
   */
  tactPlus2WithPC(): void {
    if (this.delayedAddressBus) {
      // Slow path with address bus delay
      for (let i = 0; i < 2; i++) {
        this.delayAddressBusAccess(this.pc);
 ...
 */
export type Z80Operation = (cpu: Z80CpuNew) => void;
...
// 0x02: LD (BC),A
function ldBciA(cpu: Z80CpuNew) {
  cpu.writeMemory(cpu.bc, cpu.a);
  cpu.wh = cpu.a;
}

// 0x03: INC BC
function incBc(cpu: Z80CpuNew) {
  cpu.bc++;
  cpu.tactPlus2WithIR();
}
...
// 0x05: DEC B
function decB(cpu: Z80CpuNew) {
  cpu.f = decFlags[cpu.b--] | cpu.flagCValue;
}

// 0x06: LD B,n
function ldBN(cpu: Z80CpuNew) {
  cpu.b = cpu.fetchCodeByte();
}

// 0x07: RLCA
function rlca(cpu: Z80CpuNew) {
  let rlcaVal = cpu.a;
  rlcaVal = rlcaVal << 1;
  ...
  cpu.a = rlcaVal;
  cpu.f = cf | cpu.flagsSZPVValue | (rlcaVal & FlagsSetMask.R3R5);
}

// 0x08: EX AF,AF'
function exAf(cpu: Z80CpuNew) {
  const tmp = cpu.af;
  cpu.af = cpu.af_;
  cpu.af_ = tmp;
}
...
// 0x0b: DEC BC
function decBc(cpu: Z80CpuNew) {
  cpu.bc--;
  cpu.tactPlus2WithIR();
}

// 0x0c: INC C
function incC(cpu: Z80CpuNew) {
  cpu.f = incFlags[cpu.c++] | cpu.flagCValue;
}

// 0x0d: DEC C
function decC(cpu: Z80CpuNew) {
  cpu.f = decFlags[cpu.c--] | cpu.flagCValue;
}

// 0x0e: LD C,n
function ldCN(cpu: Z80CpuNew) {
  cpu.c = cpu.fetchCodeByte();
}

// 0x0f: RRCA
function rrca(cpu: Z80CpuNew) {
  let rrcaVal = cpu.a;
  ...
  cpu.a = rrcaVal;
  cpu.f = cf | cpu.flagsSZPVValue | (rrcaVal & FlagsSetMask.R3R5);
}

// 0x10: DJNZ d
function djnz(cpu: Z80CpuNew) {
  cpu.tactPlus1WithIR();
...
// 0x11: LD DE,nn
function ldDeNN(cpu: Z80CpuNew) {
  cpu.e = cpu.fetchCodeByte();
  cpu.d = cpu.fetchCodeByte();
}

// 0x12: LD (BC),A
function ldDeiA(cpu: Z80CpuNew) {
  cpu.writeMemory(cpu.de, cpu.a);
  cpu.wh = cpu.a;
}

// 0x13: INC DE
function incDe(cpu: Z80CpuNew) {
  cpu.de++;
  cpu.tactPlus2WithIR();
}

// 0x14: INC D
function incD(cpu: Z80CpuNew) {
  cpu.f = incFlags[cpu.d++] | cpu.flagCValue;
}

// 0x15: DEC D
function decD(cpu: Z80CpuNew) {
  cpu.f = decFlags[cpu.d--] | cpu.flagCValue;
}

// 0x16: LD D,n
function ldDN(cpu: Z80CpuNew) {
  cpu.d = cpu.fetchCodeByte();
}

// 0x17: RLA
function rla(cpu: Z80CpuNew) {
  let rlaVal = cpu.a;
  ...
  cpu.a = rlaVal;
  cpu.f = newCF | cpu.flagsSZPVValue | (rlaVal & FlagsSetMask.R3R5);
}

// 0x18: JR e
function jr(cpu: Z80CpuNew) {
...
// 0x1b: DEC DE
function decDe(cpu: Z80CpuNew) {
  cpu.de--;
  cpu.tactPlus2WithIR();
}

// 0x1c: INC E
function incE(cpu: Z80CpuNew) {
  cpu.f = incFlags[cpu.e++] | cpu.flagCValue;
}

// 0x1d: DEC E
function decE(cpu: Z80CpuNew) {
  cpu.f = decFlags[cpu.e--] | cpu.flagCValue;
}

// 0x1e: LD E,n
function ldEN(cpu: Z80CpuNew) {
  cpu.e = cpu.fetchCodeByte();
}

// 0x1f: RRA
function rra(cpu: Z80CpuNew) {
  let rraVal = cpu.a;
  ...
  cpu.a = rraVal;
  cpu.f = newCF | cpu.flagsSZPVValue | (rraVal & FlagsSetMask.R3R5);
}

// 0x20: JR NZ,e
function jrnz(cpu: Z80CpuNew) {
  const e = cpu.fetchCodeByte();
  if (!cpu.isZFlagSet()) {
    cpu.relativeJump(e);
  }
...
// 0x2a: LD HL,(nn)
function ldHlNNi(cpu: Z80CpuNew) {
...
// 0x3b: DEC SP
function decSp(cpu: Z80CpuNew) {
  cpu.sp--;
  cpu.tactPlus2WithIR();
}

// 0x3c: INC A
function incA(cpu: Z80CpuNew) {
  cpu.f = incFlags[cpu.a++] | cpu.flagCValue;
}

// 0x3d: DEC A
function decA(cpu: Z80CpuNew) {
  cpu.f = decFlags[cpu.a--] | cpu.flagCValue;
}

// 0x3e: LD A,n
function ldAN(cpu: Z80CpuNew) {
  cpu.a = cpu.fetchCodeByte();
}

// 0x3f: CCF
function ccf(cpu: Z80CpuNew) {
  cpu.f = cpu.flagsSZPVValue | (cpu.isCFlagSet() ? FlagsSetMask.H : FlagsSetMask.C);
  cpu.f = (cpu.f & ~FlagsSetMask.R3R5) | (cpu.a & FlagsSetMask.R3R5);
}

// 0x41: LD B,C
function ldBC(cpu: Z80CpuNew) {
  cpu.b = cpu.c;
}

// 0x42: LD B,D
function ldBD(cpu: Z80CpuNew) {
  cpu.b = cpu.d;
}

// 0x43: LD B,E
function ldBE(cpu: Z80CpuNew) {
  cpu.b = cpu.e;
}
...
// 0x47: LD B,A
function ldBA(cpu: Z80CpuNew) {
  cpu.b = cpu.a;
}

// 0x48: LD C,B
function ldCB(cpu: Z80CpuNew) {
  cpu.c = cpu.b;
}

// 0x4a: LD C,D
function ldCD(cpu: Z80CpuNew) {
  cpu.c = cpu.d;
}

// 0x4b LD C,E
function ldCE(cpu: Z80CpuNew) {
  cpu.c = cpu.e;
}
...
// 0x4f: LD C,A
function ldCA(cpu: Z80CpuNew) {
  cpu.c = cpu.a;
}

// 0x50: LD D,B
function ldDB(cpu: Z80CpuNew) {
  cpu.d = cpu.b;
}

// 0x51: LD D,C
function ldDC(cpu: Z80CpuNew) {
  cpu.d = cpu.c;
}

// 0x53: LD D,E
function ldDE(cpu: Z80CpuNew) {
  cpu.d = cpu.e;
}
...
// 0x57: LD D,A
function ldDA(cpu: Z80CpuNew) {
  cpu.d = cpu.a;
}

// 0x58: LD E,B
function ldEB(cpu: Z80CpuNew) {
  cpu.e = cpu.b;
}

// 0x59: LD E,C
function ldEC(cpu: Z80CpuNew) {
  cpu.e = cpu.c;
}

// 0x5a: LD E,D
function ldED(cpu: Z80CpuNew) {
  cpu.e = cpu.d;
}
...
// 0x71: LD (HL),C
function ldHliC(cpu: Z80CpuNew) {
  cpu.writeMemory(cpu.hl, cpu.c);
}

// 0x72: LD (HL),D
function ldHliD(cpu: Z80CpuNew) {
  cpu.writeMemory(cpu.hl, cpu.d);
}

// 0x73: LD (HL),E
function ldHliE(cpu: Z80CpuNew) {
  cpu.writeMemory(cpu.hl, cpu.e);
}

// 0x74: LD (HL),H
function ldHliH(cpu: Z80CpuNew) {
  cpu.writeMemory(cpu.hl, cpu.h);
}

// 0x75: LD (HL),L
function ldHliL(cpu: Z80CpuNew) {
  cpu.writeMemory(cpu.hl, cpu.l);
}

// 0x76: HALT
function halt(cpu: Z80CpuNew) {
  cpu.halted = true;
  cpu.pc--;
}

// 0x77: LD (HL),A
function ldHliA(cpu: Z80CpuNew) {
  cpu.writeMemory(cpu.hl, cpu.a);
}

// 0x78: LD A,B
function ldAB(cpu: Z80CpuNew) {
  cpu.a = cpu.b;
}

// 0x79: LD A,C
function ldAC(cpu: Z80CpuNew) {
  cpu.a = cpu.c;
}

// 0x7A: LD A,D
function ldAD(cpu: Z80CpuNew) {
  cpu.a = cpu.d;
}

// 0x7B: LD A,E
function ldAE(cpu: Z80CpuNew) {
  cpu.a = cpu.e;
}
...
// 0x80: ADD A,B
function addAB(cpu: Z80CpuNew) {
  cpu.add8(cpu.b);
}

// 0x81: ADD A,C
function addAC(cpu: Z80CpuNew) {
  cpu.add8(cpu.c);
}

// 0x82: ADD A,D
function addAD(cpu: Z80CpuNew) {
  cpu.add8(cpu.d);
}

// 0x83: ADD A,E
function addAE(cpu: Z80CpuNew) {
  cpu.add8(cpu.e);
}

// 0x84: ADD A,H
function addAH(cpu: Z80CpuNew) {
  cpu.add8(cpu.h);
}

// 0x85: ADD A,L
function addAL(cpu: Z80CpuNew) {
  cpu.add8(cpu.l);
}
...
// 0x88: ADC A,B
function adcAB(cpu: Z80CpuNew) {
  cpu.adc8(cpu.b);
}

// 0x89: ADC A,C
function adcAC(cpu: Z80CpuNew) {
  cpu.adc8(cpu.c);
}

// 0x8A: ADC A,D
function adcAD(cpu: Z80CpuNew) {
  cpu.adc8(cpu.d);
}

// 0x8B: ADC A,E
function adcAE(cpu: Z80CpuNew) {
  cpu.adc8(cpu.e);
}

// 0x8C: ADC A,H
function adcAH(cpu: Z80CpuNew) {
  cpu.adc8(cpu.h);
}

// 0x8D: ADC A,L
function adcAL(cpu: Z80CpuNew) {
  cpu.adc8(cpu.l);
}
...
// 0x90: SUB A,B
function subAB(cpu: Z80CpuNew) {
  cpu.sub8(cpu.b);
}

// 0x91: SUB A,C
function subAC(cpu: Z80CpuNew) {
  cpu.sub8(cpu.c);
}

// 0x92: SUB A,D
function subAD(cpu: Z80CpuNew) {
  cpu.sub8(cpu.d);
}

// 0x93: SUB A,E
function subAE(cpu: Z80CpuNew) {
  cpu.sub8(cpu.e);
}

// 0x94: SUB A,H
function subAH(cpu: Z80CpuNew) {
  cpu.sub8(cpu.h);
}

// 0x95: SUB A,L
function subAL(cpu: Z80CpuNew) {
  cpu.sub8(cpu.l);
}
...
// 0x98: SBC A,B
function sbcAB(cpu: Z80CpuNew) {
  cpu.sbc8(cpu.b);
}

// 0x99: SBC A,C
function sbcAC(cpu: Z80CpuNew) {
  cpu.sbc8(cpu.c);
}

// 0x9A: SBC A,D
function sbcAD(cpu: Z80CpuNew) {
  cpu.sbc8(cpu.d);
}

// 0x9B: SBC A,E
function sbcAE(cpu: Z80CpuNew) {
  cpu.sbc8(cpu.e);
}

// 0x9C: SBC A,H
function sbcAH(cpu: Z80CpuNew) {
  cpu.sbc8(cpu.h);
}

// 0x9D: SBC A,L
function sbcAL(cpu: Z80CpuNew) {
  cpu.sbc8(cpu.l);
}
...
// 0xa0: AND A,B
function andAB(cpu: Z80CpuNew) {
  cpu.and8(cpu.b);
}

// 0xa1: AND A,C
function andAC(cpu: Z80CpuNew) {
  cpu.and8(cpu.c);
}

// 0xa2: AND A,D
function andAD(cpu: Z80CpuNew) {
  cpu.and8(cpu.d);
}

// 0xa3: AND A,E
function andAE(cpu: Z80CpuNew) {
  cpu.and8(cpu.e);
}
...
// 0xa8: XOR A,B
function xorAB(cpu: Z80CpuNew) {
  cpu.xor8(cpu.b);
}

// 0xa9: XOR A,C
function xorAC(cpu: Z80CpuNew) {
  cpu.xor8(cpu.c);
}
// 0xaa: XOR A,D
function xorAD(cpu: Z80CpuNew) {
  cpu.xor8(cpu.d);
}

// 0xab: XOR A,E
function xorAE(cpu: Z80CpuNew) {
  cpu.xor8(cpu.e);
}
...
// 0xb0: OR A,B
function orAB(cpu: Z80CpuNew) {
  cpu.or8(cpu.b);
}

// 0xb1: OR A,C
function orAC(cpu: Z80CpuNew) {
  cpu.or8(cpu.c);
}

// 0xb2: OR A,D
function orAD(cpu: Z80CpuNew) {
  cpu.or8(cpu.d);
}

// 0xb3: OR A,E
function orAE(cpu: Z80CpuNew) {
  cpu.or8(cpu.e);
}
...
// 0xb8: CP B
function cpB(cpu: Z80CpuNew) {
  cpu.cp8(cpu.b);
}

// 0xb9: CP C
function cpC(cpu: Z80CpuNew) {
  cpu.cp8(cpu.c);
}

// 0xba: CP D
function cpD(cpu: Z80CpuNew) {
  cpu.cp8(cpu.d);
}

// 0xbb: CP E
function cpE(cpu: Z80CpuNew) {
  cpu.cp8(cpu.e);
}

// 0xbc: CP H
function cpH(cpu: Z80CpuNew) {
  cpu.cp8(cpu.h);
}
...
// 0xc6: ADD A,N
function addAN(cpu: Z80CpuNew) {
  cpu.add8(cpu.fetchCodeByte());
}

// 0xc7: RST 00
function rst00(cpu: Z80CpuNew) {
  cpu.rstCore(0x0000);
}

// 0xc8: RET Z
function retZ(cpu: Z80CpuNew) {
  cpu.tactPlus1WithIR();
  if (cpu.isZFlagSet()) {
    ret(cpu);
  }
...
// 0xce: ADC A,N
function adcAN(cpu: Z80CpuNew) {
  cpu.adc8(cpu.fetchCodeByte());
}

// 0xcf: RST 08
function rst08(cpu: Z80CpuNew) {
  cpu.rstCore(0x0008);
}

// 0xd0: RET NC
function retNc(cpu: Z80CpuNew) {
  cpu.tactPlus1WithIR();
  if (!cpu.isCFlagSet()) {
    ret(cpu);
  }
...
// 0xd6: SUB A,N
function subAN(cpu: Z80CpuNew) {
  cpu.sub8(cpu.fetchCodeByte());
}

// 0xd7: RST 10
function rst10(cpu: Z80CpuNew) {
  cpu.rstCore(0x0010);
}

// 0xd8: RET C
function retC(cpu: Z80CpuNew) {
  cpu.tactPlus1WithIR();
  if (cpu.isCFlagSet()) {
    ret(cpu);
  }
...
// 0xde: SBC A,N
function sbcAN(cpu: Z80CpuNew) {
  cpu.sbc8(cpu.fetchCodeByte());
}

// 0xdf: RST 18
function rst18(cpu: Z80CpuNew) {
  cpu.rstCore(0x0018);
}

// 0xe0: RET PO
function retPo(cpu: Z80CpuNew) {
  cpu.tactPlus1WithIR();
  if (!cpu.isPvFlagSet()) {
    ret(cpu);
  }
...
// 0xe3: EX (SP),HL
function exSpiHl(cpu: Z80CpuNew) {
  const sp1 = cpu.sp + 1;
...
// 0xe6: AND A,N
function andAN(cpu: Z80CpuNew) {
  cpu.and8(cpu.fetchCodeByte());
}

// 0xe7: RST 20
function rst20(cpu: Z80CpuNew) {
  cpu.rstCore(0x0020);
}

// 0xe8: RET PE
function retPe(cpu: Z80CpuNew) {
  cpu.tactPlus1WithIR();
  if (cpu.isPvFlagSet()) {
    ret(cpu);
  }
...
// 0xe9: JP (HL)
function jpHl(cpu: Z80CpuNew) {
  cpu.pc = cpu.hl;
}

// 0xea: JP PE,nn
function jpPe(cpu: Z80CpuNew) {
  cpu.wl = cpu.fetchCodeByte();
  cpu.wh = cpu.fetchCodeByte();
  if (cpu.isPvFlagSet()) {
    cpu.pc = cpu.wz;
...
// 0xee: XOR A,N
function xorAN(cpu: Z80CpuNew) {
  cpu.xor8(cpu.fetchCodeByte());
}

// 0xef: RST 28
function rst28(cpu: Z80CpuNew) {
  cpu.rstCore(0x0028);
}

// 0xf0: RET P
function retP(cpu: Z80CpuNew) {
  cpu.tactPlus1WithIR();
  if (!cpu.isSFlagSet()) {
    ret(cpu);
  }
...
// 0xf3: DI
function di(cpu: Z80CpuNew) {
  cpu.iff2 = cpu.iff1 = false;
}

// 0xf4: CALL P,nn
function callP(cpu: Z80CpuNew) {
  cpu.wl = cpu.fetchCodeByte();
  cpu.wh = cpu.fetchCodeByte();
  if (!cpu.isSFlagSet()) {
    cpu.callCore();
...
// 0xf6: OR A,N
function orAN(cpu: Z80CpuNew) {
  cpu.or8(cpu.fetchCodeByte());
}

// 0xf7: RST 30
function rst30(cpu: Z80CpuNew) {
  cpu.rstCore(0x0030);
}

// 0xf8: RET M
function retM(cpu: Z80CpuNew) {
  cpu.tactPlus1WithIR();
  if (cpu.isSFlagSet()) {
    ret(cpu);
  }
...
// 0xfb: EI
function ei(cpu: Z80CpuNew) {
  cpu.iff2 = cpu.iff1 = true;
  cpu.eiBacklog = 2;
}

// 0xfc: CALL M,nn
function callM(cpu: Z80CpuNew) {
  cpu.wl = cpu.fetchCodeByte();
  cpu.wh = cpu.fetchCodeByte();
  if (cpu.isSFlagSet()) {
    cpu.callCore();
...
// 0xfe: CP A,N
function cpAN(cpu: Z80CpuNew) {
  cpu.cp8(cpu.fetchCodeByte());
}

// 0xff: RST 38
function rst38(cpu: Z80CpuNew) {
  cpu.rstCore(0x0038);
}
...
// 0x00: RLC B
function rlcB(cpu: Z80CpuNew) {
  cpu.b = cpu.rlc8(cpu.b);
}

// 0x01: RLC C
function rlcC(cpu: Z80CpuNew) {
  cpu.c = cpu.rlc8(cpu.c);
}

// 0x02: RLC D
function rlcD(cpu: Z80CpuNew) {
  cpu.d = cpu.rlc8(cpu.d);
}

// 0x03: RLC E
function rlcE(cpu: Z80CpuNew) {
  cpu.e = cpu.rlc8(cpu.e);
}

// 0x04: RLC H
function rlcH(cpu: Z80CpuNew) {
  cpu.h = cpu.rlc8(cpu.h);
}

// 0x05: RLC L
function rlcL(cpu: Z80CpuNew) {
  cpu.l = cpu.rlc8(cpu.l);
}

// 0x06: RLC (HL)
function rlcHli(cpu: Z80CpuNew) {
  const hlAddress = cpu.hl;
  const tmp = cpu.rlc8(cpu.readMemory(hlAddress));
  cpu.tactPlus1WithHL();
  cpu.writeMemory(hlAddress, tmp);
}
...
// 0x08: RRC B
function rrcB(cpu: Z80CpuNew) {
  cpu.b = cpu.rrc8(cpu.b);
}

// 0x09: RRC C
function rrcC(cpu: Z80CpuNew) {
  cpu.c = cpu.rrc8(cpu.c);
}

// 0x0a: RRC D
function rrcD(cpu: Z80CpuNew) {
  cpu.d = cpu.rrc8(cpu.d);
}

// 0x0b: RRC E
function rrcE(cpu: Z80CpuNew) {
  cpu.e = cpu.rrc8(cpu.e);
}

// 0x0c: RRC H
function rrcH(cpu: Z80CpuNew) {
  cpu.h = cpu.rrc8(cpu.h);
}

// 0x0d: RRC L
function rrcL(cpu: Z80CpuNew) {
  cpu.l = cpu.rrc8(cpu.l);
}

// 0x0e: RRC (HL)
function rrcHli(cpu: Z80CpuNew) {
  const hlAddress = cpu.hl;
  const tmp = cpu.rrc8(cpu.readMemory(hlAddress));
  cpu.tactPlus1WithHL();
  cpu.writeMemory(hlAddress, tmp);
}
...
// 0x10: RL B
function rlB(cpu: Z80CpuNew) {
  cpu.b = cpu.rl8(cpu.b);
}

// 0x11: RL C
function rlC(cpu: Z80CpuNew) {
  cpu.c = cpu.rl8(cpu.c);
}

// 0x12: RL D
function rlD(cpu: Z80CpuNew) {
  cpu.d = cpu.rl8(cpu.d);
}

// 0x13: RL E
function rlE(cpu: Z80CpuNew) {
  cpu.e = cpu.rl8(cpu.e);
}

// 0x14: RL H
function rlH(cpu: Z80CpuNew) {
  cpu.h = cpu.rl8(cpu.h);
}

// 0x15: RL L
function rlL(cpu: Z80CpuNew) {
  cpu.l = cpu.rl8(cpu.l);
}

// 0x16: RL (HL)
function rlHli(cpu: Z80CpuNew) {
  const hlAddress = cpu.hl;
  const tmp = cpu.rl8(cpu.readMemory(hlAddress));
  cpu.tactPlus1WithHL();
  cpu.writeMemory(hlAddress, tmp);
}
...
// 0x18: RR B
function rrB(cpu: Z80CpuNew) {
  cpu.b = cpu.rr8(cpu.b);
}

// 0x19: RR C
function rrC(cpu: Z80CpuNew) {
  cpu.c = cpu.rr8(cpu.c);
}

// 0x1a: RR D
function rrD(cpu: Z80CpuNew) {
  cpu.d = cpu.rr8(cpu.d);
}

// 0x1b: RR E
function rrE(cpu: Z80CpuNew) {
  cpu.e = cpu.rr8(cpu.e);
}

// 0x1c: RR H
function rrH(cpu: Z80CpuNew) {
  cpu.h = cpu.rr8(cpu.h);
}

// 0x1d: RR L
function rrL(cpu: Z80CpuNew) {
  cpu.l = cpu.rr8(cpu.l);
}

// 0x1e: RR (HL)
function rrHli(cpu: Z80CpuNew) {
  const hlAddress = cpu.hl;
  const tmp = cpu.rr8(cpu.readMemory(hlAddress));
  cpu.tactPlus1WithHL();
  cpu.writeMemory(hlAddress, tmp);
}
...
// 0x20: SLA B
function slaB(cpu: Z80CpuNew) {
  cpu.b = cpu.sla8(cpu.b);
}

// 0x21: SLA C
function slaC(cpu: Z80CpuNew) {
  cpu.c = cpu.sla8(cpu.c);
}

// 0x22: SLA D
function slaD(cpu: Z80CpuNew) {
  cpu.d = cpu.sla8(cpu.d);
}

// 0x23: SLA E
function slaE(cpu: Z80CpuNew) {
  cpu.e = cpu.sla8(cpu.e);
}

// 0x24: SLA H
function slaH(cpu: Z80CpuNew) {
  cpu.h = cpu.sla8(cpu.h);
}

// 0x25: SLA L
function slaL(cpu: Z80CpuNew) {
  cpu.l = cpu.sla8(cpu.l);
}

// 0x26: SLA (HL)
function slaHli(cpu: Z80CpuNew) {
  const hlAddress = cpu.hl;
  const tmp = cpu.sla8(cpu.readMemory(hlAddress));
  cpu.tactPlus1WithHL();
  cpu.writeMemory(hlAddress, tmp);
}

// 0x27: SLA A
function slaA(cpu: Z80CpuNew) {
  cpu.a = cpu.sla8(cpu.a);
}

// 0x28: SRA B
function sraB(cpu: Z80CpuNew) {
  cpu.b = cpu.sra8(cpu.b);
}

// 0x29: SRA C
function sraC(cpu: Z80CpuNew) {
  cpu.c = cpu.sra8(cpu.c);
}

// 0x2A: SRA D
function sraD(cpu: Z80CpuNew) {
  cpu.d = cpu.sra8(cpu.d);
}

// 0x2B: SRA E
function sraE(cpu: Z80CpuNew) {
  cpu.e = cpu.sra8(cpu.e);
}

// 0x2C: SRA H
function sraH(cpu: Z80CpuNew) {
  cpu.h = cpu.sra8(cpu.h);
}

// 0x2D: SRA L
function sraL(cpu: Z80CpuNew) {
  cpu.l = cpu.sra8(cpu.l);
}

// 0x2E: SRA (HL)
function sraHli(cpu: Z80CpuNew) {
  const hlAddress = cpu.hl;
  const tmp = cpu.sra8(cpu.readMemory(hlAddress));
  cpu.tactPlus1WithHL();
  cpu.writeMemory(hlAddress, tmp);
}

// 0x2F: SRA A
function sraA(cpu: Z80CpuNew) {
  cpu.a = cpu.sra8(cpu.a);
}

// 0x30: SLL B
function sllB(cpu: Z80CpuNew) {
  cpu.b = cpu.sll8(cpu.b);
}

// 0x31: SLL C
function sllC(cpu: Z80CpuNew) {
  cpu.c = cpu.sll8(cpu.c);
}

// 0x32: SLL D
function sllD(cpu: Z80CpuNew) {
  cpu.d = cpu.sll8(cpu.d);
}

// 0x33: SLL E
function sllE(cpu: Z80CpuNew) {
  cpu.e = cpu.sll8(cpu.e);
}

// 0x34: SLL H
function sllH(cpu: Z80CpuNew) {
  cpu.h = cpu.sll8(cpu.h);
}

// 0x35: SLL L
function sllL(cpu: Z80CpuNew) {
  cpu.l = cpu.sll8(cpu.l);
}

// 0x36: SLL (HL)
function sllHli(cpu: Z80CpuNew) {
  const hlAddress = cpu.hl;
  const tmp = cpu.sll8(cpu.readMemory(hlAddress));
  cpu.tactPlus1WithHL();
  cpu.writeMemory(hlAddress, tmp);
}

// 0x37: SLL A
function sllA(cpu: Z80CpuNew) {
  cpu.a = cpu.sll8(cpu.a);
}

// 0x38: SRL B
function srlB(cpu: Z80CpuNew) {
  cpu.b = cpu.srl8(cpu.b);
}

// 0x39: SRL C
function srlC(cpu: Z80CpuNew) {
  cpu.c = cpu.srl8(cpu.c);
}

// 0x3A: SRL D
function srlD(cpu: Z80CpuNew) {
  cpu.d = cpu.srl8(cpu.d);
}

// 0x3B: SRL E
function srlE(cpu: Z80CpuNew) {
  cpu.e = cpu.srl8(cpu.e);
}

// 0x3C: SRL H
function srlH(cpu: Z80CpuNew) {
  cpu.h = cpu.srl8(cpu.h);
}

// 0x3D: SRL L
function srlL(cpu: Z80CpuNew) {
  cpu.l = cpu.srl8(cpu.l);
}

// 0x3E: SRL (HL)
function srlHli(cpu: Z80CpuNew) {
  const hlAddress = cpu.hl;
  const tmp = cpu.srl8(cpu.readMemory(hlAddress));
  cpu.tactPlus1WithHL();
  cpu.writeMemory(hlAddress, tmp);
}

// 0x3F: SRL A
function srlA(cpu: Z80CpuNew) {
  cpu.a = cpu.srl8(cpu.a);
}

// 0x40: BIT 0,B
function bit0B(cpu: Z80CpuNew) {
  cpu.bit8(0, cpu.b);
}

// 0x41: BIT 0,C
function bit0C(cpu: Z80CpuNew) {
  cpu.bit8(0, cpu.c);
}

// 0x42: BIT 0,D
function bit0D(cpu: Z80CpuNew) {
  cpu.bit8(0, cpu.d);
}

// 0x43: BIT 0,E
function bit0E(cpu: Z80CpuNew) {
  cpu.bit8(0, cpu.e);
}

// 0x44: BIT 0,H
function bit0H(cpu: Z80CpuNew) {
  cpu.bit8(0, cpu.h);
}

// 0x45: BIT 0,L
function bit0L(cpu: Z80CpuNew) {
  cpu.bit8(0, cpu.l);
}

// 0x46: BIT 0,(HL)
function bit0Hli(cpu: Z80CpuNew) {
  const hlAddress = cpu.hl;
  const tmp = cpu.readMemory(hlAddress);
  cpu.tactPlus1WithHL();
  cpu.bit8W(0, tmp);
}

// 0x47: BIT 0,A
function bit0A(cpu: Z80CpuNew) {
  cpu.bit8(0, cpu.a);
}

// 0x48: BIT 1,B
function bit1B(cpu: Z80CpuNew) {
  cpu.bit8(1, cpu.b);
}

// 0x49: BIT 1,C
function bit1C(cpu: Z80CpuNew) {
  cpu.bit8(1, cpu.c);
}

// 0x4A: BIT 1,D
function bit1D(cpu: Z80CpuNew) {
  cpu.bit8(1, cpu.d);
}

// 0x4B: BIT 1,E
function bit1E(cpu: Z80CpuNew) {
  cpu.bit8(1, cpu.e);
}

// 0x4C: BIT 1,H
function bit1H(cpu: Z80CpuNew) {
  cpu.bit8(1, cpu.h);
}

// 0x4D: BIT 1,L
function bit1L(cpu: Z80CpuNew) {
  cpu.bit8(1, cpu.l);
}

// 0x4E: BIT 1,(HL)
function bit1Hli(cpu: Z80CpuNew) {
  const hlAddress = cpu.hl;
  const tmp = cpu.readMemory(hlAddress);
  cpu.tactPlus1WithHL();
  cpu.bit8W(1, tmp);
...
// 0x4F: BIT 1,A
function bit1A(cpu: Z80CpuNew) {
  cpu.bit8(1, cpu.a);
}

// 0x50: BIT 2,B
function bit2B(cpu: Z80CpuNew) {
  cpu.bit8(2, cpu.b);
}

// 0x51: BIT 2,C
function bit2C(cpu: Z80CpuNew) {
  cpu.bit8(2, cpu.c);
}

// 0x52: BIT 2,D
function bit2D(cpu: Z80CpuNew) {
  cpu.bit8(2, cpu.d);
}

// 0x53: BIT 2,E
function bit2E(cpu: Z80CpuNew) {
  cpu.bit8(2, cpu.e);
}

// 0x54: BIT 2,H
function bit2H(cpu: Z80CpuNew) {
  cpu.bit8(2, cpu.h);
}

// 0x55: BIT 2,L
function bit2L(cpu: Z80CpuNew) {
  cpu.bit8(2, cpu.l);
}

// 0x56: BIT 2,(HL)
function bit2Hli(cpu: Z80CpuNew) {
  const hlAddress = cpu.hl;
  const tmp = cpu.readMemory(hlAddress);
  cpu.tactPlus1WithHL();
  cpu.bit8W(2, tmp);
...
// 0x57: BIT 2,A
function bit2A(cpu: Z80CpuNew) {
  cpu.bit8(2, cpu.a);
}

// 0x58: BIT 3,B
function bit3B(cpu: Z80CpuNew) {
  cpu.bit8(3, cpu.b);
}

// 0x59: BIT 3,C
function bit3C(cpu: Z80CpuNew) {
  cpu.bit8(3, cpu.c);
}

// 0x5A: BIT 3,D
function bit3D(cpu: Z80CpuNew) {
  cpu.bit8(3, cpu.d);
}

// 0x5B: BIT 3,E
function bit3E(cpu: Z80CpuNew) {
  cpu.bit8(3, cpu.e);
}

// 0x5C: BIT 3,H
function bit3H(cpu: Z80CpuNew) {
  cpu.bit8(3, cpu.h);
}

// 0x5D: BIT 3,L
function bit3L(cpu: Z80CpuNew) {
  cpu.bit8(3, cpu.l);
}

// 0x5E: BIT 3,(HL)
function bit3Hli(cpu: Z80CpuNew) {
  const hlAddress = cpu.hl;
  const tmp = cpu.readMemory(hlAddress);
  cpu.tactPlus1WithHL();
  cpu.bit8W(3, tmp);
...
// 0x5F: BIT 3,A
function bit3A(cpu: Z80CpuNew) {
  cpu.bit8(3, cpu.a);
}

// 0x60: BIT 4,B
function bit4B(cpu: Z80CpuNew) {
  cpu.bit8(4, cpu.b);
}

// 0x61: BIT 4,C
function bit4C(cpu: Z80CpuNew) {
  cpu.bit8(4, cpu.c);
}

// 0x62: BIT 4,D
function bit4D(cpu: Z80CpuNew) {
  cpu.bit8(4, cpu.d);
}

// 0x63: BIT 4,E
function bit4E(cpu: Z80CpuNew) {
  cpu.bit8(4, cpu.e);
}

// 0x64: BIT 4,H
function bit4H(cpu: Z80CpuNew) {
  cpu.bit8(4, cpu.h);
}

// 0x65: BIT 4,L
function bit4L(cpu: Z80CpuNew) {
  cpu.bit8(4, cpu.l);
}

// 0x66: BIT 4,(HL)
function bit4Hli(cpu: Z80CpuNew) {
  const hlAddress = cpu.hl;
  const tmp = cpu.readMemory(hlAddress);
  cpu.tactPlus1WithHL();
  cpu.bit8W(4, tmp);
...
// 0x67: BIT 4,A
function bit4A(cpu: Z80CpuNew) {
  cpu.bit8(4, cpu.a);
}

// 0x68: BIT 5,B
function bit5B(cpu: Z80CpuNew) {
  cpu.bit8(5, cpu.b);
}

// 0x69: BIT 5,C
function bit5C(cpu: Z80CpuNew) {
  cpu.bit8(5, cpu.c);
}

// 0x6A: BIT 5,D
function bit5D(cpu: Z80CpuNew) {
  cpu.bit8(5, cpu.d);
}

// 0x6B: BIT 5,E
function bit5E(cpu: Z80CpuNew) {
  cpu.bit8(5, cpu.e);
}

// 0x6C: BIT 5,H
function bit5H(cpu: Z80CpuNew) {
  cpu.bit8(5, cpu.h);
}

// 0x6D: BIT 5,L
function bit5L(cpu: Z80CpuNew) {
  cpu.bit8(5, cpu.l);
}

// 0x6E: BIT 5,(HL)
function bit5Hli(cpu: Z80CpuNew) {
  const hlAddress = cpu.hl;
  const tmp = cpu.readMemory(hlAddress);
  cpu.tactPlus1WithHL();
  cpu.bit8W(5, tmp);
...
// 0x6F: BIT 5,A
function bit5A(cpu: Z80CpuNew) {
  cpu.bit8(5, cpu.a);
}

// 0x70: BIT 6,B
function bit6B(cpu: Z80CpuNew) {
  cpu.bit8(6, cpu.b);
}

// 0x71: BIT 6,C
function bit6C(cpu: Z80CpuNew) {
  cpu.bit8(6, cpu.c);
}

// 0x72: BIT 6,D
function bit6D(cpu: Z80CpuNew) {
  cpu.bit8(6, cpu.d);
}

// 0x73: BIT 6,E
function bit6E(cpu: Z80CpuNew) {
  cpu.bit8(6, cpu.e);
}

// 0x74: BIT 6,H
function bit6H(cpu: Z80CpuNew) {
  cpu.bit8(6, cpu.h);
}

// 0x75: BIT 6,L
function bit6L(cpu: Z80CpuNew) {
  cpu.bit8(6, cpu.l);
}

// 0x76: BIT 6,(HL)
function bit6Hli(cpu: Z80CpuNew) {
  const hlAddress = cpu.hl;
  const tmp = cpu.readMemory(hlAddress);
  cpu.tactPlus1WithHL();
  cpu.bit8W(6, tmp);
...
// 0x77: BIT 6,A
function bit6A(cpu: Z80CpuNew) {
  cpu.bit8(6, cpu.a);
}

// 0x78: BIT 7,B
function bit7B(cpu: Z80CpuNew) {
  cpu.bit8(7, cpu.b);
}

// 0x79: BIT 7,C
function bit7C(cpu: Z80CpuNew) {
  cpu.bit8(7, cpu.c);
}

// 0x7A: BIT 7,D
function bit7D(cpu: Z80CpuNew) {
  cpu.bit8(7, cpu.d);
}

// 0x7B: BIT 7,E
function bit7E(cpu: Z80CpuNew) {
  cpu.bit8(7, cpu.e);
}

// 0x7C: BIT 7,H
function bit7H(cpu: Z80CpuNew) {
  cpu.bit8(7, cpu.h);
}

// 0x7D: BIT 7,L
function bit7L(cpu: Z80CpuNew) {
  cpu.bit8(7, cpu.l);
}

// 0x7E: BIT 7,(HL)
function bit7Hli(cpu: Z80CpuNew) {
  const hlAddress = cpu.hl;
  const tmp = cpu.readMemory(hlAddress);
  cpu.tactPlus1WithHL();
  cpu.bit8W(7, tmp);
...
// 0x7F: BIT 7,A
function bit7A(cpu: Z80CpuNew) {
  cpu.bit8(7, cpu.a);
}

// 0x80: RES 0,B
function res0B(cpu: Z80CpuNew) {
  cpu.b &= 0xfe;
}

// 0x81: RES 0,C
function res0C(cpu: Z80CpuNew) {
  cpu.c &= 0xfe;
}

// 0x82: RES 0,D
function res0D(cpu: Z80CpuNew) {
  cpu.d &= 0xfe;
}

// 0x83: RES 0,E
function res0E(cpu: Z80CpuNew) {
  cpu.e &= 0xfe;
}

// 0x84: RES 0,H
function res0H(cpu: Z80CpuNew) {
  cpu.h &= 0xfe;
}

// 0x85: RES 0,L
function res0L(cpu: Z80CpuNew) {
  cpu.l &= 0xfe;
}

// 0x86: RES 0,(HL)
function res0Hli(cpu: Z80CpuNew) {
  const hlAddress = cpu.hl;
  const tmp = cpu.readMemory(hlAddress) & 0xfe;
  cpu.tactPlus1WithHL();
  cpu.writeMemory(hlAddress, tmp);
}

// 0x87: RES 0,A
function res0A(cpu: Z80CpuNew) {
  cpu.a &= 0xfe;
}

// 0x88: RES 1,B
function res1B(cpu: Z80CpuNew) {
  cpu.b &= 0xfd;
}

// 0x89: RES 1,C
function res1C(cpu: Z80CpuNew) {
  cpu.c &= 0xfd;
}

// 0x8A: RES 1,D
function res1D(cpu: Z80CpuNew) {
  cpu.d &= 0xfd;
}

// 0x8B: RES 1,E
function res1E(cpu: Z80CpuNew) {
  cpu.e &= 0xfd;
}

// 0x8C: RES 1,H
function res1H(cpu: Z80CpuNew) {
  cpu.h &= 0xfd;
}

// 0x8D: RES 1,L
function res1L(cpu: Z80CpuNew) {
  cpu.l &= 0xfd;
}

// 0x8E: RES 1,(HL)
function res1Hli(cpu: Z80CpuNew) {
...
// 0x8F: RES 1,A
function res1A(cpu: Z80CpuNew) {
  cpu.a &= 0xfd;
}

// 0x90: RES 2,B
function res2B(cpu: Z80CpuNew) {
  cpu.b &= 0xfb;
}

// 0x91: RES 2,C
function res2C(cpu: Z80CpuNew) {
  cpu.c &= 0xfb;
}

// 0x92: RES 2,D
function res2D(cpu: Z80CpuNew) {
  cpu.d &= 0xfb;
}

// 0x93: RES 2,E
function res2E(cpu: Z80CpuNew) {
  cpu.e &= 0xfb;
}

// 0x94: RES 2,H
function res2H(cpu: Z80CpuNew) {
  cpu.h &= 0xfb;
}

// 0x95: RES 2,L
function res2L(cpu: Z80CpuNew) {
  cpu.l &= 0xfb;
}

// 0x96: RES 2,(HL)
function res2Hli(cpu: Z80CpuNew) {
...
// 0x97: RES 2,A
function res2A(cpu: Z80CpuNew) {
  cpu.a &= 0xfb;
}

// 0x98: RES 3,B
function res3B(cpu: Z80CpuNew) {
  cpu.b &= 0xf7;
}

// 0x99: RES 3,C
function res3C(cpu: Z80CpuNew) {
  cpu.c &= 0xf7;
}

// 0x9A: RES 3,D
function res3D(cpu: Z80CpuNew) {
  cpu.d &= 0xf7;
}

// 0x9B: RES 3,E
function res3E(cpu: Z80CpuNew) {
  cpu.e &= 0xf7;
}

// 0x9C: RES 3,H
function res3H(cpu: Z80CpuNew) {
  cpu.h &= 0xf7;
}

// 0x9D: RES 3,L
function res3L(cpu: Z80CpuNew) {
  cpu.l &= 0xf7;
}

// 0x9E: RES 3,(HL)
function res3Hli(cpu: Z80CpuNew) {
...
// 0x9F: RES 3,A
function res3A(cpu: Z80CpuNew) {
  cpu.a &= 0xf7;
}

// 0xA0: RES 4,B
function res4B(cpu: Z80CpuNew) {
  cpu.b &= 0xef;
}

// 0xA1: RES 4,C
function res4C(cpu: Z80CpuNew) {
  cpu.c &= 0xef;
}

// 0xA2: RES 4,D
function res4D(cpu: Z80CpuNew) {
  cpu.d &= 0xef;
}

// 0xA3: RES 4,E
function res4E(cpu: Z80CpuNew) {
  cpu.e &= 0xef;
}

// 0xA4: RES 4,H
function res4H(cpu: Z80CpuNew) {
  cpu.h &= 0xef;
}

// 0xA5: RES 4,L
function res4L(cpu: Z80CpuNew) {
  cpu.l &= 0xef;
}

// 0xA6: RES 4,(HL)
function res4Hli(cpu: Z80CpuNew) {
...
// 0xA7: RES 4,A
function res4A(cpu: Z80CpuNew) {
  cpu.a &= 0xef;
}

// 0xA8: RES 5,B
function res5B(cpu: Z80CpuNew) {
  cpu.b &= 0xdf;
}

// 0xA9: RES 5,C
function res5C(cpu: Z80CpuNew) {
  cpu.c &= 0xdf;
}

// 0xAA: RES 5,D
function res5D(cpu: Z80CpuNew) {
  cpu.d &= 0xdf;
}

// 0xAB: RES 5,E
function res5E(cpu: Z80CpuNew) {
  cpu.e &= 0xdf;
}

// 0xAC: RES 5,H
function res5H(cpu: Z80CpuNew) {
  cpu.h &= 0xdf;
}

// 0xAD: RES 5,L
function res5L(cpu: Z80CpuNew) {
  cpu.l &= 0xdf;
}

// 0xAE: RES 5,(HL)
function res5Hli(cpu: Z80CpuNew) {
...
// 0xAF: RES 5,A
function res5A(cpu: Z80CpuNew) {
  cpu.a &= 0xdf;
}

// 0xB0: RES 6,B
function res6B(cpu: Z80CpuNew) {
  cpu.b &= 0xbf;
}

// 0xB1: RES 6,C
function res6C(cpu: Z80CpuNew) {
  cpu.c &= 0xbf;
}

// 0xB2: RES 6,D
function res6D(cpu: Z80CpuNew) {
  cpu.d &= 0xbf;
}

// 0xB3: RES 6,E
function res6E(cpu: Z80CpuNew) {
  cpu.e &= 0xbf;
}

// 0xB4: RES 6,H
function res6H(cpu: Z80CpuNew) {
  cpu.h &= 0xbf;
}

// 0xB5: RES 6,L
function res6L(cpu: Z80CpuNew) {
  cpu.l &= 0xbf;
}

// 0xB6: RES 6,(HL)
function res6Hli(cpu: Z80CpuNew) {
...
// 0xB7: RES 6,A
function res6A(cpu: Z80CpuNew) {
  cpu.a &= 0xbf;
}

// 0xB8: RES 7,B
function res7B(cpu: Z80CpuNew) {
  cpu.b &= 0x7f;
}

// 0xB9: RES 7,C
function res7C(cpu: Z80CpuNew) {
  cpu.c &= 0x7f;
}

// 0xBA: RES 7,D
function res7D(cpu: Z80CpuNew) {
  cpu.d &= 0x7f;
}

// 0xBB: RES 7,E
function res7E(cpu: Z80CpuNew) {
  cpu.e &= 0x7f;
}

// 0xBC: RES 7,H
function res7H(cpu: Z80CpuNew) {
  cpu.h &= 0x7f;
}

// 0xBD: RES 7,L
function res7L(cpu: Z80CpuNew) {
  cpu.l &= 0x7f;
}

// 0xBE: RES 7,(HL)
function res7Hli(cpu: Z80CpuNew) {
...
// 0xBF: RES 7,A
function res7A(cpu: Z80CpuNew) {
  cpu.a &= 0x7f;
}

// 0xC0: SET 0,B
function set0B(cpu: Z80CpuNew) {
  cpu.b |= 0x1;
}

// 0xC1: SET 0,C
function set0C(cpu: Z80CpuNew) {
  cpu.c |= 0x1;
}

// 0xC2: SET 0,D
function set0D(cpu: Z80CpuNew) {
  cpu.d |= 0x1;
}

// 0xC3: SET 0,E
function set0E(cpu: Z80CpuNew) {
  cpu.e |= 0x1;
}

// 0xC4: SET 0,H
function set0H(cpu: Z80CpuNew) {
  cpu.h |= 0x1;
}

// 0xC5: SET 0,L
function set0L(cpu: Z80CpuNew) {
  cpu.l |= 0x1;
}

// 0xC6: SET 0,(HL)
function set0Hli(cpu: Z80CpuNew) {
  const hlAddress = cpu.hl;
  const tmp = cpu.readMemory(hlAddress) | 0x1;
  cpu.tactPlus1WithHL();
  cpu.writeMemory(hlAddress, tmp);
}

// 0xC7: SET 0,A
function set0A(cpu: Z80CpuNew) {
  cpu.a |= 0x1;
}

// 0xC8: SET 1,B
function set1B(cpu: Z80CpuNew) {
  cpu.b |= 0x2;
}

// 0xC9: SET 1,C
function set1C(cpu: Z80CpuNew) {
  cpu.c |= 0x2;
}

// 0xCA: SET 1,D
function set1D(cpu: Z80CpuNew) {
  cpu.d |= 0x2;
}

// 0xCB: SET 1,E
function set1E(cpu: Z80CpuNew) {
  cpu.e |= 0x2;
}

// 0xCC: SET 1,H
function set1H(cpu: Z80CpuNew) {
  cpu.h |= 0x2;
}

// 0xCD: SET 1,L
function set1L(cpu: Z80CpuNew) {
  cpu.l |= 0x2;
}

// 0xCE: SET 1,(HL)
function set1Hli(cpu: Z80CpuNew) {
  const hlAddress = cpu.hl;
  const tmp = cpu.readMemory(hlAddress) | 0x2;
  cpu.tactPlus1WithHL();
  cpu.writeMemory(hlAddress, tmp);
}

// 0xCF: SET 1,A
function set1A(cpu: Z80CpuNew) {
  cpu.a |= 0x2;
}

// 0xD0: SET 2,B
function set2B(cpu: Z80CpuNew) {
  cpu.b |= 0x4;
}

// 0xD1: SET 2,C
function set2C(cpu: Z80CpuNew) {
  cpu.c |= 0x4;
}

// 0xD2: SET 2,D
function set2D(cpu: Z80CpuNew) {
  cpu.d |= 0x4;
}

// 0xD3: SET 2,E
function set2E(cpu: Z80CpuNew) {
  cpu.e |= 0x4;
}

// 0xD4: SET 2,H
function set2H(cpu: Z80CpuNew) {
  cpu.h |= 0x4;
}

// 0xD5: SET 2,L
function set2L(cpu: Z80CpuNew) {
  cpu.l |= 0x4;
}

// 0xD6: SET 2,(HL)
function set2Hli(cpu: Z80CpuNew) {
...
// 0xD7: SET 2,A
function set2A(cpu: Z80CpuNew) {
  cpu.a |= 0x4;
}

// 0xD8: SET 3,B
function set3B(cpu: Z80CpuNew) {
  cpu.b |= 0x8;
}

// 0xD9: SET 3,C
function set3C(cpu: Z80CpuNew) {
  cpu.c |= 0x8;
}

// 0xDA: SET 3,D
function set3D(cpu: Z80CpuNew) {
  cpu.d |= 0x8;
}

// 0xDB: SET 3,E
function set3E(cpu: Z80CpuNew) {
  cpu.e |= 0x8;
}

// 0xDC: SET 3,H
function set3H(cpu: Z80CpuNew) {
  cpu.h |= 0x8;
}

// 0xDD: SET 3,L
function set3L(cpu: Z80CpuNew) {
  cpu.l |= 0x8;
}

// 0xDE: SET 3,(HL)
function set3Hli(cpu: Z80CpuNew) {
...
// 0xDF: SET 3,A
function set3A(cpu: Z80CpuNew) {
  cpu.a |= 0x8;
}

// 0xE0: SET 4,B
function set4B(cpu: Z80CpuNew) {
  cpu.b |= 0x10;
}

// 0xE1: SET 4,C
function set4C(cpu: Z80CpuNew) {
  cpu.c |= 0x10;
}

// 0xE2: SET 4,D
function set4D(cpu: Z80CpuNew) {
  cpu.d |= 0x10;
}

// 0xE3: SET 4,E
function set4E(cpu: Z80CpuNew) {
  cpu.e |= 0x10;
}

// 0xE4: SET 4,H
function set4H(cpu: Z80CpuNew) {
  cpu.h |= 0x10;
}

// 0xE5: SET 4,L
function set4L(cpu: Z80CpuNew) {
  cpu.l |= 0x10;
}

// 0xE6: SET 4,(HL)
function set4Hli(cpu: Z80CpuNew) {
...
// 0xE7: SET 4,A
function set4A(cpu: Z80CpuNew) {
  cpu.a |= 0x10;
}

// 0xE8: SET 5,B
function set5B(cpu: Z80CpuNew) {
  cpu.b |= 0x20;
}

// 0xE9: SET 5,C
function set5C(cpu: Z80CpuNew) {
  cpu.c |= 0x20;
}

// 0xEA: SET 5,D
function set5D(cpu: Z80CpuNew) {
  cpu.d |= 0x20;
}

// 0xEB: SET 5,E
function set5E(cpu: Z80CpuNew) {
  cpu.e |= 0x20;
}

// 0xEC: SET 5,H
function set5H(cpu: Z80CpuNew) {
  cpu.h |= 0x20;
}

// 0xED: SET 5,L
function set5L(cpu: Z80CpuNew) {
  cpu.l |= 0x20;
}

// 0xEE: SET 5,(HL)
function set5Hli(cpu: Z80CpuNew) {
...
// 0xEF: SET 5,A
function set5A(cpu: Z80CpuNew) {
  cpu.a |= 0x20;
}

// 0xF0: SET 6,B
function set6B(cpu: Z80CpuNew) {
  cpu.b |= 0x40;
}

// 0xF1: SET 6,C
function set6C(cpu: Z80CpuNew) {
  cpu.c |= 0x40;
}

// 0xF2: SET 6,D
function set6D(cpu: Z80CpuNew) {
  cpu.d |= 0x40;
}

// 0xF3: SET 6,E
function set6E(cpu: Z80CpuNew) {
  cpu.e |= 0x40;
}

// 0xF4: SET 6,H
function set6H(cpu: Z80CpuNew) {
  cpu.h |= 0x40;
}

// 0xF5: SET 6,L
function set6L(cpu: Z80CpuNew) {
  cpu.l |= 0x40;
}

// 0xF6: SET 6,(HL)
function set6Hli(cpu: Z80CpuNew) {
...
// 0xF7: SET 6,A
function set6A(cpu: Z80CpuNew) {
  cpu.a |= 0x40;
}

// 0xF8: SET 7,B
function set7B(cpu: Z80CpuNew) {
  cpu.b |= 0x80;
}

// 0xF9: SET 7,C
function set7C(cpu: Z80CpuNew) {
  cpu.c |= 0x80;
}

// 0xFA: SET 7,D
function set7D(cpu: Z80CpuNew) {
  cpu.d |= 0x80;
}

// 0xFB: SET 7,E
function set7E(cpu: Z80CpuNew) {
  cpu.e |= 0x80;
}

// 0xFC: SET 7,H
function set7H(cpu: Z80CpuNew) {
  cpu.h |= 0x80;
}

// 0xFD: SET 7,L
function set7L(cpu: Z80CpuNew) {
  cpu.l |= 0x80;
}

// 0xFE: SET 7,(HL)
function set7Hli(cpu: Z80CpuNew) {
...
// 0x2B: DEC IX
function decX(cpu: Z80CpuNew) {
  cpu.indexReg--;
  cpu.tactPlus2WithIR();
}

// 0x2C: INC XL
function incXl(cpu: Z80CpuNew) {
  cpu.f = incFlags[cpu.indexL++] | cpu.flagCValue;
}

// 0x2d: DEC XL
function decXl(cpu: Z80CpuNew) {
  cpu.f = decFlags[cpu.indexL--] | cpu.flagCValue;
}

// 0x2e: LD XL,n
function ldXlN(cpu: Z80CpuNew) {
  cpu.indexL = cpu.fetchCodeByte();
}

// 0x34: INC (IX+d)
function incXi(cpu: Z80CpuNew) {
...
// 0x39: ADD IX,SP
function addXSp(cpu: Z80CpuNew) {
  cpu.tactPlus7WithIR();
  cpu.indexReg = cpu.add16(cpu.indexReg, cpu.sp);
}

// 0x44: LD B,XH
function ldBXh(cpu: Z80CpuNew) {
  cpu.b = cpu.indexH;
}

// 0x45: LD B,XL
function ldBXl(cpu: Z80CpuNew) {
  cpu.b = cpu.indexL;
}

// 0x46: LD B,(IX+d)
function ldBXi(cpu: Z80CpuNew) {
  const dist = cpu.readMemory(cpu.pc);
  cpu.tactPlus5WithPC();
  cpu.pc++;
  cpu.wz = cpu.indexReg + sbyte(dist);
...
// 0x4C: LD C,XH
function ldCXh(cpu: Z80CpuNew) {
  cpu.c = cpu.indexH;
}

// 0x4D: LD C,XL
function ldCXl(cpu: Z80CpuNew) {
  cpu.c = cpu.indexL;
}

// 0x4E: LD C,(IX+d)
function ldCXi(cpu: Z80CpuNew) {
  const dist = cpu.readMemory(cpu.pc);
  cpu.tactPlus5WithPC();
  cpu.pc++;
  cpu.wz = cpu.indexReg + sbyte(dist);
...
// 0x54: LD D,XH
function ldDXh(cpu: Z80CpuNew) {
  cpu.d = cpu.indexH;
}

// 0x55: LD D,XL
function ldDXl(cpu: Z80CpuNew) {
  cpu.d = cpu.indexL;
}

// 0x56: LD D,(IX+d)
function ldDXi(cpu: Z80CpuNew) {
  const dist = cpu.readMemory(cpu.pc);
  cpu.tactPlus5WithPC();
  cpu.pc++;
  cpu.wz = cpu.indexReg + sbyte(dist);
...
// 0x5C: LD E,XH
function ldEXh(cpu: Z80CpuNew) {
  cpu.e = cpu.indexH;
}

// 0x5D: LD E,XL
function ldEXl(cpu: Z80CpuNew) {
  cpu.e = cpu.indexL;
}

// 0x5E: LD E,(IX+d)
function ldEXi(cpu: Z80CpuNew) {
  const dist = cpu.readMemory(cpu.pc);
  cpu.tactPlus5WithPC();
  cpu.pc++;
  cpu.wz = cpu.indexReg + sbyte(dist);
...
// 0x60: LD XH,B
function ldXhB(cpu: Z80CpuNew) {
  cpu.indexH = cpu.b;
}

// 0x61: LD XH,C
function ldXhC(cpu: Z80CpuNew) {
  cpu.indexH = cpu.c;
}

// 0x62: LD XH,D
function ldXhD(cpu: Z80CpuNew) {
  cpu.indexH = cpu.d;
}

// 0x63: LD XH,E
function ldXhE(cpu: Z80CpuNew) {
  cpu.indexH = cpu.e;
}

// 0x65: LD XH,XL
function ldXhXl(cpu: Z80CpuNew) {
  cpu.indexH = cpu.indexL;
}

// 0x66: LD H,(IX+d)
function ldHXi(cpu: Z80CpuNew) {
  const dist = cpu.readMemory(cpu.pc);
  cpu.tactPlus5WithPC();
  cpu.pc++;
  cpu.wz = cpu.indexReg + sbyte(dist);
...
// 0x67: LD XH,A
function ldXhA(cpu: Z80CpuNew) {
  cpu.indexH = cpu.a;
}

// 0x68: LD XL,B
function ldXlB(cpu: Z80CpuNew) {
  cpu.indexL = cpu.b;
}

// 0x69: LD XL,C
function ldXlC(cpu: Z80CpuNew) {
  cpu.indexL = cpu.c;
}

// 0x6A: LD XL,D
function ldXlD(cpu: Z80CpuNew) {
  cpu.indexL = cpu.d;
}

// 0x6B: LD XL,E
function ldXlE(cpu: Z80CpuNew) {
  cpu.indexL = cpu.e;
}

// 0x6C: LD XL,XH
function ldXlXh(cpu: Z80CpuNew) {
  cpu.indexL = cpu.indexH;
}

// 0x6E: LD L,(IX+d)
function ldLXi(cpu: Z80CpuNew) {
  const dist = cpu.readMemory(cpu.pc);
  cpu.tactPlus5WithPC();
  cpu.pc++;
  cpu.wz = cpu.indexReg + sbyte(dist);
...
// 0x6F: LD XL,A
function ldXlA(cpu: Z80CpuNew) {
  cpu.indexL = cpu.a;
}

// 0x70: LD (IX+d),B
function ldXiB(cpu: Z80CpuNew) {
  const dist = cpu.readMemory(cpu.pc);
  cpu.tactPlus5WithPC();
  cpu.pc++;
  cpu.wz = cpu.indexReg + sbyte(dist);
...
// 0x7C: LD A,XH
function ldAXh(cpu: Z80CpuNew) {
  cpu.a = cpu.indexH;
}

// 0x7D: LD A,XL
function ldAXl(cpu: Z80CpuNew) {
  cpu.a = cpu.indexL;
}

// 0x7E: LD A,(IX+d)
function ldAXi(cpu: Z80CpuNew) {
  const dist = cpu.readMemory(cpu.pc);
  cpu.tactPlus5WithPC();
  cpu.pc++;
  cpu.wz = cpu.indexReg + sbyte(dist);
...
// 0x84: ADD A,XH
function addAXh(cpu: Z80CpuNew) {
  cpu.add8(cpu.indexH);
}

// 0x85: ADD A,XL
function addAXl(cpu: Z80CpuNew) {
  cpu.add8(cpu.indexL);
}

// 0x86: ADD A,(IX+d)
function addAXi(cpu: Z80CpuNew) {
  const dist = cpu.readMemory(cpu.pc);
  cpu.tactPlus5WithPC();
  cpu.pc++;
  cpu.wz = cpu.indexReg + sbyte(dist);
...
// 0x8C: ADC A,XH
function adcAXh(cpu: Z80CpuNew) {
  cpu.adc8(cpu.indexH);
}

// 0x8D: ADC A,XL
function adcAXl(cpu: Z80CpuNew) {
  cpu.adc8(cpu.indexL);
}

// 0x8E: ADC A,(IX+d)
function adcAXi(cpu: Z80CpuNew) {
  const dist = cpu.readMemory(cpu.pc);
  cpu.tactPlus5WithPC();
  cpu.pc++;
  cpu.wz = cpu.indexReg + sbyte(dist);
...
// 0x94: SUB A,XH
function subAXh(cpu: Z80CpuNew) {
  cpu.sub8(cpu.indexH);
}

// 0x95: ADC A,XL
function subAXl(cpu: Z80CpuNew) {
  cpu.sub8(cpu.indexL);
}

// 0x96: SUB A,(IX+d)
function subAXi(cpu: Z80CpuNew) {
  const dist = cpu.readMemory(cpu.pc);
  cpu.tactPlus5WithPC();
  cpu.pc++;
  cpu.wz = cpu.indexReg + sbyte(dist);
...
// 0x9C: SBC A,XH
function sbcAXh(cpu: Z80CpuNew) {
  cpu.sbc8(cpu.indexH);
}

// 0x9D: SBC A,XL
function sbcAXl(cpu: Z80CpuNew) {
  cpu.sbc8(cpu.indexL);
}

// 0x9E: SBC A,(IX+d)
function sbcAXi(cpu: Z80CpuNew) {
  const dist = cpu.readMemory(cpu.pc);
  cpu.tactPlus5WithPC();
  cpu.pc++;
  cpu.wz = cpu.indexReg + sbyte(dist);
...
// 0xA4: AND A,XH
function andAXh(cpu: Z80CpuNew) {
  cpu.and8(cpu.indexH);
}

// 0xA5: AND A,XL
function andAXl(cpu: Z80CpuNew) {
  cpu.and8(cpu.indexL);
}

// 0xA6: AND A,(IX+d)
function andAXi(cpu: Z80CpuNew) {
  const dist = cpu.readMemory(cpu.pc);
  cpu.tactPlus5WithPC();
  cpu.pc++;
  cpu.wz = cpu.indexReg + sbyte(dist);
...
// 0xAC: XOR A,XH
function xorAXh(cpu: Z80CpuNew) {
  cpu.xor8(cpu.indexH);
}

// 0xAD: XOR A,XL
function xorAXl(cpu: Z80CpuNew) {
  cpu.xor8(cpu.indexL);
}

// 0xAE: XOR A,(IX+d)
function xorAXi(cpu: Z80CpuNew) {
  const dist = cpu.readMemory(cpu.pc);
  cpu.tactPlus5WithPC();
  cpu.pc++;
  cpu.wz = cpu.indexReg + sbyte(dist);
...
// 0xB4: OR A,XH
function orAXh(cpu: Z80CpuNew) {
  cpu.or8(cpu.indexH);
}

// 0xB5: OR A,XL
function orAXl(cpu: Z80CpuNew) {
  cpu.or8(cpu.indexL);
}

// 0xB6: OR A,(IX+d)
function orAXi(cpu: Z80CpuNew) {
  const dist = cpu.readMemory(cpu.pc);
  cpu.tactPlus5WithPC();
  cpu.pc++;
  cpu.wz = cpu.indexReg + sbyte(dist);
...
// 0xBC: CP XH
function cpAXh(cpu: Z80CpuNew) {
  cpu.cp8(cpu.indexH);
}

// 0xBD: CP XL
function cpAXl(cpu: Z80CpuNew) {
  cpu.cp8(cpu.indexL);
}

// 0xBE: CP (IX+d)
function cpAXi(cpu: Z80CpuNew) {
  const dist = cpu.readMemory(cpu.pc);
  cpu.tactPlus5WithPC();
  cpu.pc++;
  cpu.wz = cpu.indexReg + sbyte(dist);
...
// 0xe9: JP (IX)
function jpX(cpu: Z80CpuNew) {
  cpu.pc = cpu.indexReg;
}

// 0xf9: LD SP,IX
function ldSpX(cpu: Z80CpuNew) {
  cpu.tactPlus2WithIR();
  cpu.sp = cpu.indexReg;
}
...
// 0x41: OUT (C),B
function outCB(cpu: Z80CpuNew) {
  cpu.writePort(cpu.bc, cpu.b);
  cpu.wz = cpu.bc + 1;
}

// 0x42: SBC HL,BC
function sbcHlBc(cpu: Z80CpuNew) {
  cpu.tactPlus7WithIR();
  cpu.sbc16(cpu.bc);
}

// 0x43: LD (nn),BC
function ldNniBc(cpu: Z80CpuNew) {
  cpu.store16(cpu.c, cpu.b);
}

// 0x44: NEG
function neg(cpu: Z80CpuNew) {
  var tmp = cpu.a;
  cpu.a = 0;
  cpu.sub8(tmp);
}
...
// 0x45: RETN
function retn(cpu: Z80CpuNew) {
  cpu.iff1 = cpu.iff2;
  ret(cpu);
}

// 0x46: IM 0
function im0(cpu: Z80CpuNew) {
  cpu.interruptMode = 0;
}

// 0x47: LD I,A
function ldIA(cpu: Z80CpuNew) {
  cpu.tactPlus1WithIR();
  cpu.i = cpu.a;
}
...
// 0x49: OUT (C),C
function outCC(cpu: Z80CpuNew) {
  cpu.writePort(cpu.bc, cpu.c);
  cpu.wz = cpu.bc + 1;
}

// 0x4A: ADC HL,BC
function adcHlBc(cpu: Z80CpuNew) {
  cpu.tactPlus7WithIR();
  cpu.adc16(cpu.bc);
}

// 0x4B: LD BC,(nn)
function ldBcNni(cpu: Z80CpuNew) {
...
// 0x4F: LD R,A
function ldRA(cpu: Z80CpuNew) {
  cpu.tactPlus1WithIR();
  cpu.r = cpu.a;
}

// 0x50: IN D,(C)
function inDC(cpu: Z80CpuNew) {
  cpu.wz = cpu.bc + 1;
...
// 0x51: OUT (C),D
function outCD(cpu: Z80CpuNew) {
  cpu.writePort(cpu.bc, cpu.d);
  cpu.wz = cpu.bc + 1;
}

// 0x52: SBC HL,DE
function sbcHlDe(cpu: Z80CpuNew) {
  cpu.tactPlus7WithIR();
  cpu.sbc16(cpu.de);
}

// 0x53: LD (nn),DE
function ldNniDe(cpu: Z80CpuNew) {
  cpu.store16(cpu.e, cpu.d);
}

// 0x56: IM 1
function im1(cpu: Z80CpuNew) {
  cpu.interruptMode = 1;
}

// 0x57: LD A,I
function ldAI(cpu: Z80CpuNew) {
  cpu.tactPlus1WithIR();
  cpu.a = cpu.i;
...
// 0x59: OUT (C),E
function outCE(cpu: Z80CpuNew) {
  cpu.writePort(cpu.bc, cpu.e);
  cpu.wz = cpu.bc + 1;
}

// 0x5A: ADC HL,DE
function adcHlDe(cpu: Z80CpuNew) {
  cpu.tactPlus7WithIR();
  cpu.adc16(cpu.de);
}

// 0x5B: LD DE,(nn)
function ldDeNni(cpu: Z80CpuNew) {
  let tmp = cpu.fetchCodeByte();
  tmp += cpu.fetchCodeByte() << 8;
...
// 0x5E: IM 2
function im2(cpu: Z80CpuNew) {
  cpu.interruptMode = 2;
}

// 0x5F: LD A,R
function ldAR(cpu: Z80CpuNew) {
  cpu.tactPlus1WithIR();
  cpu.a = cpu.r;
  cpu.f = cpu.flagCValue | sz53Table[cpu.a] | (cpu.iff2 ? FlagsSetMask.PV : 0);
}
...
// 0x61: OUT (C),H
function outCH(cpu: Z80CpuNew) {
  cpu.writePort(cpu.bc, cpu.h);
  cpu.wz = cpu.bc + 1;
}

// 0x62: SBC HL,HL
function sbcHlHl(cpu: Z80CpuNew) {
  cpu.tactPlus7WithIR();
  cpu.sbc16(cpu.hl);
}
...
// 0x69: OUT (C),L
function outCL(cpu: Z80CpuNew) {
  cpu.writePort(cpu.bc, cpu.l);
  cpu.wz = cpu.bc + 1;
}

// 0x6A: ADC HL,HL
function adcHlHl(cpu: Z80CpuNew) {
  cpu.tactPlus7WithIR();
  cpu.adc16(cpu.hl);
}
...
// 0x71: OUT (C),0
function outC0(cpu: Z80CpuNew) {
  cpu.writePort(cpu.bc, 0);
  cpu.wz = cpu.bc + 1;
}

// 0x72: SBC HL,SP
function sbcHlSp(cpu: Z80CpuNew) {
  cpu.tactPlus7WithIR();
  cpu.sbc16(cpu.sp);
}

// 0x73: LD (nn),SP
function ldNniSp(cpu: Z80CpuNew) {
  cpu.store16(cpu.sp & 0xff, cpu.sp >>> 8);
}

// 0x78: IN A,(C)
function inAC(cpu: Z80CpuNew) {
  cpu.wz = cpu.bc + 1;
...
// 0x79: OUT (C),A
function outCA(cpu: Z80CpuNew) {
  cpu.writePort(cpu.bc, cpu.a);
  cpu.wz = cpu.bc + 1;
}

// 0x7A: ADC HL,SP
function adcHlSp(cpu: Z80CpuNew) {
  cpu.tactPlus7WithIR();
  cpu.adc16(cpu.sp);
}

// 0x7B: LD SP,(nn)
function ldSpNni(cpu: Z80CpuNew) {
  let tmp = cpu.fetchCodeByte();
  tmp += cpu.fetchCodeByte() << 8;
