...
async function defaultReadV2Artifact(artifactName: string): Promise<ArrayBuffer> {
  ...
  if (!artifactUrl.toString().endsWith(`/${artifactName}`)) {
    throw new Error(
      `ZX Spectrum 48K WASM v2 artifact '${artifactName}' was not found among the packaged renderer assets ` +
      `(resolved to '${artifactUrl.toString()}' instead). This build is missing its compiled WASM binary ` +
      `- verify 'npm run build:sp48-wasm' produced a non-empty file before packaging.`
    );
  }
  ...
  try {
    ...
    throw new Error(
      `Cannot load ZX Spectrum 48K WASM v2 artifact '${artifactName}' from ${artifactUrl.toString()}: ` +
