...
export type Sp48KeyEventDetail = {
  key: number;
  down: boolean;
  source: "physical" | "virtual";
};

export const Sp48KeyCode: Record<Sp48KeyName, number> = {
  CShift: 0,
  Z: 1,
  X: 2,
  C: 3,
...
export function dispatchSp48KeyStatus(key: number, down: boolean, source: Sp48KeyEventDetail["source"]): void {
  window.dispatchEvent(
    new CustomEvent<Sp48KeyEventDetail>(SP48_KEY_EVENT, {
      detail: { key, down, source }
    })
  );
}
