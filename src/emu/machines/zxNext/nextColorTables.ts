/*
 * The ZX Spectrum Next's colour tables: the RGB333 value of every 9-bit palette entry as a 24-bit
 * RGB code and as the BGRA word the screen buffer holds, and the default global transparency colour.
 *
 * Neutral data: shared by both cores' renderers (the TypeScript composed screen, the tests that check
 * the WASM picture) and the IDE's palette helpers (`palette.ts`).
 */
