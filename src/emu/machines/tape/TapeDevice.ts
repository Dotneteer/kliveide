...
export class TapeDevice implements ITapeDevice {
    ...
    updateTapeMode(): void {
        ...
        if ((this.machine.isSpectrum48RomSelected && this.machine.pc === 0x0008) || 
    ...
    getTapeEarBit(): boolean {
        ...
        if (this._playPhase === PlayPhase.Pilot || this._playPhase === PlayPhase.Sync) {
            ...
            this._tapeBitPulseLen = (block.data[this._dataIndex] & this._tapeBitMask) != 0
        ...
        if (this._playPhase === PlayPhase.Data) {
            ...
            this._tapeTermEndPos = this._tapeBitStartPos + 2 * this._tapeBitPulseLen + block.endSyncPulseLenght;
            return false;
        }
    ...
    nextTapeBlock(): void {
        ...
        this._tapePilotEndPos = block.pilotPulseLength * ((block.data[0] & 0x80)
            ? DATA_PILOT_COUNT
            : HEADER_PILOT_COUNT);
