        ...
        try
        {
            ...
            while (this._reader.position !== this._reader.length) {
                ...
                try {
                    ...
                    if (!block) {
                        throw Error(`Cannot instantiate TZX data block with type ${blockType}.`);
                    }
                    ...
                    throw new Error(`Cannot read TZX data block ${blockType}. (${err.message})`);
                }
            }
...
export class TzxHeader extends TzxBlockBase
{
    ...
    get isValid(): boolean {
        return this.signature.length === tzxSignature.length
        && this.eot === 0x1A
        && this.majorVersion === 1;
    }
}
