...
const PANE_ID_EMU = "emu";

const settingDefinitions: Setting[] = [
  {
    id: SETTING_EMU_NOTE,
    title: "Emulator Note",
    description: "Demo note persisted with the emulator settings.",
    type: "string",
    defaultValue: "",
    persist: true,
    saveWithIde: true,
  ...
  },
  {
    id: SETTING_IDE_NOTE,
    title: "IDE Note",
    description: "Demo note persisted with the IDE settings.",
    type: "string",
    defaultValue: "",
    persist: true,
    saveWithIde: true,
...
export function getDefaultGlobalSettings(): Record<string, unknown> {
  return settingDefinitions.reduce(
    (acc, setting) => set(acc, setting.id, setting.defaultValue),
