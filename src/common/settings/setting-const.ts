export const SETTING_EMU_NOTE = "emuNote";
export const SETTING_IDE_NOTE = "ideNote";

export const SETTING_EMU_SHOW_TOOLBAR = "emuViewOptions.showToolbar";
export const SETTING_EMU_SHOW_STATUS_BAR = "emuViewOptions.showStatusBar";
export const SETTING_EMU_SHOW_KEYBOARD = "emuViewOptions.showKeyboard";
export const SETTING_EMU_KEYBOARD_SPLITTER_POSITION = "emuViewOptions.keyboardSplitterPosition";
export const SETTING_EMU_SHOW_INSTANT_SCREEN = "emuViewOptions.showInstantScreen";
export const SETTING_EMU_SCANLINE_EFFECT = "emuOptions.scanlineEffect";
export const SETTING_EMU_FAST_LOAD = "emuOptions.fastLoad";
export const SETTING_EMU_STAY_ON_TOP = "emuOptions.stayOnTop";
export const SETTING_EMU_MACHINE_TYPE = "emuOptions.machineType";
export const SETTING_EMU_KEYBOARD_LAYOUT = "emuOptions.keyboardLayout";
export const SETTING_EMU_KEYBOARD_HEIGHT = "emuOptions.keyboardPanelHeight";
export const SETTING_IDE_SHOW_TOOLBAR = "ideViewOptions.showToolbar";
export const SETTING_IDE_SHOW_STATUS_BAR = "ideViewOptions.showStatusBar";
export const SETTING_IDE_SHOW_SIDEBAR = "ideViewOptions.showSidebar";
export const SETTING_IDE_SHOW_PRIMARY_SIDEBAR = "ideViewOptions.showPrimarySideBar";
export const SETTING_IDE_SHOW_SECONDARY_SIDEBAR = "ideViewOptions.showSecondarySideBar";
export const SETTING_IDE_SIDEBAR_TO_RIGHT = "ideViewOptions.sideBarToRight";
export const SETTING_IDE_SHOW_TOOLS = "ideViewOptions.showTools";
export const SETTING_IDE_TOOLS_ON_TOP = "ideViewOptions.toolPanelsOnTop";
export const SETTING_IDE_MAXIMIZE_TOOLS = "ideViewOptions.maximizeTools";
export const SETTING_IDE_SYNC_BREAKPOINTS = "ideViewOptions.syncBreakpoints";
export const SETTING_IDE_SIDEBAR_WIDTH = "ideViewOptions.sideBarWidth";
export const SETTING_IDE_PRIMARY_SIDEBAR_WIDTH = "ideViewOptions.primarySideBarWidth";
export const SETTING_IDE_SECONDARY_SIDEBAR_WIDTH = "ideViewOptions.secondarySideBarWidth";
export const SETTING_IDE_TOOLPANEL_HEIGHT = "ideViewOptions.toolPanelHeight";
export const SETTING_IDE_ACTIVE_TOOL = "ideViewOptions.activeTool";
export const SETTING_IDE_ACTIVE_OUTPUT_PANE = "ideViewOptions.activeOutputPane";
export const SETTING_IDE_PANEL_LAYOUT = "ideViewOptions.panelLayout";
export const SETTING_IDE_OPEN_LAST_PROJECT = "ideBehavior.openLastProject";
export const SETTING_IDE_CLOSE_EMU = "ideBehavior.closeEmuWithIde";
export const SETTING_EDITOR_FONT_SIZE = "editorOptions.fontSize";
export const SETTING_EDITOR_AUTOCOMPLETE = "editorOptions.autoComplete";
export const SETTING_EDITOR_TABSIZE = "editorOptions.tabSize";
export const SETTING_EDITOR_INSERT_SPACES = "editorOptions.insertSpaces";
export const SETTING_EDITOR_RENDER_WHITESPACE = "editorOptions.renderWhitespaces";
export const SETTING_EDITOR_DETECT_INDENTATION = "editorOptions.detectIndentation";
export const SETTING_EDITOR_SELECTION_HIGHLIGHT = "editorOptions.enableSelectionHighlight";
export const SETTING_EDITOR_OCCURRENCES_HIGHLIGHT = "editorOptions.enableOccurrencesHighlight";
export const SETTING_EDITOR_QUICK_SUGGESTION_DELAY = "editorOptions.quickSuggestionDelay";
export const SETTING_EDITOR_ALLOW_BACKGROUND_COMPILE = "editorOptions.allowBackgroundCompile";
