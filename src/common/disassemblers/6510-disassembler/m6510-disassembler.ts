...
export class M6510Disassembler {
  ...
  private async generateByteArray(section: MemorySection): Promise<void> {
    ...
    for (let i = 0; i < length; i += 8) {
      ...
      for (let j = 0; j < 8; j++) {
        ...
        bytes.push(this._decimalMode ? toDecimal3(byte) : `$${toHexa2(byte)}`);
      }
  ...
  private async generateWordArray(section: MemorySection): Promise<void> {
    ...
    for (let i = 0; i < length; i += 8) {
      ...
      for (let j = 0; j < 8; j += 2) {
        ...
        const value = this.memoryContents[memoryIndex1] + (this.memoryContents[memoryIndex2] << 8);
        words.push(
          this._decimalMode ? (value & 0xffff).toString(10) : `$${toHexa4(value & 0xffff)}`
        );
      }
    ...
    this._output.addItem({
      ...
      instruction:
        ...
        (this._decimalMode
          ...
          : `$${toHexa4(section.endAddress - section.startAddress + 1)}`)
    });
  }
  ...
  private decodeInstruction(
    ...
    disassemblyItem.tstates = cycles;
  ...
  private processPragma(disassemblyItem: DisassemblyItem, pragmaIndex: number): void {
    ...
    switch (pragma) {
      case "I": // Immediate value
        ...
        replacement = this._decimalMode ? `#${value}` : `#$${toHexa2(value)}`;
        symbolPresent = true;
        symbolValue = value;
        break;
      case "Z": // Zero page address
        var zpAddr = this.fetch();
        replacement = this._decimalMode ? zpAddr.toString(10) : `$${toHexa2(zpAddr)}`;
        symbolPresent = true;
        symbolValue = zpAddr;
        break;
      case "X": // Zero page,X
        var zpAddrX = this.fetch();
        replacement = this._decimalMode ? `${zpAddrX},x` : `$${toHexa2(zpAddrX)},x`;
        symbolPresent = true;
        symbolValue = zpAddrX;
        break;
      case "Y": // Zero page,Y
        var zpAddrY = this.fetch();
        replacement = this._decimalMode ? `${zpAddrY},y` : `$${toHexa2(zpAddrY)},y`;
        symbolPresent = true;
        symbolValue = zpAddrY;
        break;
      case "A": // Absolute address
        var absAddr = this.fetchWord();
        replacement = this._decimalMode ? absAddr.toString(10) : `$${toHexa4(absAddr)}`;
        symbolPresent = true;
        symbolValue = absAddr;
        break;
      case "J": // Absolute address for jump (creates label)
        ...
        replacement = `${this.options?.noLabelPrefix ?? false ? "$" : "L"}${this._decimalMode ? toDecimal5(jumpAddr) : toHexa4(jumpAddr)}`;
        symbolPresent = true;
      ...
      case "U": // Absolute,X
        ...
        replacement = this._decimalMode ? `${absAddrX},x` : `$${toHexa4(absAddrX)},x`;
        symbolPresent = true;
        symbolValue = absAddrX;
        break;
      case "V": // Absolute,Y
        var absAddrY = this.fetchWord();
        replacement = this._decimalMode ? `${absAddrY},y` : `$${toHexa4(absAddrY)},y`;
        symbolPresent = true;
      ...
      case "R": // Relative address
        ...
        replacement = `${this.options?.noLabelPrefix ?? false ? "$" : "L"}${this._decimalMode ? toDecimal5(labelAddr) : toHexa4(labelAddr)}`;
        symbolPresent = true;
        disassemblyItem.hasLabelSymbol = true;
      ...
      case "N": // (Indirect,X) - ($ZZ,X)
        var indX = this.fetch();
        replacement = this._decimalMode ? `(${indX},x)` : `($${toHexa2(indX)},x)`;
        symbolPresent = true;
        symbolValue = indX;
        break;
      case "M": // (Indirect),Y - ($ZZ),Y
        var indY = this.fetch();
        replacement = this._decimalMode ? `(${indY}),y` : `($${toHexa2(indY)}),y`;
        symbolPresent = true;
        symbolValue = indY;
        break;
      case "P": // (Indirect) - ($AAAA) for JMP
        var indAddr = this.fetchWord();
        replacement = this._decimalMode ? `(${indAddr})` : `($${toHexa4(indAddr)})`;
        symbolPresent = true;
        symbolValue = indAddr;
        break;
    }
