...
export function parseCommand (command: string): Token[] {
  const tokens: Token[] = [];
  const input = new CommandInputStream(command);
  const tokenStream = new CommandTokenStream(input);
  let eof = false;
  do {
 ...
 */
export class CommandInputStream {
 ...
 *   ;
 */
export class CommandTokenStream {
   ...
   */
  constructor (public readonly input: CommandInputStream) {}
