...
export default function BankDropdown({
  ...
  // Create an array of DIV RAM values (used when showNextItems is true)
  const divRamValues = Array.from({ length: 16 }, (_, i) => `M${i.toString(16).toUpperCase()}`);

  const [selectedIndex, setSelectedIndex] = useState(0);
  const [selectedValue, setSelectedValue] = useState<string>(
    showNextItems && initialValue < 0 ? initialValue.toString(10) : toHexa2(initialValue)
  );

  const setNewValue = (value: number) => {
    const newValue = showNextItems && value < 0 ? value.toString(10) : toHexa2(value);
  ...
  return (
    <Select.Root
      value={selectedValue}
      onValueChange={(v) =>
        setNewValue(showNextItems && v.startsWith("-") ? parseInt(v, 10) : parseInt(v, 16))
      }
    >
        ...
        >
          <Select.Viewport onKeyDown={handleKeyDown}>
            ...
            {hexRows.map((row, rowIndex) => (
              <HStack key={rowIndex}>
                {row.map((v) => {
                  const index = showNextItems ? v : hexValues.indexOf(v);
                  return (
                    <Select.Item
                      key={v}
                      value={toHexa2(v)}
