...
export const PaletteEditor = ({
  ...
  const updateColorValue = (r: number, g: number, b: number, priority: boolean) => {
    const oldColorValue = palette[selectedIndex];

    const colorValue =
      (priority ? 0x8000 : 0x0000) | (r << 5) | (g << 2) | ((b >> 1) & 0x03) | ((b & 0x01) << 8);
  ...
  return ready ? (
    <>
      <Row>
        <KeyHandler xclass={styles.headerRow} onKey={handleCommonKeys} autofocus={true}>
          <SmallIconButton
            iconName="undo"
            title={"Undo"}
      ...
      <Panel xclass={styles.editorPanel}>
        <Row>
          <div className={styles.editorArea}>
            <Row>
              <div className={styles.colorBar} style={{ backgroundColor: selectedColor }}>
              ...
              >
                ...
                {levels9.map((level) => (
                  <ColorItem
                    ...
                    onSelected={() => updateColorValue(level, selectedG, selectedB, priority)}
                  />
                ))}
              </ColorScale>
              <ColorScale
              ...
              >
                ...
                {levels9.map((level) => (
                  <ColorItem
                    ...
                    onSelected={() => updateColorValue(selectedR, level, selectedB, priority)}
                  />
                ))}
              </ColorScale>
              <ColorScale
              ...
              >
                ...
                {levels9.map((level) => (
                  <ColorItem
                    ...
                    onSelected={() => updateColorValue(selectedR, selectedG, level, priority)}
                  />
                ))}
              </ColorScale>
            </Row>
...
const ColorScale = ({ children, level, onOtherKey, onLevelChanged }: ColorScaleProps) => {
  return (
    <KeyHandler
...
const ColorItem = ({ component, level, selected, allowSelection, onSelected }: ColorItemProps) => {
  const colorValue =
    component === "R"
      ? level << 5
      : component === "G"
