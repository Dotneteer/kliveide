...
import { SymbolValueMap } from "../../src/main/compiler-common/abstractions";
import { ErrorCodes } from "../../src/main/compiler-common/assembler-errors";
import { SpectrumModelType } from "../../src/main/z80-compiler/SpectrumModelTypes";
import { AssemblerOptions, AssemblerOutput } from "../../src/main/compiler-common/assembler-in-out";
import { M6510Assembler } from "../../src/main/m6510-compiler/m6510-assembler";
import { M6510TokenType } from "../../src/main/m6510-compiler/m6510-token-stream";
import { M6510Node } from "../../src/main/m6510-compiler/assembler-tree-nodes";
...
export async function testExpression(
  ...
  const compiler = new M6510Assembler();
...
export async function expressionFails(
  ...
  if (params.length === 0) {
    ...
    for (let i = 0; i < params.length; i++) {
      ...
      if (entry[1]) {
        expect(output.errors[i].message.indexOf(entry[1])).toBeGreaterThanOrEqual(0);
      }
    }
  }
}
...
export async function compileFileWorks(
  filename: string
): Promise<AssemblerOutput<M6510Node, M6510TokenType>> {
...
export async function compileFile(
  ...
  const assembler = new M6510Assembler();
