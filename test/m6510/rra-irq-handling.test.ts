...
describe("M6510 - RRA Interrupt Handling", () => {
  ...
  describe("RRA Absolute,X (0x7F) - Read-Modify-Write Interrupt Timing", () => {
    it("clock cycle is correct when IRQ occurs before RRA $1000,X", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then RRA $1000,X to test interrupt before RRA, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x7f,
        0x00,
        0x10, // $8001: RRA $1000,X (7 cycles) --> IRQ before: 7 + 11 cycles
      ...
      testMachine.cpu.a = 0x10; // Accumulator for ADD operation
      ...
      // Trigger IRQ after NOP completes (at tact 2), before RRA starts
      ...
      expect(testMachine.readMemory(0x1005)).toBe(0x21); // Memory: 0x42 >> 1 = 0x21
    ...
    it("clock cycle is correct when IRQ occurs during RRA $1000,X", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then RRA $1000,X to test interrupt during RRA, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x7f,
        0x00,
        0x10, // $8001: RRA $1000,X (7 cycles) --> IRQ during: 7 + 11 cycles
        0xea, // $8004: NOP (2 cycles)
        0x00 // $8005: BRK (stops execution)
      ];
      ...
      // Trigger IRQ during RRA execution
      ...
      // Verify interrupt occurred during RRA execution
  ...
  describe("RRA Zero Page (0x67) - Read-Modify-Write Interrupt Timing", () => {
    it("clock cycle is correct when IRQ occurs before RRA $80", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then RRA $80 to test interrupt before RRA, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x67,
        0x80, // $8001: RRA $80 (5 cycles) --> IRQ before: 5 + 11 cycles
      ...
      // Verify the RMW operation completed correctly
      expect(testMachine.readMemory(0x80)).toBe(0x21); // Memory: 0x42 >> 1 = 0x21
    ...
    it("clock cycle is correct when IRQ occurs during RRA $90", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then RRA $90 to test interrupt during RRA, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x67,
        0x90, // $8001: RRA $90 (5 cycles) --> IRQ during: 5 + 11 cycles
        0xea, // $8003: NOP (2 cycles)
        0x00 // $8004: BRK (stops execution)
      ];
      ...
      // Verify the RMW operation completed correctly
      expect(testMachine.readMemory(0x90)).toBe(0x21); // Memory: 0x42 >> 1 = 0x21
  ...
  describe("RRA Absolute (0x6F) - Read-Modify-Write Interrupt Timing", () => {
    it("clock cycle is correct when IRQ occurs before RRA $2000", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then RRA $2000 to test interrupt before RRA, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x6f,
        0x00,
        0x20, // $8001: RRA $2000 (6 cycles) --> IRQ before: 6 + 11 cycles
        0xea, // $8004: NOP (2 cycles)
        0x00 // $8005: BRK (stops execution)
      ];
      ...
      // Verify the RMW operation completed correctly
      expect(testMachine.readMemory(0x2000)).toBe(0x21); // Memory: 0x42 >> 1 = 0x21
    ...
    it("clock cycle is correct when IRQ occurs during RRA $3000", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then RRA $3000 to test interrupt during RRA, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x6f,
        0x00,
        0x30, // $8001: RRA $3000 (6 cycles) --> IRQ during: 6 + 11 cycles
        0xea, // $8004: NOP (2 cycles)
        0x00 // $8005: BRK (stops execution)
      ];
      ...
      // Verify the RMW operation completed correctly
      expect(testMachine.readMemory(0x3000)).toBe(0x21); // Memory: 0x42 >> 1 = 0x21
  ...
  describe("RRA Zero Page,X (0x77) - Read-Modify-Write Interrupt Timing", () => {
    it("clock cycle is correct when IRQ occurs before RRA $80,X", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then RRA $80,X to test interrupt before RRA, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x77,
        0x80, // $8001: RRA $80,X (6 cycles) --> IRQ before: 6 + 11 cycles
        0xea, // $8003: NOP (2 cycles)
        0x00 // $8004: BRK (stops execution)
      ];
      ...
      // Verify the RMW operation completed correctly
      expect(testMachine.readMemory(0x85)).toBe(0x21); // Memory: 0x42 >> 1 = 0x21
    ...
    it("clock cycle is correct when IRQ occurs during RRA $F0,X (with zero page wrapping)", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then RRA $F0,X to test interrupt during RRA with zero page wrapping, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x77,
        0xf0, // $8001: RRA $F0,X (6 cycles) --> IRQ during: 6 + 11 cycles
        0xea, // $8003: NOP (2 cycles)
        0x00 // $8004: BRK (stops execution)
      ];
      ...
      expect(testMachine.readMemory(0x10)).toBe(0x21); // Memory: 0x42 >> 1 = 0x21
