...
describe("M6510 - DCP Interrupt Handling", () => {
  ...
  beforeEach(() => {
    machine = new M6510TestMachine(RunMode.UntilBrk);
  });

  describe("DCP Absolute,X (0xDF) - Read-Modify-Write Interrupt Timing", () => {
    it("clock cycle is correct when IRQ occurs before DCP $1000,X", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then DCP $1000,X to test interrupt before DCP, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0xdf,
        0x00,
        0x10, // $8001: DCP $1000,X (7 cycles) --> IRQ before: 7 + 11 cycles
      ...
      testMachine.cpu.a = 0x43; // Accumulator for CMP operation (compared with decremented memory)
      testMachine.cpu.p = FlagSetMask6510.UNUSED; // I flag clear

      // Set initial memory value at target address
      testMachine.writeMemory(0x1005, 0x42); // Will be decremented to 0x41
      testMachine.writeMemory(0xd000, 0x00); // Clear marker

      // Trigger IRQ after NOP completes (at tact 2), before DCP starts
      ...
      expect(testMachine.readMemory(0x1005)).toBe(0x41); // Memory: 0x42 - 1 = 0x41
    ...
    it("clock cycle is correct when IRQ occurs during DCP $1000,X", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then DCP $1000,X to test interrupt during DCP, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0xdf,
        0x00,
        0x10, // $8001: DCP $1000,X (7 cycles) --> IRQ during: 7 + 11 cycles
        0xea, // $8004: NOP (2 cycles)
        0x00 // $8005: BRK (stops execution)
      ];
      ...
      // Trigger IRQ during DCP execution
      ...
      // Verify interrupt occurred during DCP execution
  ...
  describe("DCP Zero Page (0xC7) - Read-Modify-Write Interrupt Timing", () => {
    it("clock cycle is correct when IRQ occurs before DCP $80", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then DCP $80 to test interrupt before DCP, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0xc7,
        0x80, // $8001: DCP $80 (5 cycles) --> IRQ before: 5 + 11 cycles
      ...
      // Set initial memory value at target address
      testMachine.writeMemory(0x80, 0x42); // Will be decremented to 0x41
      testMachine.writeMemory(0xd000, 0x00); // Clear marker
      ...
      // Verify the RMW operation completed correctly
      expect(testMachine.readMemory(0x80)).toBe(0x41); // Memory: 0x42 - 1 = 0x41
    ...
    it("clock cycle is correct when IRQ occurs during DCP $90", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then DCP $90 to test interrupt during DCP, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0xc7,
        0x90, // $8001: DCP $90 (5 cycles) --> IRQ during: 5 + 11 cycles
        0xea, // $8003: NOP (2 cycles)
        0x00 // $8004: BRK (stops execution)
      ];
      ...
      // Set initial memory value at target address
      testMachine.writeMemory(0x90, 0x42); // Will be decremented to 0x41
      ...
      // Verify the RMW operation completed correctly
      expect(testMachine.readMemory(0x90)).toBe(0x41); // Memory: 0x42 - 1 = 0x41
  ...
  describe("DCP Absolute (0xCF) - Read-Modify-Write Interrupt Timing", () => {
    it("clock cycle is correct when IRQ occurs before DCP $2000", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then DCP $2000 to test interrupt before DCP, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0xcf,
        0x00,
        0x20, // $8001: DCP $2000 (6 cycles) --> IRQ before: 6 + 11 cycles
        0xea, // $8004: NOP (2 cycles)
        0x00 // $8005: BRK (stops execution)
      ];
      ...
      // Set initial memory value at target address
      testMachine.writeMemory(0x2000, 0x42); // Will be decremented to 0x41
      ...
      // Verify the RMW operation completed correctly
      expect(testMachine.readMemory(0x2000)).toBe(0x41); // Memory: 0x42 - 1 = 0x41
    ...
    it("clock cycle is correct when IRQ occurs during DCP $3000", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then DCP $3000 to test interrupt during DCP, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0xcf,
        0x00,
        0x30, // $8001: DCP $3000 (6 cycles) --> IRQ during: 6 + 11 cycles
        0xea, // $8004: NOP (2 cycles)
        0x00 // $8005: BRK (stops execution)
      ];
      ...
      // Set initial memory value at target address
      testMachine.writeMemory(0x3000, 0x42); // Will be decremented to 0x41
      ...
      // Verify the RMW operation completed correctly
      expect(testMachine.readMemory(0x3000)).toBe(0x41); // Memory: 0x42 - 1 = 0x41
  ...
  describe("DCP Zero Page,X (0xD7) - Read-Modify-Write Interrupt Timing", () => {
    it("clock cycle is correct when IRQ occurs before DCP $80,X", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then DCP $80,X to test interrupt before DCP, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0xd7,
        0x80, // $8001: DCP $80,X (6 cycles) --> IRQ before: 6 + 11 cycles
        0xea, // $8003: NOP (2 cycles)
        0x00 // $8004: BRK (stops execution)
      ];
      ...
      // Set initial memory value at target address
      testMachine.writeMemory(0x85, 0x42); // Will be decremented to 0x41
      ...
      // Verify the RMW operation completed correctly
      expect(testMachine.readMemory(0x85)).toBe(0x41); // Memory: 0x42 - 1 = 0x41
    ...
    it("clock cycle is correct when IRQ occurs during DCP $F0,X (with zero page wrapping)", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then DCP $F0,X to test interrupt during DCP with zero page wrapping, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0xd7,
        0xf0, // $8001: DCP $F0,X (6 cycles) --> IRQ during: 6 + 11 cycles
        0xea, // $8003: NOP (2 cycles)
        0x00 // $8004: BRK (stops execution)
      ];
      ...
      // Set initial memory value at target address
      testMachine.writeMemory(0x10, 0x42); // Will be decremented to 0x41
      ...
      expect(testMachine.readMemory(0x10)).toBe(0x41); // Memory: 0x42 - 1 = 0x41
