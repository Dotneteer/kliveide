...
describe("M6510 - SRE Interrupt Handling", () => {
  ...
  describe("SRE Absolute,X (0x5F) - Read-Modify-Write Interrupt Timing", () => {
    it("clock cycle is correct when IRQ occurs before SRE $1000,X", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then SRE $1000,X to test interrupt before SRE, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x5f,
        0x00,
        0x10, // $8001: SRE $1000,X (7 cycles) --> IRQ before: 7 + 11 cycles
      ...
      testMachine.cpu.a = 0x0F; // Accumulator for EOR operation
      ...
      // Trigger IRQ after NOP completes (at tact 2), before SRE starts
    ...
    it("clock cycle is correct when IRQ occurs during SRE $1000,X", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then SRE $1000,X to test interrupt during SRE, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x5f,
        0x00,
        0x10, // $8001: SRE $1000,X (7 cycles) --> IRQ during: 7 + 11 cycles
        0xea, // $8004: NOP (2 cycles)
        0x00 // $8005: BRK (stops execution)
      ];
      ...
      // Trigger IRQ during SRE execution
      ...
      // Verify interrupt occurred during SRE execution
  ...
  describe("SRE Zero Page (0x47) - Read-Modify-Write Interrupt Timing", () => {
    it("clock cycle is correct when IRQ occurs before SRE $80", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then SRE $80 to test interrupt before SRE, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x47,
        0x80, // $8001: SRE $80 (5 cycles) --> IRQ before: 5 + 11 cycles
    ...
    it("clock cycle is correct when IRQ occurs during SRE $90", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then SRE $90 to test interrupt during SRE, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x47,
        0x90, // $8001: SRE $90 (5 cycles) --> IRQ during: 5 + 11 cycles
        0xea, // $8003: NOP (2 cycles)
        0x00 // $8004: BRK (stops execution)
      ];
  ...
  describe("SRE Absolute (0x4F) - Read-Modify-Write Interrupt Timing", () => {
    it("clock cycle is correct when IRQ occurs before SRE $2000", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then SRE $2000 to test interrupt before SRE, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x4f,
        0x00,
        0x20, // $8001: SRE $2000 (6 cycles) --> IRQ before: 6 + 11 cycles
        0xea, // $8004: NOP (2 cycles)
        0x00 // $8005: BRK (stops execution)
      ];
    ...
    it("clock cycle is correct when IRQ occurs during SRE $3000", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then SRE $3000 to test interrupt during SRE, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x4f,
        0x00,
        0x30, // $8001: SRE $3000 (6 cycles) --> IRQ during: 6 + 11 cycles
        0xea, // $8004: NOP (2 cycles)
        0x00 // $8005: BRK (stops execution)
      ];
  ...
  describe("SRE Zero Page,X (0x57) - Read-Modify-Write Interrupt Timing", () => {
    it("clock cycle is correct when IRQ occurs before SRE $80,X", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then SRE $80,X to test interrupt before SRE, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x57,
        0x80, // $8001: SRE $80,X (6 cycles) --> IRQ before: 6 + 11 cycles
        0xea, // $8003: NOP (2 cycles)
        0x00 // $8004: BRK (stops execution)
      ];
    ...
    it("clock cycle is correct when IRQ occurs during SRE $F0,X (with zero page wrapping)", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then SRE $F0,X to test interrupt during SRE with zero page wrapping, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x57,
        0xf0, // $8001: SRE $F0,X (6 cycles) --> IRQ during: 6 + 11 cycles
        0xea, // $8003: NOP (2 cycles)
        0x00 // $8004: BRK (stops execution)
      ];
