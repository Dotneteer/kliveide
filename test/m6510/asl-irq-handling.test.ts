...
describe("M6510 - ASL Interrupt Handling", () => {
  ...
  describe("ASL Absolute,X (0x1E) - Read-Modify-Write Interrupt Timing", () => {
    it("clock cycle is correct when IRQ occurs before ASL $1000,X", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then ASL $1000,X to test interrupt before ASL, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x1e,
        0x00,
        0x10, // $8001: ASL $1000,X (7 cycles) --> IRQ before: 7 + 11 cycles
      ...
      // Trigger IRQ after NOP completes (at tact 2), before ASL starts
      ...
      expect(testMachine.readMemory(0x1005)).toBe(0x84); // 0x42 << 1 = 0x84

      // Verify interrupt occurred at expected tact
      expect(testMachine.cpu.interruptedInstructionStartTact).toBe(2);

      // Verify clock cycles
      expect(testMachine.cpu.tacts).toBe(30); // Based on pattern from INC/DEC tests
    ...
    it("clock cycle is correct when IRQ occurs during ASL $1000,X", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then ASL $1000,X to test interrupt during ASL, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x1e,
        0x00,
        0x10, // $8001: ASL $1000,X (7 cycles) --> IRQ during: 7 + 11 cycles
        0xea, // $8004: NOP (2 cycles)
        0x00 // $8005: BRK (stops execution)
      ];
      ...
      // Trigger IRQ during ASL execution
      ...
      // Verify interrupt occurred during ASL execution
  ...
  describe("ASL Zero Page (0x06) - Read-Modify-Write Interrupt Timing", () => {
    it("clock cycle is correct when IRQ occurs before ASL $80", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then ASL $80 to test interrupt before ASL, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x06,
        0x80, // $8001: ASL $80 (5 cycles) --> IRQ before: 5 + 11 cycles
      ...
      // Verify the RMW operation completed correctly
      expect(testMachine.readMemory(0x80)).toBe(0x84); // 0x42 << 1 = 0x84

      // Verify interrupt occurred at expected tact
      expect(testMachine.cpu.interruptedInstructionStartTact).toBe(2);

      // Verify clock cycles: Follow the same pattern as working test
      expect(testMachine.cpu.tacts).toBe(28); // Based on DEC Zero Page pattern
    ...
    it("clock cycle is correct when IRQ occurs during ASL $90", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then ASL $90 to test interrupt during ASL, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x06,
        0x90, // $8001: ASL $90 (5 cycles) --> IRQ during: 5 + 11 cycles
        0xea, // $8003: NOP (2 cycles)
        0x00 // $8004: BRK (stops execution)
      ];
      ...
      // Verify the RMW operation completed correctly
      expect(testMachine.readMemory(0x90)).toBe(0x84); // 0x42 << 1 = 0x84
  ...
  describe("ASL Absolute (0x0E) - Read-Modify-Write Interrupt Timing", () => {
    it("clock cycle is correct when IRQ occurs before ASL $2000", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then ASL $2000 to test interrupt before ASL, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x0e,
        0x00,
        0x20, // $8001: ASL $2000 (6 cycles) --> IRQ before: 6 + 11 cycles
        0xea, // $8004: NOP (2 cycles)
        0x00 // $8005: BRK (stops execution)
      ];
      ...
      // Verify the RMW operation completed correctly
      expect(testMachine.readMemory(0x2000)).toBe(0x84); // 0x42 << 1 = 0x84

      // Verify interrupt occurred at expected tact
      expect(testMachine.cpu.interruptedInstructionStartTact).toBe(2);

      // Verify clock cycles: Follow the same pattern as working test
      expect(testMachine.cpu.tacts).toBe(29); // Based on DEC Absolute pattern
    ...
    it("clock cycle is correct when IRQ occurs during ASL $3000", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then ASL $3000 to test interrupt during ASL, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x0e,
        0x00,
        0x30, // $8001: ASL $3000 (6 cycles) --> IRQ during: 6 + 11 cycles
        0xea, // $8004: NOP (2 cycles)
        0x00 // $8005: BRK (stops execution)
      ];
      ...
      // Verify the RMW operation completed correctly
      expect(testMachine.readMemory(0x3000)).toBe(0x84); // 0x42 << 1 = 0x84
  ...
  describe("ASL Zero Page,X (0x16) - Read-Modify-Write Interrupt Timing", () => {
    it("clock cycle is correct when IRQ occurs before ASL $80,X", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then ASL $80,X to test interrupt before ASL, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x16,
        0x80, // $8001: ASL $80,X (6 cycles) --> IRQ before: 6 + 11 cycles
        0xea, // $8003: NOP (2 cycles)
        0x00 // $8004: BRK (stops execution)
      ];
      ...
      // Verify the RMW operation completed correctly
      expect(testMachine.readMemory(0x85)).toBe(0x84); // 0x42 << 1 = 0x84

      // Verify interrupt occurred at expected tact
      expect(testMachine.cpu.interruptedInstructionStartTact).toBe(2);

      // Verify clock cycles: Follow the same pattern as working test
      expect(testMachine.cpu.tacts).toBe(29); // Based on DEC Zero Page,X pattern
    ...
    it("clock cycle is correct when IRQ occurs during ASL $F0,X (with zero page wrapping)", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then ASL $F0,X to test interrupt during ASL with zero page wrapping, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x16,
        0xf0, // $8001: ASL $F0,X (6 cycles) --> IRQ during: 6 + 11 cycles
        0xea, // $8003: NOP (2 cycles)
        0x00 // $8004: BRK (stops execution)
      ];
      ...
      expect(testMachine.readMemory(0x10)).toBe(0x84); // 0x42 << 1 = 0x84
