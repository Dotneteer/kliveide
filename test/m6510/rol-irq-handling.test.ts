...
describe("M6510 - ROL Interrupt Handling", () => {
  ...
  describe("ROL Absolute,X (0x3E) - Read-Modify-Write Interrupt Timing", () => {
    it("clock cycle is correct when IRQ occurs before ROL $1000,X", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then ROL $1000,X to test interrupt before ROL, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x3e,
        0x00,
        0x10, // $8001: ROL $1000,X (7 cycles) --> IRQ before: 7 + 11 cycles
      ...
      // Trigger IRQ after NOP completes (at tact 2), before ROL starts
      ...
      // ROL with carry clear: 0x42 << 1 | 0 = 0x84
      expect(testMachine.readMemory(0x1005)).toBe(0x84);
    ...
    it("clock cycle is correct when IRQ occurs during ROL $1000,X", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then ROL $1000,X to test interrupt during ROL, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x3e,
        0x00,
        0x10, // $8001: ROL $1000,X (7 cycles) --> IRQ during: 7 + 11 cycles
        0xea, // $8004: NOP (2 cycles)
        0x00 // $8005: BRK (stops execution)
      ];
      ...
      // Trigger IRQ during ROL execution
      ...
      // Verify the RMW operation completed correctly
      // ROL with carry set: 0x42 << 1 | 1 = 0x85
      expect(testMachine.readMemory(0x1005)).toBe(0x85);

      // Verify interrupt occurred during ROL execution
  ...
  describe("ROL Zero Page (0x26) - Read-Modify-Write Interrupt Timing", () => {
    it("clock cycle is correct when IRQ occurs before ROL $80", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then ROL $80 to test interrupt before ROL, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x26,
        0x80, // $8001: ROL $80 (5 cycles) --> IRQ before: 5 + 11 cycles
      ...
      // Verify the RMW operation completed correctly
      // ROL with carry clear: 0x42 << 1 | 0 = 0x84
      expect(testMachine.readMemory(0x80)).toBe(0x84);
    ...
    it("clock cycle is correct when IRQ occurs during ROL $90", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then ROL $90 to test interrupt during ROL, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x26,
        0x90, // $8001: ROL $90 (5 cycles) --> IRQ during: 5 + 11 cycles
        0xea, // $8003: NOP (2 cycles)
        0x00 // $8004: BRK (stops execution)
      ];
      ...
      // Verify the RMW operation completed correctly
      // ROL with carry set: 0x42 << 1 | 1 = 0x85
      expect(testMachine.readMemory(0x90)).toBe(0x85);
  ...
  describe("ROL Absolute (0x2E) - Read-Modify-Write Interrupt Timing", () => {
    it("clock cycle is correct when IRQ occurs before ROL $2000", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then ROL $2000 to test interrupt before ROL, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x2e,
        0x00,
        0x20, // $8001: ROL $2000 (6 cycles) --> IRQ before: 6 + 11 cycles
        0xea, // $8004: NOP (2 cycles)
        0x00 // $8005: BRK (stops execution)
      ];
      ...
      // Verify the RMW operation completed correctly
      // ROL with carry clear: 0x42 << 1 | 0 = 0x84
      expect(testMachine.readMemory(0x2000)).toBe(0x84);
    ...
    it("clock cycle is correct when IRQ occurs during ROL $3000", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then ROL $3000 to test interrupt during ROL, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x2e,
        0x00,
        0x30, // $8001: ROL $3000 (6 cycles) --> IRQ during: 6 + 11 cycles
        0xea, // $8004: NOP (2 cycles)
        0x00 // $8005: BRK (stops execution)
      ];
      ...
      // Verify the RMW operation completed correctly
      // ROL with carry set: 0x42 << 1 | 1 = 0x85
      expect(testMachine.readMemory(0x3000)).toBe(0x85);
  ...
  describe("ROL Zero Page,X (0x36) - Read-Modify-Write Interrupt Timing", () => {
    it("clock cycle is correct when IRQ occurs before ROL $80,X", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then ROL $80,X to test interrupt before ROL, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x36,
        0x80, // $8001: ROL $80,X (6 cycles) --> IRQ before: 6 + 11 cycles
        0xea, // $8003: NOP (2 cycles)
        0x00 // $8004: BRK (stops execution)
      ];
      ...
      // Verify the RMW operation completed correctly
      // ROL with carry clear: 0x42 << 1 | 0 = 0x84
      expect(testMachine.readMemory(0x85)).toBe(0x84);
    ...
    it("clock cycle is correct when IRQ occurs during ROL $F0,X (with zero page wrapping)", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then ROL $F0,X to test interrupt during ROL with zero page wrapping, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x36,
        0xf0, // $8001: ROL $F0,X (6 cycles) --> IRQ during: 6 + 11 cycles
        0xea, // $8003: NOP (2 cycles)
        0x00 // $8004: BRK (stops execution)
      ];
      ...
      // ROL with carry set: 0x42 << 1 | 1 = 0x85
      expect(testMachine.readMemory(0x10)).toBe(0x85);
