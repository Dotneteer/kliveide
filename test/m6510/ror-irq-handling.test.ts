...
describe("M6510 - ROR Interrupt Handling", () => {
  ...
  describe("ROR Absolute,X (0x7E) - Read-Modify-Write Interrupt Timing", () => {
    it("clock cycle is correct when IRQ occurs before ROR $1000,X", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then ROR $1000,X to test interrupt before ROR, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x7e,
        0x00,
        0x10, // $8001: ROR $1000,X (7 cycles) --> IRQ before: 7 + 11 cycles
      ...
      testMachine.cpu.p = FlagSetMask6510.UNUSED; // I flag clear, Carry clear
      ...
      // Trigger IRQ after NOP completes (at tact 2), before ROR starts
      ...
      // ROR with carry clear: (0 << 7) | (0x42 >> 1) = 0x21
      expect(testMachine.readMemory(0x1005)).toBe(0x21);
    ...
    it("clock cycle is correct when IRQ occurs during ROR $1000,X", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then ROR $1000,X to test interrupt during ROR, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x7e,
        0x00,
        0x10, // $8001: ROR $1000,X (7 cycles) --> IRQ during: 7 + 11 cycles
        0xea, // $8004: NOP (2 cycles)
        0x00 // $8005: BRK (stops execution)
      ];
      ...
      testMachine.cpu.sp = 0xff;
      testMachine.cpu.x = 0x05; // No page boundary crossing
      testMachine.cpu.p = FlagSetMask6510.UNUSED | FlagSetMask6510.C; // I flag clear, Carry set

      // Set initial memory value at target address
      testMachine.writeMemory(0x1005, 0x42); // 0x42 = 01000010
      testMachine.writeMemory(0xd000, 0x00); // Clear marker

      // Trigger IRQ during ROR execution
      ...
      // Verify the RMW operation completed correctly
      // ROR with carry set: (1 << 7) | (0x42 >> 1) = 0x80 | 0x21 = 0xA1
      expect(testMachine.readMemory(0x1005)).toBe(0xA1);

      // Verify interrupt occurred during ROR execution
  ...
  describe("ROR Zero Page (0x66) - Read-Modify-Write Interrupt Timing", () => {
    it("clock cycle is correct when IRQ occurs before ROR $80", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then ROR $80 to test interrupt before ROR, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x66,
        0x80, // $8001: ROR $80 (5 cycles) --> IRQ before: 5 + 11 cycles
      ...
      // Verify the RMW operation completed correctly
      // ROR with carry clear: (0 << 7) | (0x42 >> 1) = 0x21
      expect(testMachine.readMemory(0x80)).toBe(0x21);
    ...
    it("clock cycle is correct when IRQ occurs during ROR $90", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then ROR $90 to test interrupt during ROR, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x66,
        0x90, // $8001: ROR $90 (5 cycles) --> IRQ during: 5 + 11 cycles
        0xea, // $8003: NOP (2 cycles)
        0x00 // $8004: BRK (stops execution)
      ];
      ...
      // Verify the RMW operation completed correctly
      // ROR with carry set: (1 << 7) | (0x42 >> 1) = 0x80 | 0x21 = 0xA1
      expect(testMachine.readMemory(0x90)).toBe(0xA1);
  ...
  describe("ROR Absolute (0x6E) - Read-Modify-Write Interrupt Timing", () => {
    it("clock cycle is correct when IRQ occurs before ROR $2000", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then ROR $2000 to test interrupt before ROR, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x6e,
        0x00,
        0x20, // $8001: ROR $2000 (6 cycles) --> IRQ before: 6 + 11 cycles
        0xea, // $8004: NOP (2 cycles)
        0x00 // $8005: BRK (stops execution)
      ];
      ...
      // Verify the RMW operation completed correctly
      // ROR with carry clear: (0 << 7) | (0x42 >> 1) = 0x21
      expect(testMachine.readMemory(0x2000)).toBe(0x21);
    ...
    it("clock cycle is correct when IRQ occurs during ROR $3000", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then ROR $3000 to test interrupt during ROR, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x6e,
        0x00,
        0x30, // $8001: ROR $3000 (6 cycles) --> IRQ during: 6 + 11 cycles
        0xea, // $8004: NOP (2 cycles)
        0x00 // $8005: BRK (stops execution)
      ];
      ...
      // Verify the RMW operation completed correctly
      // ROR with carry set: (1 << 7) | (0x42 >> 1) = 0x80 | 0x21 = 0xA1
      expect(testMachine.readMemory(0x3000)).toBe(0xA1);
  ...
  describe("ROR Zero Page,X (0x76) - Read-Modify-Write Interrupt Timing", () => {
    it("clock cycle is correct when IRQ occurs before ROR $80,X", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then ROR $80,X to test interrupt before ROR, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x76,
        0x80, // $8001: ROR $80,X (6 cycles) --> IRQ before: 6 + 11 cycles
        0xea, // $8003: NOP (2 cycles)
        0x00 // $8004: BRK (stops execution)
      ];
      ...
      // Verify the RMW operation completed correctly
      // ROR with carry clear: (0 << 7) | (0x42 >> 1) = 0x21
      expect(testMachine.readMemory(0x85)).toBe(0x21);
    ...
    it("clock cycle is correct when IRQ occurs during ROR $F0,X (with zero page wrapping)", () => {
      // --- Arrange
      const testMachine = new M6510TestMachine(RunMode.UntilBrk);

      // Program: NOP, then ROR $F0,X to test interrupt during ROR with zero page wrapping, then BRK to stop
      const program = [
        0xea, // $8000: NOP (2 cycles)
        0x76,
        0xf0, // $8001: ROR $F0,X (6 cycles) --> IRQ during: 6 + 11 cycles
        0xea, // $8003: NOP (2 cycles)
        0x00 // $8004: BRK (stops execution)
      ];
      ...
      // ROR with carry set: (1 << 7) | (0x42 >> 1) = 0x80 | 0x21 = 0xA1
      expect(testMachine.readMemory(0x10)).toBe(0xA1);
