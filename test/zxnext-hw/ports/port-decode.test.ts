...
describe.each(ALL_CORES)("port decoding - %s core", (core) => {
  it("PORT-004: every even port address sets the border", async () => {
    ...
    const reference = await createSession(core);
    await reference.loadCode(" .org $8000\n jr $");
    reference.runFrames(1); // --- the TS core shows a black first frame after a hard reset
