...
describe("SpriteDevice - Pattern Memory Structure", () => {
  ...
  it("should allocate 512 pattern variant arrays (64 patterns × 8 variants)", () => {
    expect(spriteDevice.patternMemoryVariants).toBeDefined();
    expect(spriteDevice.patternMemoryVariants.length).toBe(512);
  });

  it("should allocate 256 bytes for each pattern variant", () => {
    for (let i = 0; i < 512; i++) {
      expect(spriteDevice.patternMemoryVariants[i]).toBeInstanceOf(Uint8Array);
      expect(spriteDevice.patternMemoryVariants[i].length).toBe(256);
    }
  });

  it("should initialize all pattern memory to zero", () => {
    for (let i = 0; i < 512; i++) {
      for (let j = 0; j < 256; j++) {
        expect(spriteDevice.patternMemoryVariants[i][j]).toBe(0);
      }
    }
  });
...
describe("SpriteDevice - Pattern Write Incremental Updates", () => {
  ...
  it("should write single byte to all 8 variants at correct positions", () => {
    ...
    // Check all 8 variants received the byte at transformed positions
    const baseIdx = 0; // Pattern 0

    // Variant 0: [0][0] → [0][0] (no transform)
    expect(spriteDevice.patternMemoryVariants[baseIdx + 0][0]).toBe(0x42);

    // Variant 1: [0][0] → [15][0] (mirrorY)
    expect(spriteDevice.patternMemoryVariants[baseIdx + 1][(15 << 4) | 0]).toBe(0x42);

    // Variant 2: [0][0] → [0][15] (mirrorX)
    expect(spriteDevice.patternMemoryVariants[baseIdx + 2][(0 << 4) | 15]).toBe(0x42);

    // Variant 3: [0][0] → [15][15] (mirrorXY)
    expect(spriteDevice.patternMemoryVariants[baseIdx + 3][(15 << 4) | 15]).toBe(0x42);

    // Variant 4: [0][0] → [0][15] (rotate)
    expect(spriteDevice.patternMemoryVariants[baseIdx + 4][(0 << 4) | 15]).toBe(0x42);

    // Variant 5: [0][0] → [0][0] (rotate + mirrorY)
    expect(spriteDevice.patternMemoryVariants[baseIdx + 5][(0 << 4) | 0]).toBe(0x42);

    // Variant 6: [0][0] → [15][15] (rotate + mirrorX)
    expect(spriteDevice.patternMemoryVariants[baseIdx + 6][(15 << 4) | 15]).toBe(0x42);

    // Variant 7: [0][0] → [15][0] (rotate + mirrorXY)
    expect(spriteDevice.patternMemoryVariants[baseIdx + 7][(15 << 4) | 0]).toBe(0x42);
  });
  ...
  it("should write byte at center position to all variants correctly", () => {
    ...
    // Variant 0: [7][8] → [7][8]
    expect(spriteDevice.patternMemoryVariants[baseIdx + 0][(7 << 4) | 8]).toBe(0x99);

    // Variant 1: [7][8] → [8][8] (mirrorY: 15-7=8)
    expect(spriteDevice.patternMemoryVariants[baseIdx + 1][(8 << 4) | 8]).toBe(0x99);

    // Variant 2: [7][8] → [7][7] (mirrorX: 15-8=7)
    expect(spriteDevice.patternMemoryVariants[baseIdx + 2][(7 << 4) | 7]).toBe(0x99);

    // Variant 3: [7][8] → [8][7] (mirrorXY)
    expect(spriteDevice.patternMemoryVariants[baseIdx + 3][(8 << 4) | 7]).toBe(0x99);

    // Variant 4: [7][8] → [8][8] (rotate: [x][15-y] = [8][8])
    expect(spriteDevice.patternMemoryVariants[baseIdx + 4][(8 << 4) | 8]).toBe(0x99);

    // Variant 5: [7][8] → [8][7] (rotate + mirrorY: [x][y])
    expect(spriteDevice.patternMemoryVariants[baseIdx + 5][(8 << 4) | 7]).toBe(0x99);

    // Variant 6: [7][8] → [7][8] (rotate + mirrorX: [15-x][15-y])
    expect(spriteDevice.patternMemoryVariants[baseIdx + 6][(7 << 4) | 8]).toBe(0x99);

    // Variant 7: [7][8] → [7][7] (rotate + mirrorXY: [15-x][y])
    expect(spriteDevice.patternMemoryVariants[baseIdx + 7][(7 << 4) | 7]).toBe(0x99);
  });
  ...
  it("should write multiple bytes sequentially to all variants", () => {
    ...
    for (let x = 0; x < 16; x++) {
      expect(spriteDevice.patternMemoryVariants[baseIdx + 0][x]).toBe(0x10 + x);
    }

    // Check variant 1 (mirrorY) has first row in last row position
    for (let x = 0; x < 16; x++) {
      expect(spriteDevice.patternMemoryVariants[baseIdx + 1][(15 << 4) | x]).toBe(0x10 + x);
    }

    // Check variant 2 (mirrorX) has first row reversed
    for (let x = 0; x < 16; x++) {
      expect(spriteDevice.patternMemoryVariants[baseIdx + 2][15 - x]).toBe(0x10 + x);
    }
  });
...
describe("SpriteDevice - Pattern Transformation Correctness", () => {
  ...
  it("Variant 0: No transformation (identity)", () => {
    writeTestPattern(0);
    const variant = spriteDevice.patternMemoryVariants[0]; // Pattern 0, variant 0
  ...
  it("Variant 1: Mirror Y (vertical flip)", () => {
    writeTestPattern(0);
    const variant = spriteDevice.patternMemoryVariants[1]; // Pattern 0, variant 1
  ...
  it("Variant 2: Mirror X (horizontal flip)", () => {
    writeTestPattern(0);
    const variant = spriteDevice.patternMemoryVariants[2]; // Pattern 0, variant 2
  ...
  it("Variant 3: Mirror XY (180° rotation)", () => {
    writeTestPattern(0);
    const variant = spriteDevice.patternMemoryVariants[3]; // Pattern 0, variant 3
  ...
  it("Variant 4: Rotate 90° clockwise", () => {
    writeTestPattern(0);
    const variant = spriteDevice.patternMemoryVariants[4]; // Pattern 0, variant 4
  ...
  it("Variant 5: Rotate + Mirror Y (transpose)", () => {
    writeTestPattern(0);
    const variant = spriteDevice.patternMemoryVariants[5]; // Pattern 0, variant 5
  ...
  it("Variant 6: Rotate + Mirror X (270° rotation)", () => {
    writeTestPattern(0);
    const variant = spriteDevice.patternMemoryVariants[6]; // Pattern 0, variant 6
  ...
  it("Variant 7: Rotate + Mirror XY", () => {
    writeTestPattern(0);
    const variant = spriteDevice.patternMemoryVariants[7]; // Pattern 0, variant 7
...
describe("SpriteDevice - Pattern Variant Indexing", () => {
  ...
  it("should write to correct variant indices for pattern 0", () => {
    ...
    for (let v = 0; v < 8; v++) {
      const variantArray = spriteDevice.patternMemoryVariants[v];
  ...
  it("should write to correct variant indices for pattern 5", () => {
    ...
    for (let v = 0; v < 8; v++) {
      const variantArray = spriteDevice.patternMemoryVariants[baseIdx + v];
      expect(variantArray).toBeDefined();
  ...
  it("should isolate patterns from each other", () => {
    ...
    for (let v = 0; v < 8; v++) {
      const hasValue = Array.from(spriteDevice.patternMemoryVariants[v]).some(b => b === 0x11);
      expect(hasValue).toBe(true);
    }
    ...
    for (let v = 0; v < 8; v++) {
      const hasValue = Array.from(spriteDevice.patternMemoryVariants[baseIdx1 + v]).some(b => b === 0x22);
      expect(hasValue).toBe(true);
    }

    // Pattern 0 should NOT have 0x22
    for (let v = 0; v < 8; v++) {
      const hasValue = Array.from(spriteDevice.patternMemoryVariants[v]).some(b => b === 0x22);
...
describe("SpriteDevice - Pattern Memory Integration with Port 0x303B", () => {
  ...
  it("should write pattern starting at index set by port", () => {
    ...
    const variant0 = spriteDevice.patternMemoryVariants[baseIdx];
...
describe("SpriteDevice - Edge Cases and Boundary Conditions", () => {
  ...
  it("should handle writing to all 4 corners correctly", () => {
    ...
    const variant0 = spriteDevice.patternMemoryVariants[0];
  ...
  it("should handle pattern index overflow correctly", () => {
    ...
    const baseIdx = 0;
    expect(spriteDevice.patternMemoryVariants[baseIdx][0]).toBe(0xFF);
  });
  ...
  it("should handle maximum pattern writes (all 64 patterns)", () => {
    ...
    const pattern0Variant0 = spriteDevice.patternMemoryVariants[0];
    const pattern63Variant0 = spriteDevice.patternMemoryVariants[63 << 3];
