...
const nextRom0Signature0 = [0xf3, 0xc3, 0xef, 0x00, 0x45, 0x44, 0x08, 0x02];
const nextRom0Signature1 = [0xdd, 0xcb, 0x27, 0x46, 0x28, 0x03, 0x7a, 0x53];
const nextRom1Signature0 = [0x00, 0xc3, 0x00, 0x3f, 0xff, 0xff, 0xff, 0xff];
const nextRom1Signature1 = [0xcd, 0x29, 0x20, 0xd8, 0xcd, 0x27, 0x20, 0x30];
const nextRom2Signature0 = [0x00, 0x18, 0xfd, 0x00, 0x00, 0x00, 0x00, 0x00];
const nextRom2Signature1 = [0x6e, 0x74, 0x3d, 0x3f, 0x3f, 0x3f, 0x2d, 0x3f];
const nextRom3Signature0 = [0xf3, 0xaf, 0x01, 0x3b, 0x24, 0xc3, 0xe8, 0x3b];
const nextRom3Signature1 = [0x0d, 0xcd, 0x79, 0x1c, 0xcd, 0xc3, 0x1f, 0xcd];
...
describe("Next - MemoryDevice", async function () {
  ...
  it("0x1ffd/0x7ffd changes ROM bank to ROM 0", async () => {
  ...
  it("0x1ffd/0x7ffd changes ROM bank to ROM 1", async () => {
    io.writePort(0x1ffd, 0x00);
  ...
  it("0x1ffd/0x7ffd changes ROM bank to ROM 2", async () => {
  ...
  it("0x1ffd/0x7ffd changes ROM bank to ROM 3", async () => {
    io.writePort(0x1ffd, 0x04);
    io.writePort(0x7ffd, 0x10);
    expect(isRom(memDevice, 0)).toBe(true);
