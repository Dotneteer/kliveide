...
import { RunMode as RunModeNew, Z80CpuNewTestMachine } from "./test-z80-new";
...
describe("Z80 CPU Performance Benchmark", { timeout: 10000 }, () => {
  ...
  const STACK_ADDRESS = 0x8000;  // Stack area

  // Array containing the comprehensive Z80 test program
  // This shared test program is used for both CPU implementations
  const Z80_TEST_PROGRAM = [
      ...
      0x01, 0x00, 0x02,     // LD BC,0x0200  // Set loop counter to 10 (0x10)
  ...
  /**
   * Runs a benchmark test with the given CPU implementation
   * @param testMachine The test machine to use
   * @param name The name of the CPU implementation
   * @returns Performance metrics object with execution time and T-state information
   */
  function runBenchmark(testMachine: Z80TestMachine | Z80CpuNewTestMachine, name: string) {
    // Initialize the test machine
    testMachine.initCode(Z80_TEST_PROGRAM, START_ADDRESS);
    
    // Initialize data area
    for (let i = 0; i < 16; i++) {
      testMachine.memory[DATA_ADDRESS + i] = i & 0xFF;
    }
    
    testMachine.cpu.pc = START_ADDRESS;
    ...
    const tStates = testMachine.cpu.tacts;
    
    // Verify program completed successfully
    expect(testMachine.cpu.a).toBe(0xFC);
  ...
  it("should compare performance metrics between Z80Cpu and Z80CpuNew implementations with 20 iterations", () => {
    // Number of benchmark iterations to run
    const ITERATIONS = 20;
    
    // Arrays to store metrics from each iteration
    const originalMetricsArray: Array<{ tStatesPerMs: number, totalTStates: number }> = [];
    const newMetricsArray: Array<{ tStatesPerMs: number, totalTStates: number }> = [];
    
    // Run multiple benchmark iterations silently
    for (let i = 0; i < ITERATIONS; i++) {
      // Create fresh test machines for each iteration
      const originalMachine = new Z80TestMachine(RunMode.UntilHalt);
      const newMachine = new Z80CpuNewTestMachine(RunModeNew.UntilHalt);
      
      // Run benchmarks and collect metrics
      const originalMetrics = runBenchmark(originalMachine, `Z80Cpu (iteration ${i + 1})`);
      const newMetrics = runBenchmark(newMachine, `Z80CpuNew (iteration ${i + 1})`);
      
      // Store the metrics
      originalMetricsArray.push({
        tStatesPerMs: originalMetrics.tStatesPerMs,
        totalTStates: originalMetrics.totalTStates
      });
      
      newMetricsArray.push({
        tStatesPerMs: newMetrics.tStatesPerMs,
        totalTStates: newMetrics.totalTStates
      });
      
      // Verify T-states match between implementations in each iteration
      expect(originalMetrics.totalTStates).toBe(newMetrics.totalTStates);
    }
    
    // Calculate average tStatesPerMs for both implementations
    const avgOriginalTStatesPerMs = originalMetricsArray.reduce((sum, metrics) => 
      sum + metrics.tStatesPerMs, 0) / ITERATIONS;
      
    const avgNewTStatesPerMs = newMetricsArray.reduce((sum, metrics) => 
      sum + metrics.tStatesPerMs, 0) / ITERATIONS;
    
    // Calculate performance difference based on averages
    const percentageChange = ((avgNewTStatesPerMs - avgOriginalTStatesPerMs) / avgOriginalTStatesPerMs) * 100;
    const changeType = percentageChange >= 0 ? "improvement" : "regression";
    const absPercentageChange = Math.abs(percentageChange);
    
    // Calculate standard deviation for both implementations
    const originalStdDev = calculateStandardDeviation(originalMetricsArray.map(m => m.tStatesPerMs));
    const newStdDev = calculateStandardDeviation(newMetricsArray.map(m => m.tStatesPerMs));

    // Display only the essential comparison information
    console.log("=== Z80 CPU PERFORMANCE COMPARISON RESULTS ===");
    console.log("Z80Cpu avg T-states/ms: " + avgOriginalTStatesPerMs.toFixed(2) + " (σ: " + originalStdDev.toFixed(2) + ")");
    console.log("Z80CpuNew avg T-states/ms: " + avgNewTStatesPerMs.toFixed(2) + " (σ: " + newStdDev.toFixed(2) + ")");
    console.log("Performance " + changeType + ": " + absPercentageChange.toFixed(2) + "%");
    
    // Verify T-states consistency across iterations
    const allTStatesMatch = originalMetricsArray.every((m, idx) => 
      m.totalTStates === originalMetricsArray[0].totalTStates &&
      m.totalTStates === newMetricsArray[idx].totalTStates
    );
  });
  ...
  function calculateStandardDeviation(values: number[]): number {
    ...
    const sumOfSquaredDiffs = squaredDiffs.reduce((sum, val) => sum + val, 0);
    
    // Step 3: Calculate the variance (average of squared differences)
    const variance = sumOfSquaredDiffs / values.length;
