...
/**
 * This script disassembles the fourth 16KB block (ROM 3) of the enNextZX.rom file
 * and saves the output to nextRom3.txt in the next-disassembly folder.
 * The disassembly starts at address 0x0000 (not 0xC000).
 */
async function disassembleNextRom3() {
  try {
    ...
    // --- Extract fourth 16KB (bytes 0xC000-0xFFFF from file, disassembled as 0x0000-0x3FFF)
    const rom16k = new Uint8Array(romData.buffer, romData.byteOffset + 0xC000, Math.min(16384, romData.length - 0xC000));
    
    console.log(`ROM size: ${romData.length} bytes`);
    console.log(`Extracting ROM 3 (bytes 0xC000-0xFFFF from file)...`);
    ...
    let output = "";
    output += "ZX Spectrum Next ROM Disassembly (ROM 3 - Fourth 16KB)\n";
    output += "=" .repeat(80) + "\n";
    output += `File: enNextZX.rom\n`;
    output += `Source ROM Block: ROM 3 (file bytes 0xC000-0xFFFF)\n`;
    ...
    // --- Save to file
    const outputPath = path.join(__dirname, "nextRom3.txt");
