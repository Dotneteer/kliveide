...
**LoRes** mode is an alternate rendering mode for the ULA&mdash;when enabled (NextReg 0x15 bit 7), it replaces standard ULA output. Standard LoRes reduces resolution to 128×96 and gives every chunky pixel its own 8-bit palette index. It uses the ULA-family display memory, so you can't have both simultaneously&mdash;it's either ULA or LoRes.
