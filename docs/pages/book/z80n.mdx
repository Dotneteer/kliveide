...
```z80klive
IN F,(C)     ; Read from port BC, update flags, discard the data value
```

This is the undocumented variant of `IN r,(C)` where the destination register would be `F`. The data read from the port isn't stored, but the flags update normally based on the value. Useful for testing I/O ports for their effect on flags without loading a register.
...
**`BSLA DE,B`** (opcode: `ED 28`) — Barrel Shift Left Arithmetic:
...
**`BSRA DE,B`** (opcode: `ED 29`) — Barrel Shift Right Arithmetic (sign-preserving):
...
**`BSRL DE,B`** (opcode: `ED 2A`) — Barrel Shift Right Logical (zero-filled):
...
**`BSRF DE,B`** (opcode: `ED 2B`) — Barrel Shift Right Fill (ones-filled):
...
**`BRLC DE,B`** (opcode: `ED 2C`) — Barrel Rotate Left Circular:
...
Flags are not affected. And unlike the software loop shown earlier, `MUL` completes in a handful of T-states — dramatically faster, and far less code.
...
**`LDIX`** (opcode: `ED A4`) — LDI with exclusion:
...
**`LDDX`** (opcode: `ED AC`) — LDD with exclusion (HL decrements):

```z80klive
LDDX    ; if (HL) != A: (DE) = (HL); always: HL--, DE++, BC--
```

**`LDIRX`** (opcode: `ED B4`) — Repeating LDIX:

```z80klive
LDIRX   ; Repeat LDIX until BC = 0
```

**`LDDRX`** (opcode: `ED BC`) — Repeating LDDX:

```z80klive
LDDRX   ; Repeat LDDX until BC = 0
```

**`LDWS`** (opcode: `ED A5`) — Load Word and Swap:
...
**`LDPIRX`** (opcode: `ED B7`) — Block copy from page-aligned source with exclusion:
...
**`PIXELAD`** (opcode: `ED 94`) — Pixel Address:
...
**`PIXELDN`** (opcode: `ED 93`) — Pixel Down:
...
**`SETAE`** (opcode: `ED 95`) — Set A from E:
...
**`OUTINB`** (opcode: `ED 90`) — Output, Increment, and update flags:
...
**`NEXTREG reg,n`** (opcode: `ED 91`) and **`NEXTREG reg,A`** (opcode: `ED 92`)
...
| `MIRROR A` | `ED 24` | Reverse bit order of A |
| `TEST n` | `ED 27` | AND n with A, update flags, keep A |
| `BSLA DE,B` | `ED 28` | Barrel shift DE left by B bits |
| `BSRA DE,B` | `ED 29` | Barrel shift DE right arithmetic (sign-extend) |
| `BSRL DE,B` | `ED 2A` | Barrel shift DE right logical (zero-fill) |
| `BSRF DE,B` | `ED 2B` | Barrel shift DE right, fill with ones |
| `BRLC DE,B` | `ED 2C` | Barrel rotate DE left circular |
| `MUL D,E` | `ED 30` | Unsigned 8×8 multiply → DE |
| `ADD HL,A` | `ED 31` | HL = HL + A (zero-extended) |
| `ADD DE,A` | `ED 32` | DE = DE + A (zero-extended) |
| `ADD BC,A` | `ED 33` | BC = BC + A (zero-extended) |
| `ADD HL,nn` | `ED 34` | HL = HL + 16-bit immediate |
| `ADD DE,nn` | `ED 35` | DE = DE + 16-bit immediate |
| `ADD BC,nn` | `ED 36` | BC = BC + 16-bit immediate |
| `PUSH nn` | `ED 8A` | Push 16-bit immediate value onto stack |
| `OUTINB` | `ED 90` | OUT(BC) ← (HL), HL++, update flags |
| `NEXTREG r,n` | `ED 91` | Write immediate n to NextReg register r |
| `NEXTREG r,A` | `ED 92` | Write A to NextReg register r |
| `PIXELDN` | `ED 93` | Advance HL to next pixel row in screen memory |
| `PIXELAD` | `ED 94` | HL = screen byte address for pixel at (D, E) |
| `SETAE` | `ED 95` | A = pixel bit mask for column position E[2:0] |
| `JP (C)` | `ED 98` | Jump to page-relative address via port BC |
| `LDIX` | `ED A4` | LDI, skip write if (HL) == A |
| `LDWS` | `ED A5` | (DE) ← (HL), L++, D++ (page-preserving) |
| `LDDX` | `ED AC` | LDD, skip write if (HL) == A |
| `LDIRX` | `ED B4` | Repeat LDIX until BC = 0 |
| `LDPIRX` | `ED B7` | Block copy from 8-aligned source pattern, skip if == A |
| `LDDRX` | `ED BC` | Repeat LDDX until BC = 0 |
