---
description: "The ZX Spectrum Next assembler in Klive IDE provides a comprehensive development environment for the ZX Spectrum Next platform."
---
...
.org $c000              ; ⚠️ Warning Z0370: Address $c000 exceeds $bfff
    ld a, 7
trap:
    jr $
```
...
```
Z0370: Unbanked code address $C000 exceeds typical bank 2 range ($8000-$bfff). This will create a gap
in bank 2 when exporting to NEX. Consider using explicit .bank for separate bank layout.
```
...
- **Z0370**: Unbanked code address exceeds $bfff (typical bank 2 range)
...
- [.dma Pragma Reference](./zx-next-dma) - DMA register group pragma (ZX Spectrum Next)
