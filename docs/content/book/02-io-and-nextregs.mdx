---
description: "Before you can write a single line of useful Z80 code on the ZX Spectrum Next, you need to understand how Z80 programs talk to hardware."
---
...
This chapter covers the fundamentals. First, how the Z80's I/O bus actually works and why the Next's port addresses look the way they do. Then, NextRegs — the FPGA's control panel, accessible through two dedicated ports, that gives you access to everything the Next adds beyond the original Spectrum. If you want the CPU-side background first, see [Z80N: The Next's Z80 CPU](./01-z80n.mdx).
...
The [Z80N instruction set](./01-z80n.mdx) adds two faster alternatives that fold the select and write into a single instruction:
...
The select port (`0x243B`) remembers the last written register number, so if you've just written to a register via the port method, you can skip the select step and read straight from `0x253B`. Don't rely on this across [interrupt](./04-interrupts.mdx) boundaries though—an ISR that touches NextRegs will clobber the selection.
...
- **[The CTC: Counter/Timer Circuit](./05-ctc.mdx)** — uses plain I/O ports `$183B`–`$1B3B` and, optionally, the IM2 mechanism from the [Interrupts](./04-interrupts.mdx) chapter.
