...
The __Klive__ assembler provides you a powerful way to declare macros, and apply them in the code.
While in most programming languages and assemblers the macros are preprocessor constructs and use simple
text replacement, the __Klive__ implementation is different.

> Unlike in C, C++ (and some Z80 Assemblers), __Klive__ macros emit only code (through instructions
...
use a separate markup for a placeholder &mdash; they'd just use `wait` &mdash; __Klive__ applies this markup for 
...
__Klive__ allows you to pass a set of lines as a macro argument. You can invoke `RepeatLight` like this:
...
It's time to deep into the nitty-gritty details of creating and using macros in __Klive__.
...
__Klive__ allows you to invoke a macro from another macro, too. Here is a short sample:
...
The __Klive__ Assembler allows using several parse-time functions with macro arguments the
