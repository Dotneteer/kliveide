...
main:
    ld a, 5
    ld bc, $123b        ; Page bank 5 into slot 1 ($c000-$ffff)
    nextreg bc, a
    call $c000          ; Call into bank 5
    ret

; Explicit bank 5 code
.bank 5
.org $0000
.disp $c000
...
When using `.bank` pragmas:

- Bank 2 is the default for unbanked code - don't manually specify `.bank 2` for simple programs
- Banks 0-1 are ROM (typically not writable)
- Banks 2-111 are available for user programs
- Bank 5+ are commonly used for paged code/data

## Backward Compatibility

The new automatic conveniences don't break existing Next code:

**Traditional explicit multi-bank code still works:**

```z80klive
.model next

.savenex file "legacy.nex"
.savenex ram 768
.savenex border 7
...
The difference is that you can now omit the boilerplate `.bank 2`, `.org $0000`, and `.disp $8000` for simple single-bank programs.
...
Here's a complete example showing various `.savenex` configurations with the new automatic conveniences:

```z80klive
    .model next
    
    ; Only specify what you need - ram, border, and entryaddr have defaults
    .savenex file "demo.nex"
    .savenex core 3, 1, 10
    
    ; Optional: Override defaults if needed
    ; .savenex ram 1792        ; Only if you need more than 768K
    ; .savenex border 5        ; Only if you want a different border color
    
    ; Entry point configuration (optional)
    .savenex stackaddr 0xFF00
    ; .savenex entryaddr $8000    ; Already the default
    ...
    ; Your program code - automatically starts at $8000 and maps to bank 2
Start:
    ld a, 7
    out (0xFE), a
    ret
...
### Simple Single-Bank Programs

For most programs that fit in 16KB:
...
; Your code - automatically handled
main:
    ; Game logic here
    ret
```
...
For larger programs requiring multiple banks:

```z80klive
.model next

.savenex file "bigapp.nex"
.savenex core "3.1.0"

; Main code in bank 2 (unbanked)
main:
    ; Initialize
    call setup_banks
    ; Main loop
...
setup_banks:
    ; Page in additional banks as needed
    ret

; Additional code in bank 5
.bank 5
.org $0000
.disp $c000
...
; More additional code in bank 6
.bank 6
.org $0000
.disp $c000
...
### From Traditional Multi-Bank to Simplified Syntax
...
The simplified version is equivalent and much cleaner. The automatic conveniences handle:
- `.savenex ram 768` (default)
- `.savenex border 7` (default)
- `.savenex entryaddr $8000` (default)
- Bank 2 mapping at $8000 (automatic for unbanked code)
...
- [Z80 Assembler Overview](./z80-assembler) - General assembler documentation
