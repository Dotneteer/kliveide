...
### MSI Installer (.msi) - For enterprise environments and as an alternative

The `.msi` setup file uses the Windows Installer framework. Double-click the file to start the installation process. This format is ideal for:

- Enterprise deployments through Group Policy
- System administrators who need standardized installation packages
- Environments requiring Windows Installer transaction logging and rollback capabilities
- **Alternative installation method** if the NSIS (.exe) installer hangs or takes too long on your system
