...
- an address with a partition Available aliases: `shmem`

## `wp-add`

This command adds a watchpoint for a symbol.

```
wp-add <symbol>[:<type>[:<length>]]
```

Available aliases: `wp`

The watchpoint specification format is `<symbol>[:<type>[:<length>]]` where:
...
Examples:
- `wp-add playerScore` - 8-bit value (default type)
- `wp-add playerScore:w` - 16-bit little-endian value
- `wp-add playerName:s:32` - 32-byte string
- `wp-add inventory:a:64` - 64-byte array
- `wp-add isAlive:f` - boolean flag

## `wp-del`

This command removes a watchpoint by symbol name.
...
The `symbol` parameter is the name of the symbol for which the watchpoint should be removed.

## `wp-ea`

This command erases all watchpoints.
...
This command lists all defined watchpoints.

```
wp-list
```

Available aliases: `wpl`
...
## `show-memory`e partition and the address separated with a colon), for example, `r1:$32ac` (meaning $32ac, provided ROM 1 is paged in);
